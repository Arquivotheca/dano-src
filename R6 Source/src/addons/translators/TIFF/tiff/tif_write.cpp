#ifndef lint
static char rcsid[] = "$Header: /net/bally/be/rcs_rel/src/apps/3dmov/tiff/tif_write.cpp,v 1.1 1996/12/22 23:45:35 pierre Exp $";
#endif
...
TIFFWriteScanline(TIFF* tif, tdata_t buf, uint32 row, tsample_t sample)
{
	...
	status = (*tif->tif_encoderow)(tif, (tidata_t)buf, tif->tif_scanlinesize, sample);
	tif->tif_row++;
	return (status);
}
...
TIFFWriteEncodedTile(TIFF* tif, ttile_t tile, tdata_t data, tsize_t cc)
{
	...
	if (cc > tif->tif_tilesize)
		cc = tif->tif_tilesize;
	if (!(*tif->tif_encodetile)(tif, (tidata_t)data, cc, tile/td->td_stripsperimage))
		return ((tsize_t) 0);
	if (tif->tif_postencode && !(*tif->tif_postencode)(tif))
		return ((tsize_t) -1);
	if (td->td_fillorder != tif->tif_fillorder &&
