...
void mp1Dequantize(
	...
	for (i=0;i<SBLIMIT;i++)
		for (k=0;k<stereo;k++)
			if (bitAlloc[k][i]) {
				nb = bitAlloc[k][i];
				int32 accum = (((sample[k][0][i] >> nb) & 1) == 1) ?
					0 :
					int32_to_fixed32(-1);

				accum += int32_to_fixed32((sample[k][0][i] & ((1<<nb)-1)) >> nb);
					
				outSpectrum[i*2+k] = fixed32_mul(
					accum + (int32_to_fixed32(1) >> nb),
					fixed32_div(
						 int32_to_fixed32(1) << nb+1,
						(int32_to_fixed32(1) << nb+1)-1));
			}
			else
				outSpectrum[i*2+k] = 1;
}
...
void mp1Denormalize(
	int32 ioSpectrum[SSLIMIT*SBLIMIT*2],
	unsigned int scaleIndex[2][3][SBLIMIT],
	const CMpegHeader& header,
	int jsbound) {
	...
	for (i=0;i<SBLIMIT;i++)
		for (j=0;j<stereo;j++)
			ioSpectrum[i*2+j] = fixed32_mul(
				ioSpectrum[i*2+j],
				sDenormalizeScaleInt[scaleIndex[j][0][i]]) >> fixed32_guard_bits;
...
void mp2Denormalize(
	...
	for (i=0;i<sblimit;i++)
		for (j=0;j<stereo;j++) {
			ioSpectrum[j][0][i] *= sDenormalizeScale[scaleIndex[j][seg][i]];
			ioSpectrum[j][1][i] *= sDenormalizeScale[scaleIndex[j][seg][i]];
			ioSpectrum[j][2][i] *= sDenormalizeScale[scaleIndex[j][seg][i]];
		}	
}
		...
		for (j=0;j<3;j++)
			for (k=0;k<stereo;k++)
				if (bitAlloc[k][i]) {
					...
					/* MSB inversion */
					int32 accum = ((sample[k][j][i] >> x-1) & 1) ?
						0 :
						int32_to_fixed32(-1);
						
					/* Form a 2's complement sample */
					accum += int32_to_fixed32(sample[k][j][i] & ((1<<x-1)-1)) >> x-1;
					
					/* Dequantize the sample */
					accum += sDequantOffsetInt[table.data[i][bitAlloc[k][i]].quant];
					outSpectrum[((j*SBLIMIT)+i)*2+k] = fixed32_mul(
						accum,
						sDequantScaleInt[table.data[i][bitAlloc[k][i]].quant]);
				}
				else
...
void mp2Denormalize(
	...
	for (i=0;i<sblimit;i++)
		for (j=0;j<stereo;j++) {
			ioSpectrum[i*2+j] = fixed32_mul(
				ioSpectrum[i*2+j],
				sDenormalizeScaleInt[scaleIndex[j][seg][i]]) >> fixed32_guard_bits;
			ioSpectrum[(SBLIMIT+i)*2+j] = fixed32_mul(
				ioSpectrum[(SBLIMIT+i)*2+j],
				sDenormalizeScaleInt[scaleIndex[j][seg][i]]) >> fixed32_guard_bits;
			// +++++ egcs inline asm screws up here, so use C++ form +++++
			ioSpectrum[(SBLIMIT*2+i)*2+j] = fixed32_mul_round(
				ioSpectrum[(SBLIMIT*2+i)*2+j],
				sDenormalizeScaleInt[scaleIndex[j][seg][i]]) >> fixed32_guard_bits;
		}
}
