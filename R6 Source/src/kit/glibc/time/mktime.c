     ...
     struct tm *tp;
{
  ...
  ret = __mktime_internal (tp, localtime_r, &localtime_offset);
#if HAVE_TZ_NAME
  /* This needs to be set; otherwise a subsequent call to strftime will
     dereference a potentially bad pointer (see the code in strftime.c).
     This bug fix is necessary for apache to run. */
  tp->tm_zone = tzname;
#endif
