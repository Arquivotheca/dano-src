...
//	fFatBits->MoveSelectionTo(fHPixelCount/2, fVPixelCount/2); //center the crosshair
...
TWindow::GetPrefs(int32 overridePixelCount)
{
	...
	/* Now we must also look in COMMON directory */

	int ref = -1;
	if (find_directory (B_USER_SETTINGS_DIRECTORY, &path) == B_OK)
	{
		path.Append(kPrefsFileName);
		if ((ref = open(path.Path(), O_RDONLY)) < 0)
		{
...
{
	(void)rec_position; (void)rec_width; (void)rec_height;
	if (fFatBits->Active())
		ShowInfo(!fShowInfo);
}
...
TInfoView::Draw(BRect updateRect)
{
	...
	sprintf(str, "%ld x %ld  @ %ld pixels/pixel", hPixelCount, vPixelCount,
		pixelSize);
	...
	if (fMagView) {
		...
		if (ch2Showing) {
			MovePenTo(10, h-12);
			sprintf(str, "2) x: %ld y: %ld   y: %i", (int32)pt2.x, (int32)pt2.y,
				abs((int)(pt1.y - pt2.y)));
		...
		if (ch1Showing && ch2Showing) {
			...
			sprintf(str, "1) x: %ld  y: %ld   x: %i", (int32)pt1.x, (int32)pt1.y,
				abs((int)(pt1.x - pt2.x)));
			...
			MovePenTo(10, h-10);
			sprintf(str, "x: %ld  y: %ld", (int32)pt1.x, (int32)pt1.y);
