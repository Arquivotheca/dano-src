//depot/main/src/kit/textencoding/EncodingConversions.cpp#10 - edit change 40908 (text)
#include "UnicodeMappings.h"
#include <UTF8.h>
#include <string.h>
#include <stdio.h>

#if LIBTEXTENCODING_SUPPORTS_CHINESE
				...
				char		substitute)
{
	...
	switch (srcEncoding) {
		...
		case B_ISO13_CONVERSION:
			return (one_to_utf8(iso13tou, src, srcLen, dst, dstLen, substitute));

		case B_ISO14_CONVERSION:
			return (one_to_utf8(iso14tou, src, srcLen, dst, dstLen, substitute));

		case B_ISO15_CONVERSION:
			return (one_to_utf8(iso15tou, src, srcLen, dst, dstLen, substitute));

		case B_MAC_ROMAN_CONVERSION:
			return (one_to_utf8(macromantou, src, srcLen, dst, dstLen, substitute));

		case B_MAC_CENTEURO_CONVERSION:
			return (one_to_utf8(maccenteurotou, src, srcLen, dst, dstLen, substitute));

		case B_MAC_CROATIAN_CONVERSION:
			return (one_to_utf8(maccroatiantou, src, srcLen, dst, dstLen, substitute));

		case B_MAC_CYRILLIC_CONVERSION:
			return (one_to_utf8(maccyrillictou, src, srcLen, dst, dstLen, substitute));

		case B_MAC_GREEK_CONVERSION:
			return (one_to_utf8(macgreektou, src, srcLen, dst, dstLen, substitute));

		case B_MAC_HEBREW_CONVERSION:
			return (one_to_utf8(machebrewtou, src, srcLen, dst, dstLen, substitute));

		case B_MAC_ICELAND_CONVERSION:
			return (one_to_utf8(macicelandtou, src, srcLen, dst, dstLen, substitute));

		case B_MAC_TURKISH_CONVERSION:
			return (one_to_utf8(macturkishtou, src, srcLen, dst, dstLen, substitute));
		...
		case B_MS_WINDOWS_CONVERSION:
			return (one_to_utf8(mswin1252tou, src, srcLen, dst, dstLen, substitute));

		case B_MS_WINDOWS_1253_CONVERSION:
			return (one_to_utf8(mswin1253tou, src, srcLen, dst, dstLen, substitute));

		case B_MS_WINDOWS_1254_CONVERSION:
			return (one_to_utf8(mswin1254tou, src, srcLen, dst, dstLen, substitute));

		case B_MS_WINDOWS_1255_CONVERSION:
			return (one_to_utf8(mswin1255tou, src, srcLen, dst, dstLen, substitute));

		case B_MS_WINDOWS_1256_CONVERSION:
			return (one_to_utf8(mswin1256tou, src, srcLen, dst, dstLen, substitute));

		case B_MS_WINDOWS_1257_CONVERSION:
			return (one_to_utf8(mswin1257tou, src, srcLen, dst, dstLen, substitute));

		case B_MS_WINDOWS_1258_CONVERSION:
			return (one_to_utf8(mswin1258tou, src, srcLen, dst, dstLen, substitute));

		case B_MS_DOS_866_CONVERSION:
			return (one_to_utf8(msdos866tou, src, srcLen, dst, dstLen, substitute));

		case B_MS_DOS_CONVERSION:
			return (one_to_utf8(msdos437tou, src, srcLen, dst, dstLen, substitute));

		case B_MS_DOS_737_CONVERSION:
			return (one_to_utf8(msdos737tou, src, srcLen, dst, dstLen, substitute));

		case B_MS_DOS_775_CONVERSION:
			return (one_to_utf8(msdos775tou, src, srcLen, dst, dstLen, substitute));

		case B_MS_DOS_850_CONVERSION:
			return (one_to_utf8(msdos850tou, src, srcLen, dst, dstLen, substitute));

		case B_MS_DOS_852_CONVERSION:
			return (one_to_utf8(msdos852tou, src, srcLen, dst, dstLen, substitute));

		case B_MS_DOS_855_CONVERSION:
			return (one_to_utf8(msdos855tou, src, srcLen, dst, dstLen, substitute));

		case B_MS_DOS_857_CONVERSION:
			return (one_to_utf8(msdos857tou, src, srcLen, dst, dstLen, substitute));

		case B_MS_DOS_860_CONVERSION:
			return (one_to_utf8(msdos860tou, src, srcLen, dst, dstLen, substitute));

		case B_MS_DOS_861_CONVERSION:
			return (one_to_utf8(msdos861tou, src, srcLen, dst, dstLen, substitute));

		case B_MS_DOS_862_CONVERSION:
			return (one_to_utf8(msdos862tou, src, srcLen, dst, dstLen, substitute));

		case B_MS_DOS_863_CONVERSION:
			return (one_to_utf8(msdos863tou, src, srcLen, dst, dstLen, substitute));

		case B_MS_DOS_864_CONVERSION:
			return (one_to_utf8(msdos864tou, src, srcLen, dst, dstLen, substitute));

		case B_MS_DOS_865_CONVERSION:
			return (one_to_utf8(msdos865tou, src, srcLen, dst, dstLen, substitute));

		case B_MS_DOS_869_CONVERSION:
			return (one_to_utf8(msdos869tou, src, srcLen, dst, dstLen, substitute));

		case B_MS_DOS_874_CONVERSION:
			return (one_to_utf8(msdos874tou, src, srcLen, dst, dstLen, substitute));
		...
		case B_BIG5_CONVERSION:
			return (big5_to_utf8(src, srcLen, dst, dstLen, state, substitute));
		
		case B_GBK_CONVERSION:
			return (gbk_to_utf8(src, srcLen, dst, dstLen, state, substitute));
#endif
	}
				  ...
				  char			substitute)
{
	...
	switch (dstEncoding) {
		...
		case B_ISO13_CONVERSION:
			return (utf8_to_one(iso13tou, src, srcLen, dst, dstLen, substitute));

		case B_ISO14_CONVERSION:
			return (utf8_to_one(iso14tou, src, srcLen, dst, dstLen, substitute));

		case B_ISO15_CONVERSION:
			return (utf8_to_one(iso15tou, src, srcLen, dst, dstLen, substitute));

		case B_MAC_ROMAN_CONVERSION:
			return (utf8_to_one(macromantou, src, srcLen, dst, dstLen, substitute));

		case B_MAC_CENTEURO_CONVERSION:
			return (utf8_to_one(maccenteurotou, src, srcLen, dst, dstLen, substitute));

		case B_MAC_CROATIAN_CONVERSION:
			return (utf8_to_one(maccroatiantou, src, srcLen, dst, dstLen, substitute));

		case B_MAC_CYRILLIC_CONVERSION:
			return (utf8_to_one(maccyrillictou, src, srcLen, dst, dstLen, substitute));

		case B_MAC_GREEK_CONVERSION:
			return (utf8_to_one(macgreektou, src, srcLen, dst, dstLen, substitute));

		case B_MAC_HEBREW_CONVERSION:
			return (utf8_to_one(machebrewtou, src, srcLen, dst, dstLen, substitute));

		case B_MAC_ICELAND_CONVERSION:
			return (utf8_to_one(macicelandtou, src, srcLen, dst, dstLen, substitute));

		case B_MAC_TURKISH_CONVERSION:
			return (utf8_to_one(macturkishtou, src, srcLen, dst, dstLen, substitute));
		...
		case B_MS_WINDOWS_1250_CONVERSION:
			return (utf8_to_one(mswin1250tou, src, srcLen, dst, dstLen, substitute));

		case B_MS_WINDOWS_1251_CONVERSION:
			return (utf8_to_one(mswin1251tou, src, srcLen, dst, dstLen, substitute));

		case B_MS_WINDOWS_CONVERSION:
			return (utf8_to_one(mswin1252tou, src, srcLen, dst, dstLen, substitute));

		case B_MS_WINDOWS_1253_CONVERSION:
			return (utf8_to_one(mswin1253tou, src, srcLen, dst, dstLen, substitute));

		case B_MS_WINDOWS_1254_CONVERSION:
			return (utf8_to_one(mswin1254tou, src, srcLen, dst, dstLen, substitute));

		case B_MS_WINDOWS_1255_CONVERSION:
			return (utf8_to_one(mswin1255tou, src, srcLen, dst, dstLen, substitute));

		case B_MS_WINDOWS_1256_CONVERSION:
			return (utf8_to_one(mswin1256tou, src, srcLen, dst, dstLen, substitute));

		case B_MS_WINDOWS_1257_CONVERSION:
			return (utf8_to_one(mswin1257tou, src, srcLen, dst, dstLen, substitute));

		case B_MS_WINDOWS_1258_CONVERSION:
			return (utf8_to_one(mswin1258tou, src, srcLen, dst, dstLen, substitute));

		case B_MS_DOS_866_CONVERSION:
			return (utf8_to_one(msdos866tou, src, srcLen, dst, dstLen, substitute));

		case B_MS_DOS_CONVERSION:
			return (utf8_to_one(msdos437tou, src, srcLen, dst, dstLen, substitute));

		case B_MS_DOS_737_CONVERSION:
			return (utf8_to_one(msdos737tou, src, srcLen, dst, dstLen, substitute));

		case B_MS_DOS_775_CONVERSION:
			return (utf8_to_one(msdos775tou, src, srcLen, dst, dstLen, substitute));

		case B_MS_DOS_850_CONVERSION:
			return (utf8_to_one(msdos850tou, src, srcLen, dst, dstLen, substitute));

		case B_MS_DOS_852_CONVERSION:
			return (utf8_to_one(msdos852tou, src, srcLen, dst, dstLen, substitute));

		case B_MS_DOS_855_CONVERSION:
			return (utf8_to_one(msdos855tou, src, srcLen, dst, dstLen, substitute));

		case B_MS_DOS_857_CONVERSION:
			return (utf8_to_one(msdos857tou, src, srcLen, dst, dstLen, substitute));

		case B_MS_DOS_860_CONVERSION:
			return (utf8_to_one(msdos860tou, src, srcLen, dst, dstLen, substitute));

		case B_MS_DOS_861_CONVERSION:
			return (utf8_to_one(msdos861tou, src, srcLen, dst, dstLen, substitute));

		case B_MS_DOS_862_CONVERSION:
			return (utf8_to_one(msdos862tou, src, srcLen, dst, dstLen, substitute));

		case B_MS_DOS_863_CONVERSION:
			return (utf8_to_one(msdos863tou, src, srcLen, dst, dstLen, substitute));

		case B_MS_DOS_864_CONVERSION:
			return (utf8_to_one(msdos864tou, src, srcLen, dst, dstLen, substitute));

		case B_MS_DOS_865_CONVERSION:
			return (utf8_to_one(msdos865tou, src, srcLen, dst, dstLen, substitute));

		case B_MS_DOS_869_CONVERSION:
			return (utf8_to_one(msdos869tou, src, srcLen, dst, dstLen, substitute));

		case B_MS_DOS_874_CONVERSION:
			return (utf8_to_one(msdos874tou, src, srcLen, dst, dstLen, substitute));
...
#if LIBTEXTENCODING_SUPPORTS_CHINESE
		case B_BIG5_CONVERSION:
			return (utf8_to_big5(src, srcLen, dst, dstLen, state, substitute));
		
		case B_GBK_CONVERSION:
			return (utf8_to_gbk(src, srcLen, dst, dstLen, state, substitute));
#endif
	}
