...
const char *gName = "EPSON Stylus Photo EX";
const tPrinterInfo gPrinterInfo =
{
	gName,						// Name
	ID_STYLUS_PHOTO_EX,			// ID
	7,							// DefaultPage
	32,							// Nb Black Nozzles
	32,							// Nb Color Nozzles
...
const tPage gPageLimits[2] = {
		...
		{ "_max",					42, 4125, 42, 42, 15600, 198, M_UNKNOWN_FORMAT }
						};
...
const uint8 paperFeedValue[sizeof(gPage)/sizeof(tPage)][sizeof(gPaper)/sizeof(tPrinterPaper)] = {
	...
	{ 1, 1, 1, 3, 0, 3 },	// legal
	{ 1, 1, 1, 3, 0, 3 },	// letter
	{ 1, 1, 1, 3, 0, 3 },	// letter landscape
	{ 1, 1, 1, 3, 0, 3 },	// executive
	{ 1, 1, 1, 3, 0, 3 },	// half letter
	{ 1, 1, 1, 3, 0, 3 },	// a3
	{ 1, 1, 1, 3, 0, 3 },	// a4
	{ 1, 1, 1, 3, 0, 3 },	// a4 landscape
	{ 1, 1, 1, 3, 0, 3 },	// a5
	{ 1, 1, 1, 3, 0, 3 },	// a6
	{ 1, 1, 1, 3, 0, 3 },	// B
	{ 1, 1, 1, 3, 0, 3 },	// B4
...
const tRes2Paper gRes2Paper[] =	{	{1, 0, 0,	2, 0, 0 },	// 360x360 normal : double - ok
									{2, 0, 1,	1, 2, 0 },	// 720x360 normal : single - ok
									{3, 0, 2,	0, 2, 1 },	// 720x720 normal : micro  - ok
									{2, 1, 1,	1, 2, 1 },	// 720x360 couche : single - ok
									{3, 2, 1,	1, 2, 1 },	// 720x720 couche : single - ok
									{4, 2, 2,	0, 2, 1 },	// 1440x720 couche : micro - ok
									{1, 3, 1,	2, 2, 1 },	// 360x360 transp : Double - ok
									{3, 4, 1,	1, 2, 1 },  // 720x720 photo  : single - ok
									{4, 4, 2,	0, 2, 1 },  // 1440x720 photo  : micro - ok
									{3, 5, 1,	0, 2, 1 },	// 720x720 film   : micro  - ok(?)
									{4, 5, 2,	0, 2, 1 }	// 1440x720 film   : micro  - ok(??)
								};
