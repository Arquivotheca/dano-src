	...
	size_t size)
{
	...
	*buf = 0;
	...
	{
	case B_MEDIA_RAW_AUDIO: {
		sprintf(buf, "raw_audio;%g;%d;%x;%d;%x", f.u.raw_audio.frame_rate, f.u.raw_audio.channel_count, 
	...
	case B_MEDIA_RAW_VIDEO: {
		...
		if (f.u.raw_video.orientation < B_VIDEO_TOP_LEFT_RIGHT || f.u.raw_video.orientation > B_VIDEO_BOTTOM_LEFT_RIGHT) {
			sprintf(dig, "%d", f.u.raw_video.orientation);
		}
		else {
			const char * o[] = { "TopLR", "BotLR" };
			ostr = o[f.u.raw_video.orientation-1];
		}
		sprintf(buf, "raw_video;%g;%d;%d;%d;%s;%d;%d;%d;%d;%d;%d;%d;%d",
	...
	case B_MEDIA_ENCODED_AUDIO: {
		...
		sprintf(buf, "caudio;%s;%g;%d;(%g;%d;%x;%d;%x)", mstr, f.u.encoded_audio.bit_rate, 
	...
	case B_MEDIA_ENCODED_VIDEO: {
		...
		if (f.u.encoded_video.output.orientation < B_VIDEO_TOP_LEFT_RIGHT || f.u.encoded_video.output.orientation > B_VIDEO_BOTTOM_LEFT_RIGHT) {
			sprintf(ddig, "%d", f.u.raw_video.orientation);
		}
		else {
			const char * o[] = { "TopLR", "BotLR" };
			ostr = o[f.u.encoded_video.output.orientation-1];
		}
		sprintf(buf, "cvideo;%s;%g;%g;%d;(%g;%x;%d;%d;%d;%d;%s;%d;%d)", cstr, 
	...
	ASSERT(strlen(buf) < in_size);	/* make sure we don't overwrite */
	return ok;
}
