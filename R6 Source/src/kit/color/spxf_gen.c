...
/*
	...
	Windows Revision Level:
		$Workfile:   SPXF_GEN.C  $
		$Logfile:   O:\pc_src\dll\stdprof\spxf_gen.c_v  $
		$Revision:   2.2  $
		$Date:   04 Apr 1994 13:03:14  $
		$Author:   lsh  $

	SCCS Revision:
	@(#)spxf_gen.c	1.55	12/22/97
...
/* check to see if this xform should be written to disk */
	if (writePTs())
	{
		...
		Status = SpXformGetDataFromCP(PTRefNum, 0, &size, &ptData);
				...
				SpXform_t       FAR *Xform)
{
	...
	*Xform = NULL;
	...
	PTStat = PTNewMonoPT((ResponseRecord FAR *) gTRC,
				gridsize, (bool)invert, &PTRefNum);
...
/* Make the fut - set the modes  */
	if (adapt) PTmodes.adaptMode = KCP_VONKRIESD50_ADAPTATION; /* VonKries adapation for LCS */
	else PTmodes.adaptMode = KCP_NO_ADAPTATION; /* no adaptation for pt2pf display pf generation */
	...
	PTStat = PTNewMatGamAIPT ((FixedXYZColor FAR *) rXYZ,
							...
							(ResponseRecord FAR *) gTRC,
							(ResponseRecord FAR *) bTRC, 
							gridsize, (bool)invert, &PTmodes, &PTRefNum);
	if (KCP_SUCCESS != PTStat)
		return SpStatusFromPTErr(PTStat);
...
/* Make the fut - set the modes  */
	...
	if (adapt)
		PTmodes.adaptMode = KCP_VONKRIESD50_ADAPTATION; 
	else
...

static SpStatus_t SetWtPt(SpProfile_t Profile,
