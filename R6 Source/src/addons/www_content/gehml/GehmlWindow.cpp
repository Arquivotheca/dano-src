...
// Mmmmmm....
#define BLooper GehmlWindow
#include <Handler.h>
#undef BLooper
#define BWindow GehmlWindow
#include <Looper.h>
#undef BWindow
#define BTabView GehmlWindow
#include <View.h>
#undef BTabView
...
BMessage * _CEventPort_::GenerateMessage()
{
	...
	_CEvent_ *msg = (_CEvent_ *)(*m_nextBunch + m_eventBase);
...
GehmlWindow::GehmlWindow(BStringMap &attr) : GehmlRoot(attr)
{
	...
	if ((s=attr.Find("decor"))) {
		if (*s == "true") m_look = B_NORMAL_WINDOW_LOOK;
	}
}
...
GehmlWindow::SetParent(const binder_node &parent)
{
	...
	Lock();
	if (m_parent) m_parent["removeChild"](&This,NULL);
	m_parent = parent;
	checkpoint
	if (m_namespace) {
	checkpoint
	...
	if (m_parent) m_parent["addChild"](&This,NULL);
	Unlock();
...
GehmlWindow::OpenSession(window_look look, window_feel feel, uint32 flags, int32 workspace)
{
	...
	((GroupNamespace*)Namespace())->AddProperty("windowID",m_windowToken);
}
...
//	printf("::Update ");updateRegion.PrintToStream();
