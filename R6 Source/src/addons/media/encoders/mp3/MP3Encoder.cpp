...
const double default_bit_rate = 160000.0;
...
MP3Encoder::GetCodecInfo(media_codec_info *mci) const
{
	...
	strcpy(mci->pretty_name, "LAME MP3 Encoder");
...
{
	if (m_is_wav_file == false && m_is_avi_file == false)
		return B_OK;
...
MP3Encoder::StartEncoder(void)
{
	...
	if(m_format.channel_count == 1)
		curConfig.mode = mp3_config::MODE_MONO;
...
MP3Encoder::SetParameterValue(int32 id, bigtime_t when, const void *value, size_t size)
{
	...
	case P_MODE:
		if(size < 4)
			return;
		m_config->mode = mp3_config::mode_t(val);
		break;
...
MP3Encoder::InitParameterWeb(mp3_config* config)
{
	...
	p = g->MakeDiscreteParameter(
		P_MODE, B_MEDIA_NO_TYPE, "Channel Mode", B_GENERIC);
	p->AddItem(mp3_config::MODE_STEREO, "stereo");
	p->AddItem(mp3_config::MODE_DUAL_CHANNEL, "dual-channel");
	p->SetValue(&config->mode, sizeof(int32), 0LL);
	
	g = main->MakeGroup("Options");
	
	p = g->MakeDiscreteParameter(
		P_PSYCHO, B_MEDIA_NO_TYPE, "Psychoacoustic Modeling", B_ENABLE);
	p->AddItem(0, "");
	p->AddItem(1, "");
	p->SetValue(&config->psycho, sizeof(bool), 0LL);
...
//	p = g->MakeDiscreteParameter(
//		P_PRIVATE, B_MEDIA_NO_TYPE, "Label 'Private'", B_ENABLE);
//	p->AddItem(0, "");
//	p->AddItem(1, "");
//	p->SetValue(&config->priv, sizeof(bool), 0LL);
//
//	p = g->MakeDiscreteParameter(
