...
METHODDEF(boolean) get_app1(j_decompress_ptr cinfo)
{
  	INT32 	length;
 	UINT8 	b[JFIF_LEN];
  	int 	buffp;
	...
	if (length == 40) {
		mjpeg_header *header = &cinfo->mjpegHeader;
		int32 *headerc = (int32 *)header;

	printf("length of app1 = %ld, mjpeg header %ld\n",length-2,sizeof(mjpeg_header));

		// possible endian problem!
		for (i = 0;i < 40 / sizeof(INT32);i++) {
	
			int32 one, two;
			INPUT_2BYTES(cinfo, one, return FALSE);
			INPUT_2BYTES(cinfo, two, return FALSE);
			
			#if defined(__INTEL__) || defined(__ARMEL__)	/* FIXME: This should probably use <endian.h> for the right define */
	  		headerc[i] = one >> 16 | two;
	  		#else
	  		headerc[i] = two >> 16 | one;  		
	  		#endif
	  		length -= sizeof(int32);
	  	}
	
		printf("tag:%4x %4x %4x fO:%x\n", header->unused, header->tag, header->fieldSize, header->offsetToNextField);
	} else {
		while (length--)
	      INPUT_BYTE(cinfo, b[0], return FALSE);	
	}
...
//  //	printf("header->offsettonextfield = %ld %x\n",header->offsettonextfield,cinfo->myFieldHook);
//  
//  	if ((header->offsettonextfield) && (cinfo->myFieldHook)) {
//		cinfo->myFieldHook(cinfo->myUserData, header->offsettonextfield, header);
//  	}
//  
//// 	printf("in jpeg, offset = %x\n",header->offsettonextfield);
//
////	printf("tag = %ld\n",header->tag);
////	printf("fieldsize = %ld\n",header->fieldsize);
////	printf("paddedfieldsize = %ld\n",header->paddedfieldsize);
////	printf("offsettonextfield = %ld\n",header->offsettonextfield);
////
////	printf("quanttableoffset = %ld\n",header->quanttableoffset);
////	printf("huffmantableoffset = %ld\n",header->huffmantableoffset);
////	printf("startofimageoffset = %ld\n",header->startofimageoffset);
////	printf("startofscanoffset = %ld\n",header->startofscanoffset);
////	printf("startofdataoffset = %ld\n",header->startofdataoffset); 
//  
