...
/* static */
int URL::GetEscapedLength(const char *c)
{
	int len = 0;
	while (*c) {
...
/* static */
char *URL::EscapePathString(bool escape_all, char *outString, const char *inString, size_t inLen)
{
	return EscapeString(escape_all, outString, inString, inLen, kValidInPath, kEscapeInPath);
}

/* static */
char *URL::EscapeQueryString(bool escape_all, char *outString, const char *inString, size_t inLen)
{
	return EscapeString(escape_all, outString, inString, inLen, kValidInQuery, kEscapeInQuery);
}

/* static */
char* URL::EscapeString(bool escape_all, char *outString, const char *inString, size_t inLen,
...
URL::URL(const char *urlString, bool escape_all)
	:	fData(0)
{
...
URL::URL(const URL &baseURL, const char *relativePath, bool escape_all)
	:	fData(0)
{
	SetTo(baseURL, relativePath, escape_all);
}

URL::URL(const char *name, const BMessage *message)
	:	fData(0)
{
...
URL& URL::SetTo(const char *urlString, bool escape_all)
{
...
URL& URL::SetTo(const URL &baseURL, const char *relativePath, bool escape_all)
{
...
URL& URL::SetToInternal(const URL &baseURL, const char *relativePath, bool escape_all)
{
...
void URL::AppendToInternal(StringBuffer &str, bool minimal) const
{
	if (!fData)
		return;
...
void URL::GetString(char *buffer, size_t size) const
{
	StringBuffer tmp;
	AppendTo(tmp);
	if (tmp.Length() < size)
...
void URL::AppendTo(StringBuffer &str) const
{
	AppendToInternal(str, false);
}

QueryMethod URL::GetQueryMethod() const
{
	if (fData)
...
void URL::GetUnescapedFileName(char *out, int size) const
{
	const char *fname = 0;
...
void URL::SetFragment(const char *fragment)
{
	...
	URL tmp(GetScheme(), GetHostName(), GetPath(), GetPort(),
	        fragment, GetUserName(), GetPassword(),
	        GetQuery(), GetQueryMethod(), GetUploadQuery());
	SetTo(tmp);
}

void URL::SetQuery(const char *query, bool escape_all)
{
	if (fData && fData->fScheme == kJavaScript) return;
...
void URL::SetUploadQuery(const char *uploadQuery, bool escape_all)
{
	if (fData && fData->fScheme == kJavaScript) return;
...
void URL::SetQueryMethod(QueryMethod method, bool compressIfPost)
{
	if (fData == 0)
...
const char* URL::GetExtension() const
{
...
void URL::GetUnescapedPath(char *out, int size) const
{
	UnescapeString(out, GetPath(), size);
}

void URL::GetUnescapedQuery(char *out, int size) const
{
	UnescapeString(out, GetQuery(), size);
}

const char* URL::GetQuery() const
{
	if (fData == 0)
		return "";
...
const char* URL::GetUploadQuery() const
{
	if (fData == 0)
		return "";
...
void URL::AddQueryParameter(const char *name, const char *value, bool escape_all)
{
	if (!IsValid())
		return;
...
void URL::AddUploadQueryParameter(const char *name, const char *value, bool escape_all)
{
	if (!IsValid())
		return;
...
status_t URL::GetQueryParameterInternal(const char *query, const char *name, BString *out_value) const
{
	if( query ) {
		BString buffer;
		while( *query ) {
...
status_t URL::GetUploadQueryParameter(const char* name, BString* out_value) const
{
	return GetQueryParameterInternal(GetUploadQuery(), name, out_value);
}

status_t URL::GetQueryParameter(const char* name, BString* out_value) const
{
	return GetQueryParameterInternal(GetQuery(), name, out_value);
}

status_t URL::RemoveQueryParameter(const char *name)
{
...
status_t URL::RemoveUploadQueryParameter(const char *name)
{
...
status_t URL::ReplaceUploadQueryParameter(const char *name, const char *value, bool addIfNotPresent, bool escape_all)
{
	char *query = (char*) GetUploadQuery();
	if(query) {
		BString buffer;
...
status_t URL::ReplaceQueryParameter(const char *name, const char *value, bool addIfNotPresent, bool escape_all)
{
	char *query = (char*) GetQuery();
//	char *query_start = query;
	if(query) {
...
int URL::LookupScheme(const char *scheme)
{
...
int URL::LookupScheme(const char *scheme, int len)
{
	for (int i = 0; kSchemes[i] != 0; i++)
...
void URL::AddToMessage(const char *name, BMessage *message) const
{
	if (fData == 0) {
		// This is an invalid URL, add a zero length member to indicate
		// this.
		message->AddData(name, kURLType, 0, 0, false, 1);
		return;
	}
	...
	// Add fData to message
	message->AddData(name, kURLType, fData, len, false, 1);
}

void URL::ExtractFromMessage(const char *name, const BMessage *message)
{
	const void *data;
	ssize_t len;
	if (message->FindData(name, kURLType, 0, &data, &len) != B_OK) {
		Reset();
		return;
	}
...
void URL::GetMinimalString(StringBuffer &buf) const
{
...
bool URL::Equals(const char *str) const
{
	URL tmp(str);
	return Equals(tmp);
}

bool URL::operator==(const char *urlString) const
{
...
void URL::ParsePathURL(const char *_src, bool escape_all)
{
	const char *src = _src;
...
void URL::ParseMailToURL(const char *src)
{
	char *dest = fData->fStrings;
	fData->fPort = 0;
...
void URL::ParseJavaScriptURL(const char *url)
{
	fData->fPort = 0;
	char *dest = fData->fStrings;
	*dest++ = '\0';	// user
...
/* static */
char *URL::ExtractQueryString(char *dest, const char **src, bool escape_all) {
	int query_length = 0;
	if (**src == '?') {
		++(*src);
