...
extern long _kget_tzfilename_ __P((char *));
  ...
  if ((size_t) ((char *) (h + 1) + tzstring_last_buffer_size - p) < needed)
    {
      ...
      h->next = malloc (sizeof *h + buffer_size);
      if (h->next == NULL)
	return NULL;
      h = h->next;
      h->next = NULL;
     ...
     int always;
{
  ...
  long ret;
  char kernel_tzname[MAXPATHLEN+1];
  ...
  if (tz == NULL) {
    /* No user specification; use the site-wide default.  */
	  tz = &kernel_tzname[0];
	  ret = _kget_tzfilename_(kernel_tzname);
	  if (ret != 0) 
		  tz = "Factory";
  }
  else if (*tz == '\0')
