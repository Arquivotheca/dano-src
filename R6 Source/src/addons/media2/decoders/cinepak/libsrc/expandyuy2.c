...
static unsigned char ScaleTable5 [] = {
...
static unsigned char ScaleTable6 [] = {
	0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,	// underflow
	0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
	0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
	0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
...
ExpandDetailCodeBookYUY2 (
    ...
    int iNumCodes;
printf("ExpandDetailCodeBookYUY2\n");
    ...
    switch (cbType) {
	    ...
	    {
			unsigned long eax,ebx,ecx,edx,ebp;
...
//		pD->L0=pC->Y[0]|(pC->U<<8)|(pC->Y[1]<<16)|(pC->V<<24);
//		pD->L1=pC->Y[2]|(pC->U<<8)|(pC->Y[3]<<16)|(pC->V<<24);
#if 1
		edx=0x8094+(pC->U<<8)+(pC->U*3)*2;
		
		eax=(0x3da4e4+((pC->V<<13)+edx)*5)>>7;
		
		ecx=((eax&0xff00)<<16)|(edx&0xff00);	// 0 U 0 V
		
		ecx=((ecx&0xffff)<<16)|((ecx&0xffff0000)>>16);
		...
		eax|=(pC->Y[0]<<8);
		edx=(eax*5)>>5;
		eax=(eax&0xffff0000)+(((eax&0xffff)+(ebp&0xffff))&0xffff);
		eax=(eax&0xffff0000)+(((eax&0xffff)-(edx&0xffff))&0xffff);
		
		eax=eax>>8;
	//	printf("eax was %08x, ecx was %08x\n",eax,ecx);
		eax|=ecx;	// Y0 U Y1 V
		
//		eax=((eax&0xff00ff00)>>8)|((eax&0x00ff00ff)<<8);
		pD->L0=eax;
		printf("val=%08x, first=%02x\n",eax,*((unsigned char *)&eax));
		
		eax=(pC->Y[3]<<8);
		eax+=ebp;
		edx=(eax*5)>>5;
		eax-=edx;
		eax=(eax&0xffffff00)<<16;
		
		eax=(eax&0xffff00ff)|(pC->Y[2]<<8);
		edx=eax*5;
		eax=(eax&0xffff0000)|(((eax&0xffff)+(ebp&0xffff))&0xffff);
		edx=edx>>5;
		eax=(eax&0xffff0000)|(((eax&0xffff)-(edx&0xffff))&0xffff);
...
printf("ExpandSmoothCodeBookYUY2\n");
    iNumCodes = swiz2 (pCBin->SizeType.SwizSize >> 16) - sizeof (SIZETYPE);
