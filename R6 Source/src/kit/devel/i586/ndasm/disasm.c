...
long disasm(unsigned char *data, 	   /* inst data */
			char *output, 			   /* output buffer to hold disassembled inst */
			int segsize, 			   /* usually 32 */
			long curaddr,			   /* address of instruction */
			int autosync,			   /* autosync flag */
			int (*lookup_symbol)(void *context, /* Caller specified lookup routine */
								 long addr,
								 long *sym_addr,
								 char *sym_name,
								 int max_name_length,
								 int is_lower),		   
			void *context)			   /* data (usually symbol table)
										* passed to lookup_symbol */
{
    struct itemplate **p;
    int length = 0;
    char *segover;
...
#define UNAME_SIZE 512
    ...
    for (i = 0; i < elements(ico); i++)
        if ((*p)->opcode == ico[i]) {
            ...
            sprintf(hold, "%s%s", icn[i], whichcond(ins.condition));
            slen += sprintf(output+slen, "%s", hold);
			for(j=0; j < 6-strlen(hold); j++)
				output[slen++] = ' ';
            break;
        }
    if (i >= elements(ico)) {
		char *name = insn_names[(*p)->opcode];
		int len = strlen(name);
		char *format;
		if (len > 6)
			format = "%-10s";
			...
			format = "%-6s";
        slen += sprintf(output+slen, format, insn_names[(*p)->opcode]);
	}
    ...
    for (i=0; i<(*p)->operands; i++) {
        ...
        if (ins.oprs[i].segment & SEG_RELATIVE) {
            ins.oprs[i].offset += curaddr + length;
            /*
             * sort out wraparound
             */
        ...
        if (((*p)->opd[i] & (REGISTER | FPUREG)) ||
			...
			if (lookup_symbol  &&
				(*output == 'j' || strncmp(output, "call", 4) == 0) &&
				lookup_symbol(context, ins.oprs[i].offset, &naddr, nname, sizeof(nname), 1) >= 0) {
				char *thename = nname;
				char *stub = "";
				
				if (strncmp(thename, "_imp_", 5) == 0) {
					thename += 5;
					stub = " [stub]";
				}
...
#if !_BUILDING_kernel				
				if (!(*stub == '\0'  &&  nname[0] == '_')) {
					if (MWUnmangle(thename, uname, UNAME_SIZE)) {
						thename = uname;
					}
				} 
#endif

				if ((ins.oprs[i].offset-naddr) != 0)
					slen += sprintf(output+slen, "%s%s+0x%x ", thename, stub,
									ins.oprs[i].offset-naddr);
				else
					slen += sprintf(output+slen, "%s%s ", thename, stub);
            ...
            if (ins.oprs[i].segment & SEG_DISP8) {
				...
				if (lookup_symbol  &&
					(*output == 'j' || strncmp(output, "call", 4) == 0) &&
					lookup_symbol(context, ins.oprs[i].offset, &naddr,
