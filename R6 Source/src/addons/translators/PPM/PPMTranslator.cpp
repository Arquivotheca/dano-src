...
public:
	infoFiller()
		{
			sprintf(translatorInfo, "PPM image translator v%d.%d.%d, %s",
					...
					while (true) {
						...
						if (sscanf(ptr, "%31[a-zA-Z_0-9] =", name) != 1) {
							...
							if (!strcmp(name, "color_space")) {
								...
								ptr++;
								if (sscanf(ptr, "%d", &g_settings.out_space) != 1) {
				...
				if (g_settings.settings_touched) {
					...
					if (f) {
						fprintf(f, "# PPMTranslator settings version %d.%d.%d\n", 
							translatorVersion>>8, (translatorVersion>>4)&0xf,
							translatorVersion&0xf);
	...
	BPositionIO * outDestination)
{
	...
	if (err < B_OK) {
		dprintf("read_ppm_header() error %s [%x]\n", strerror(err), err);
		return err;
	}
	...
	if (outType == PPM_TYPE) {
		...
		if (err == strlen(magic)) err = 0;
		comment = NULL;
#if defined(_PR3_COMPATIBLE_)	/* R4 headers? */
		if ((ioExtension != NULL) && !ioExtension->FindString(B_TRANSLATOR_EXT_COMMENT, (const char **)&comment)) {
#else
		if ((ioExtension != NULL) && !ioExtension->FindString("/comment", (const char **)&comment)) {
#endif
			err = write_comment(comment, outDestination);
		}
		if (err == B_OK) {
			char data[40];
			sprintf(data, "%d %d %d\n", width, height, max);
			err = outDestination->Write(data, strlen(data));
			if (err == strlen(data)) err = 0;
		}
		/* header done */
	}
	else {
		...
		dprintf("PPMTranslator: Write() returns %x\n", err);
#if DEBUG
		{
...
public:
		PPMView(
			...
			BView(frame, name, resize, flags)
			{
				...
				r.top = r.bottom + 10;
				r.bottom = r.top + 24;
				r.right = r.left + be_plain_font->StringWidth("Input Color Space") +
					be_plain_font->StringWidth("System Palette 8 bits") + 32;

				mField = new BMenuField(r, "Color Space Field", "Input Color Space", mMenu);
				...
				r.top = mField->Frame().bottom + 5;
				r.bottom = r.top + 10;
				r.right = r.left + be_plain_font->StringWidth("Write ASCII") + 20;
				mAscii = new BCheckBox(r, "Write ASCII", "Write ASCII", msg);
...
//				GetFontHeight(&fh);
//				char str[100];
//				sprintf(str, "PPMTranslator version %d.%d.%d", 
//							translatorVersion>>8, (translatorVersion>>4)&0xf,
//							translatorVersion&0xf);
//				SetLowColor(ViewColor());
//				DrawString(str, BPoint(20, (fh.ascent+fh.descent*2+fh.leading)*2));
			}
...
#else
				if (!message->FindInt32("bits/space", (int32*)&space)) {
#endif
...
#else
						if (m && !m->FindInt32("bits/space", &s) && (s == space)) {
#endif
	...
	BRect * outExtent)
{
	...
	PPMView * v = new PPMView(*outExtent, "PPMTranslator Settings", B_FOLLOW_NONE, B_WILL_DRAW);
	*outView = v;
