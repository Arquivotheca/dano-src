...
Resource::QueuedRequest::~QueuedRequest()
{
	...
	listener = (BHandler*)0xDEADBEEF;
};

void Resource::QueuedRequest::SetListener(BHandler *_listener)
{
...
Resource::QueuedRequest & Resource::QueuedRequest::operator=(const Resource::QueuedRequest &r)
{
...
Resource::QueuedRequest::QueuedRequest(const Resource::QueuedRequest &r)
{
	listener = NULL;
	*this = r;
};

Resource::Resource(const BUrl &url)
	:
		fURL(url),
		fHashNext(0),
		fState(kStateNew),
...
#if DEBUG_RESOURCE_LEAKS
	BAutolock l(resource_access.Lock());
...
const BUrl& Resource::GetURL() const
{
	return fURL;
}
...
void Resource::HandleContentInitialized()
{
	...
	LockQueue();
	BVector<QueuedRequest> createRequests(fQueuedCreateRequests);
	...
	for (int requestIndex = 0; requestIndex < requestCount; requestIndex++) {
		request = createRequests.EditItemAt(requestIndex);
		bool failed = false;
		if (!(request.flags & LOAD_ON_ERROR) && fLoadError.Length() > 0) {
			...
			atom_ptr<ContentInstance> instance;
...
#endif
#warning "Fix the hell out of me"
...
//
void Resource::CreateContentInstance(BHandler *listener, uint32 id, const BMessage &userData,
...
#warning "Fix the fuck out of me"
//			int32 recursion = 0;
...
//			msg.AddInt32("id", id);
//			msg.AddMessage("user_data", &userData);
//			if (instance) msg.AddAtom("instance", instance);
//			else {
...
//			if (msg.what == CONTENT_INSTANCE_ERROR) ReleaseReference();
		...
		LockQueue();
		QueuedRequest & request = fQueuedCreateRequests.EditItemAt(fQueuedCreateRequests.AddItem());
...
//		msg.AddString("error_msg", fLoadError.String());
		...
		while (fQueuedCreateRequests.CountItems() > 0) {
			QueuedRequest &request = fQueuedCreateRequests.EditItemAt(0);
...
#warning "Fix the hell out of me"	
//			fQueuedCreateRequests.ItemAt(0).Listener()->PostMessage(sendMessage);
//			fQueuedCreateRequests.RemoveItemsAt(0);
		}
...
Content::ptr Resource::GetContent() const
{
	return fContent;
}
...
bool Resource::GetRefresh(BUrl &url, bigtime_t *time)
{
...
void Resource::SetRefresh(BUrl &url, bigtime_t time)
{
	fRefreshURL = url;
	fRefreshInterval = time;
}

void Resource::SetReferrer(const BUrl &url)
{
	fReferrer = url;
}

const BUrl& Resource::GetReferrer() const
{
	return fReferrer;
}

bool Resource::CancelRequest(uint32 id, BHandler *listener)
{
	LockQueue();
	...
	for (int i = fQueuedCreateRequests.CountItems() - 1; i >= 0; i--)
		if (fQueuedCreateRequests.ItemAt(i).id == id &&
			fQueuedCreateRequests.EditItemAt(i).Listener() == listener) {
			fQueuedCreateRequests.RemoveItemsAt(i);
			removed = true;
...
status_t Resource::ReadFromStream(Protocol *stream, size_t contentLength)
{
	// Read the file data
