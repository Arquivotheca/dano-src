...
typedef struct {		/* portion of the overlay buffer displayed in an overlay window */
	uint16	h_start;	/* the pixel at h_start,v_start maps to the pixel at */
	uint16	v_start;	/*   h_start,v_start in the overlay_window */
	uint16	width;		/* the pixel at h_start+width-1,v_start+height-1 maps to the pixel */
	uint16	height;		/*   at h_start+width-1,v_start+height-1 in the overlay_window */
} overlay_view;			/* this relationship defines the scaling for the overlay */

enum {
	B_OVERLAY_COLOR_KEY = 1 << 0,
	B_OVERLAY_CHROMA_KEY = 1 << 1,
	B_OVERLAY_HORIZONTAL_FITLERING = 1 << 2,
	B_OVERLAY_VERTICAL_FILTERING = 1 << 3,
	B_OVERLAY_HORIZONTAL_MIRRORING = 1 << 4,
	B_OVERLAY_KEYING_USES_ALPHA = 1 << 5,
	B_OVERLAY_FROM_VIP = 1 << 31	/* HACK */
};
