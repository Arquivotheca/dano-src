...
public:
	...
	virtual status_t	Open(const BUrl &url, const BUrl&, BMessage* outErrorParams,
							uint32 flags);
	virtual status_t	OpenRW(const BUrl &url, const BUrl&, BMessage* outErrorParams,
	...
	virtual bool		GetRedirectURL(BUrl&, bigtime_t *out);
	virtual void		Abort();
...
protected:
	...
	status_t			OpenCommon(const BUrl &url, const BUrl&, BMessage* outErrorParams,
							uint32 flags, bool no_html = false, bool no_list = false);
	status_t			Connect(const BUrl &url, int &outcode);
...
private:
	...
	void				GenerateHTMLDirListing(BString &listing, const BUrl &base);

						// generate a non-HTML from an FTP LIST result and sticks it
						// into the fMallocIO member.
	void				GenerateDirListing(BString &listing, const BUrl &base);
