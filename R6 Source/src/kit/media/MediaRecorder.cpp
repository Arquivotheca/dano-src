// DesktopMediaRecorder.cpp

#include "MediaRecorder.h"
#include "MediaRecorderNode.h"
#include "SourceList.h"
#include <MediaRoster.h>
#include <TimeSource.h>
#include <MediaAddOn.h>


namespace BPrivate {
	class StNodeRelease {
		media_node & n;
		bool r;
	public:
		StNodeRelease(media_node & nod, bool rel = true) : n(nod), r(rel) { }
		~StNodeRelease() { if (r) BMediaRoster::Roster()->ReleaseNode(n); }
		void SetRelease(bool rel) { r = rel; }
	};
};
using namespace BPrivate;

BMediaRecorder::BMediaRecorder(
	const char *name,
	int32 priority) :
	_mNode(0),
	_mTimeSource(0),
	_mReleaseSourceNode(false),
	_mInitErr(0),
	_mBufferHook(0),
	_mBufferCookie(0),
	_mConnected(false),
	_mRunning(false),
	_mSources(0)
{
	(void)BMediaRoster::Roster(&_mInitErr);
	if (_mInitErr == B_OK) {
		if (priority < 5) priority = 115;
		_mNode = new BMediaRecorderNode(name, this, priority);
		_mInitErr = BMediaRoster::CurrentRoster()->RegisterNode(_mNode);
		if (_mInitErr == B_OK) {
			_mMyNode = _mNode->Node();
		}
	}
}

BMediaRecorder::~BMediaRecorder()
{
	if (_mNode != 0) {
		(void)Stop();
		(void)Disconnect();
		_mNode->Release();
	}
	if (_mTimeSource != 0) {
		_mTimeSource->Release();
	}
	if (_mSources != 0) {
		delete _mSources;
	}
}

status_t
BMediaRecorder::InitCheck()
{
	return _mInitErr;
}

status_t
BMediaRecorder::SetBufferHook(buffer_hook hook, void *cookie)
{
	if (_mInitErr < B_OK) {
		return _mInitErr;
	}
	if (_mRunning) {
		return EPERM;
	}
	_mBufferHook = hook;
	_mBufferCookie = cookie;
	return B_OK;
}

void
BMediaRecorder::SetCookie(void *cookie)
{
	_mBufferCookie = cookie;
}

void 
BMediaRecorder::BufferReceived(void *data, size_t size, const media_header &header)
{
	if (_mBufferHook && _mRunning) {
		(*_mBufferHook)(_mBufferCookie, data, size, header);
	}
}

status_t
BMediaRecorder::FetchSources(
	const media_format& format,
	bool physicalSourcesOnly) {

	if (_mInitErr < B_OK)
		return _mInitErr;
	if (_mConnected)
		return B_MEDIA_ALREADY_CONNECTED;
	if(!_mSources)
		_mSources = new BSourceList;
		
	return _mSources->Fetch(format, physicalSourcesOnly);
}
								
int32
BMediaRecorder::CountSources() const {

	if (_mInitErr < B_OK)
		return _mInitErr;
	if (_mConnected)
		return B_MEDIA_ALREADY_CONNECTED;
	if(!_mSources)
		return 0;
		
	return _mSources->CountItems();
}
	
status_t
BMediaRecorder::GetSourceAt( 
	int32 index,
	BString* outName,
	media_format* outFormat) const {

	if (_mInitErr < B_OK)
		return _mInitErr;
	if (_mConnected)
		return B_MEDIA_ALREADY_CONNECTED;
	if(!_mSources)
		return B_BAD_INDEX;
	
	return _mSources->GetItemAt(index, outName, outFormat);
}

status_t
BMediaRecorder::ConnectSourceAt( 
	int32 index, 
	uint32 flags) {

	if (_mInitErr < B_OK)
		return _mInitErr;
	if (_mConnected)
		return B_MEDIA_ALREADY_CONNECTED;
	if(!_mSources)
		return B_BAD_INDEX;

	source_list_item item;
	media_format format;	
	status_t err = _mSources->GetItemAt(index, 0, &format, &item);
	if(err < B_OK)
		return err;
	
	switch(item.type) {

		case B_SLIT_LIVE_NODE: {
			// fetch node
			media_node node;
			err = BMediaRoster::CurrentRoster()->GetNodeFor(item.live.node_id, &node);
			if(err < B_OK)
				return err;
			// describe source
			media_source source(node.port, item.live.source_id);
			// try to connect
			err = giga_connect(&format, flags, 0, &node, &source);
			if(err == B_OK)
				_mReleaseSourceNode = true;
			else
				BMediaRoster::CurrentRoster()->ReleaseNode(node);
			return err;
		}			

		case B_SLIT_DORMANT_NODE: {
			dormant_node_info info;
			info.addon = item.dormant.addon;
			info.flavor_id = item.dormant.flavor_id;
			return giga_connect(&format, flags, &info, 0, 0);
		}

		default:
			return B_ERROR; // oh-so-informative
	}
} 

status_t 
BMediaRecorder::Connect(const media_format &format, uint32 flags)
{
	if (_mInitErr < B_OK) {
		return _mInitErr;
	}
	if (_mConnected) {
		return B_MEDIA_ALREADY_CONNECTED;
	}
	return giga_connect(&format, flags, 0, 0, 0);
}

status_t 
BMediaRecorder::Connect(const dormant_node_info &info, const media_format *format, uint32 flags)
{
	if (_mInitErr < B_OK) {
		return _mInitErr;
	}
	if (_mConnected) {
		return B_MEDIA_ALREADY_CONNECTED;
	}
	return giga_connect(format, flags, &info, 0, 0);
}

status_t 
BMediaRecorder::Connect(const media_node &node, const media_source *use_source, const media_format *format, uint32 flags)
{
	if (_mInitErr < B_OK) {
		return _mInitErr;
	}
	if (_mConnected) {
		return B_MEDIA_ALREADY_CONNECTED;
	}
	return giga_connect(format, flags, 0, &node, use_source);
}

status_t 
BMediaRecorder::Disconnect()
{
	if (_mInitErr < B_OK) {
		return _mInitErr;
	}
	if (!_mConnected) {
		return B_MEDIA_NOT_CONNECTED;
	}
	if (!_mNode) {
		return B_ERROR;
	}

	//	do the disconnect
	status_t err = BMediaRoster::CurrentRoster()->Disconnect(
			_mSourceNode.node, _mSourceOutput.source, _mMyNode.node, _mMyNodeInput.destination);

	if (_mTimeSource != 0) {
		_mTimeSource->Release();
		_mTimeSource = 0;
	}

	_mConnected = false;
	_mRunning = false;

	return err;
}

status_t 
BMediaRecorder::Start(bool force)
{
	if (_mInitErr < B_OK) {
		return _mInitErr;
	}
	if (!_mConnected) {
		return B_MEDIA_NOT_CONNECTED;
	}
	if (_mRunning && !force) {
		return EALREADY;
	}
	if (!_mNode) {
		return B_ERROR;
	}
	//	start node here
	status_t err = B_OK;
	if (_mSourceNode.kind & B_TIME_SOURCE) {
		err = BMediaRoster::CurrentRoster()->StartTimeSource(_mSourceNode, BTimeSource::RealTime());
	}
	else {
		err = BMediaRoster::CurrentRoster()->StartNode(_mSourceNode, _mTimeSource->Now());
	}
	//	then un-mute it
	if (err == B_OK) {
		_mNode->SetDataEnabled(true);
	}
	_mRunning = (err == B_OK);
	return err;
}

status_t 
BMediaRecorder::Stop(bool force)
{
	if (_mInitErr < B_OK) {
		return _mInitErr;
	}
	if (!_mRunning && !force) {
		return EALREADY;
	}
	if (!_mNode) {
		return B_ERROR;
	}
	//	should have the Node mute the output here
	_mNode->SetDataEnabled(false);

	_mRunning = false;
	return B_OK;
}

bool
BMediaRecorder::IsRunning() const
{
	return _mRunning;
}

bool
BMediaRecorder::IsConnected() const
{
	return _mConnected;
}

const media_node &
BMediaRecorder::Node() const
{
	return _mMyNode;
}

const media_node &
BMediaRecorder::SourceNode() const
{
	return _mSourceNode;
}

const media_input &
BMediaRecorder::Input() const
{
	return _mMyNodeInput;
}

const media_output &
BMediaRecorder::SourceOutput() const
{
	return _mSourceOutput;
}

const media_format & 
BMediaRecorder::Format() const
{
	return _mSourceOutput.format;
}


status_t 
BMediaRecorder::giga_connect( const media_format *in_format, uint32 /*in_flags*/, const dormant_node_info *in_dormant, const media_node *in_node, const media_source *in_source)
{
	media_format fmt;
	media_node node;
	StNodeRelease away(node, false);
	status_t err = B_OK;
//	media_output out;
	media_source outSource;

	//	argument checking and set-up
	if (in_format != 0) fmt = *in_format;
	if (_mNode == 0) return B_ERROR;
	if ((in_node == 0) && (in_source != 0)) return B_MISMATCHED_VALUES;
	if ((in_dormant != 0) && ((in_node != 0) || (in_source != 0))) return B_MISMATCHED_VALUES;
//	if ((in_format == 0) && (in_output != 0)) fmt = in_output->format;

	_mNode->SetOKFormat(fmt);

	//	figure out the node
	//	instantiate?
	if (in_dormant != 0) {
		err = BMediaRoster::Roster()->InstantiateDormantNode(*in_dormant, &node, B_FLAVOR_IS_GLOBAL);
		away.SetRelease(true);
	}
	//	provided?
	else if (in_node != 0) {
		node = *in_node;
	}
	//	switch on format for default?
	else switch (fmt.type) {
	case B_MEDIA_RAW_AUDIO:
		err = BMediaRoster::Roster()->GetAudioInput(&node);
		away.SetRelease(true);
		break;
	case B_MEDIA_RAW_VIDEO:
	case B_MEDIA_ENCODED_VIDEO:
		err = BMediaRoster::Roster()->GetVideoInput(&node);
		away.SetRelease(true);
		break;
	//	give up?
	default:
		return B_MEDIA_BAD_FORMAT;
		break;
	}

	_mSourceNode = node;

	//	figure out the output
	//	provided?
	if (in_source != 0) {
		outSource = *in_source;
	}
	//	iterate?
	else if (err == B_OK) {
		media_output outputs[10];
		int32 count = 10;
		err = BMediaRoster::Roster()->GetFreeOutputsFor(node, outputs, count, &count, fmt.type);
		if (err == B_OK) {
			err = B_MEDIA_BAD_SOURCE;
			for (int ix=0; ix<count; ix++) {
				if (format_is_compatible(outputs[ix].format, fmt)) {
					outSource = outputs[ix].source;
					err = B_OK;
					fmt = outputs[ix].format;
					break;
				}
			}
		}
	}
	//	give up?
	if (err != B_OK) {
		return err;
	}
	if (outSource == media_source::null) {
		return B_MEDIA_BAD_SOURCE;
	}

	//	find our Node's free input
	media_input in;
	err = _mNode->GetInput(&in);

	media_node time_source;

	if (node.kind & B_TIME_SOURCE) {
		time_source = node;
	}
	else {
		BMediaRoster::Roster()->GetSystemTimeSource(&time_source);
	}

	//	set time source
	if (err == B_OK) {
		BMediaRoster::Roster()->SetTimeSourceFor(_mMyNode.node, time_source.node);
		_mTimeSource = BMediaRoster::CurrentRoster()->MakeTimeSourceFor(_mMyNode);
	}

	//	start the recorder node (it's always running)
	if (err == B_OK) {
		err = BMediaRoster::CurrentRoster()->StartNode(_mMyNode, _mTimeSource->Now());
	}

	//	perform the connection
	if (err == B_OK) {
		_mMyNodeInput = in;

		err = BMediaRoster::CurrentRoster()->Connect(
			outSource, _mMyNodeInput.destination, &fmt,
			&_mSourceOutput, &_mMyNodeInput, BMediaRoster::B_CONNECT_MUTED);
		if (err == B_OK) {
			_mConnected = true;
			away.SetRelease(false);
			if(in_dormant)
				_mReleaseSourceNode = true;
		}
	}
	return err;
}

status_t BMediaRecorder::_Reserved_MediaRecorder_0(void*, ...) { return B_ERROR; }
status_t BMediaRecorder::_Reserved_MediaRecorder_1(void*, ...) { return B_ERROR; }
status_t BMediaRecorder::_Reserved_MediaRecorder_2(void*, ...) { return B_ERROR; }
status_t BMediaRecorder::_Reserved_MediaRecorder_3(void*, ...) { return B_ERROR; }
status_t BMediaRecorder::_Reserved_MediaRecorder_4(void*, ...) { return B_ERROR; }
status_t BMediaRecorder::_Reserved_MediaRecorder_5(void*, ...) { return B_ERROR; }
status_t BMediaRecorder::_Reserved_MediaRecorder_6(void*, ...) { return B_ERROR; }
status_t BMediaRecorder::_Reserved_MediaRecorder_7(void*, ...) { return B_ERROR; }


// END -- MediaRecorder.cpp --
