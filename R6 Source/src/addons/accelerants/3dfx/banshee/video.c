...
// dprintf(("3dfx_accel: vid_selectmode - ENTER\n"));
	...
	{
		temp = _V3_ReadReg( ci, V3_2D_INTR_CTRL );
...
//      dprintf(("3dfx_accel: InitPlls - Setting Graphics Clock (%d MHz)\n",grxSpeedInMHz));
		_V3_WriteReg_NC( ci, V3_VID_PLL_CTRL_1, pllTable[grxSpeedInMHz]);
	}
...
//      dprintf(("3dfx_accel: InitPlls - Setting Memory Clock (%d MHz)\n",memSpeedInMHz));
		_V3_WriteReg_NC( ci, V3_VID_PLL_CTRL_2, pllTable[memSpeedInMHz]);
	}
}										  // h3InitPlls
...
void InitVga (uint32 legacyDecode) // 1=enable VGA decode, 0=disable-------------------------------------*/
{
	...
	check_for_room ();
	_V3_WriteReg_NC( ci, V3_VID_VGA_INIT_0, vgaInit0);

	check_for_room ();
	_V3_WriteReg_NC( ci, V3_VID_VGA_INIT_1, 0);

	check_for_room ();
	write_phys_8 (0xc3, 0x1);

	miscInit1 = _V3_ReadReg( ci, V3_VID_MISC_INIT_1 );
	miscInit1 |= BIT (0);
	check_for_room ();

	_V3_WriteReg_NC( ci, V3_VID_MISC_INIT_1, miscInit1);
...
uint32 GetMemSize ()
{
	...
	dramInit1_strap = _V3_ReadReg( ci, V3_VID_DRAM_INIT_1 );
	...
	dramInit0_strap = _V3_ReadReg( ci, V3_VID_DRAM_INIT_0 );
...
{
	uint32 reg_vidProcCfg = _V3_ReadReg( ci, V3_VID_PROC_CFG );
...
// dprintf(("3dfx_accel: SetVideoDesktopSurface - new vidProcCfg = 0x%x\n", vidProcCfg));
	_V3_WriteReg_NC( ci, V3_VID_PROC_CFG, reg_vidProcCfg);

	_V3_WriteReg_NC( ci, V3_VID_DESKTOP_START_ADDR, 
	...
	//
	reg_vidDesktopOverlayStride = _V3_ReadReg( ci, V3_VID_DESKTOP_OVERLAY_STRIDE );
...
//    dprintf(("3dfx_accel: SetVideoDesktopSurface - new stride = 0x%x\n", doStride));
	_V3_WriteReg_NC( ci, V3_VID_DESKTOP_OVERLAY_STRIDE, reg_vidDesktopOverlayStride );
}
...
// dprintf(("3dfx_accel: SetupCRTC - Disabling VGA Decode\n"));   
	...
	temp = _V3_ReadReg( ci, V3_VID_PROC_CFG );
	temp |= 0x01;  // Enable Video Processor
	_V3_WriteReg_NC( ci, V3_VID_PROC_CFG, temp );
...
// dprintf(("3dfx_v4_accel: SetupCRTC - called CalcPLL(freq = %d KHz), fout = %d KHz, PLLReg = 0x%x\n", dm->timing.pixel_clock, fout, PLLReg));
	_V3_WriteReg_NC( ci, V3_VID_PLL_CTRL_0, PLLReg);
...
dprintf(( "setting gamma \n" ));
		_V3_SetGamma( gamma, gamma, gamma );
	}
...
status_t movedisplay (uint16 x, uint16 y)
{
	...
	stride = ci->BytesPerRow;
	fb_offset += y * stride;
	_V3_WriteReg_NC( ci, V3_VID_DESKTOP_START_ADDR, (fb_offset & SST_VIDEO_START_ADDR) << SST_VIDEO_START_ADDR_SHIFT);
	return B_OK;
}
