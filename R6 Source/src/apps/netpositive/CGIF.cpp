...
bool CGIF::GetRect(BRect *r)
{
	r->Set(0,0,mWidth,mHeight);
	return (mWidth && mHeight);
}

void CGIF::SetBG(bool hasBGColor, long bgColor, bool hasBGImage, Pixels *bgImage, float bgHOffset, float bgVOffset)
{
...
long	CGIF::Write(uchar *data, long dataLength, Pixels *pixels)
{
	switch (mPhase) {
...
//		case kLocalImageBodySetup:	return SetupImageBody(data,dataLength,pixels);
		...
		case kLocalImageBody:		return ReadImageBody(data,dataLength,pixels);
...
long CGIF::ReadGlobalHeader(uchar *data, long dataLength)
{
...
long CGIF::ReadComment(uchar *data, long dataLength)
{
	short 	count;
	uchar *src = data + 2;
	dataLength -= 2;
...
long CGIF::ReadGraphicControl(uchar *data, long dataLength)
{
...
long CGIF::ReadApplication(uchar *data, long dataLength)
{
	uchar *src = data + 2;
	short count;
...
long CGIF::ReadExtension(uchar *data, long dataLength)
{
	short	count;
...
long CGIF::ReadImageHeader(uchar *data, long dataLength)
{
...
//	Global with and height was not spec, set it now
	...
	if (mWidth == 0 || mLocalWidth > mWidth || mFirstFrameOnly)
		mWidth = mLocalWidth;
	if (mHeight == 0 || mLocalHeight > mHeight || mFirstFrameOnly)
...
long CGIF::SetupImageBody(uchar *data, long dataLength, Pixels *pixels)
{
	uchar	*src = data;
...
int CGIF::FillBuffer(uchar *src, int dataLength)
{
	int added = 0;
...
long CGIF::ReadImageBody(uchar *data, long dataLength, Pixels *pixels)
{
	short	p = 0;
	uchar	*src = data;
...
//			pprint("Terminated at line %d",mYPos);
		if (gPreferences.FindBool("ShowAnimations")  && !mFirstFrameOnly)
