...
#ifndef _STORAGE2_MIME_H
#define _STORAGE2_MIME_H
...
//		status_t	GetIcon(BBitmap *icon, icon_size) const;
		...
		status_t	GetAttrInfo(BValueMap *info) const;
		status_t	GetFileExtensions(BValueMap *extensions) const;
		status_t	GetShortDescription(char *description) const;
		status_t	GetLongDescription(char *description) const;
		status_t	GetSupportingApps(BValueMap *signatures) const;

//		status_t	SetIcon(const BBitmap *icon, icon_size);
		status_t	SetPreferredApp(const char *signature,
									app_verb verb = B_OPEN);
		status_t	SetAttrInfo(const BValueMap *info);
		status_t	SetFileExtensions(const BValueMap *extensions);
		status_t	SetShortDescription(const char *description);
		status_t	SetLongDescription(const char *description);

static	status_t	GetInstalledSupertypes(BValueMap *super_types);
static	status_t	GetInstalledTypes(BValueMap *types);
static	status_t	GetInstalledTypes(const char *super_type,
									BValueMap *subtypes);
static	status_t	GetWildcardApps(BValueMap *wild_ones);
...
//static  void		_set_local_dispatch_target_(BMessenger *, void (*)(BValueMap *));
		void        _touch_(); 
...
virtual	void		_ReservedMimeType3();
		...
		status_t	GetSupportedTypes(BValueMap *types);
		status_t	SetSupportedTypes(const BValueMap *types);
