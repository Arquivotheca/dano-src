...
/*
 * From 'value', extract value from bitfield 'from' and set format it for
 * the bitfield 'to'.  Use of mismatched bitfield sizes is recommended only
 * for the Talented.
 */
...
#define	FIELDDEF(reg,field,def)		SetBF (reg##_##field, \
					       reg##_##field##_##def)
#define	FIELDVAL(reg,field,val)		SetBF (reg##_##field, val)
#define	FIELDMASK(reg,field)		MASKEXPAND (reg##_##field)
