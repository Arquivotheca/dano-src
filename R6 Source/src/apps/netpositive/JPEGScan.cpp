...
static short	
GetCoeff(JPEGDecoder *j,HuffmanTable *h, ushort& run)
{
	uInt16	code;
	short	l,s,i;
	FastRec *f;
...
//	Do the slow case, rare: < 5%, bad huffman codes show up here
	...
	if (l > 16) {
		pprint("Bad Huffman code in GetCoeff");
		j->error = kBadHuffmanErr;
		return 0;
	}
	i = h->valptr[l] + (code - h->mincode[l]);
	s = h->huffval[i];
	//pprint("Code: %4X (%2d) Symbol: %2X (0x%8X:0x%8X)",code,l+1,s,j->last32,j->next32);
	run = s >> 4;				// Run
...
HuffmanDecode(JPEGDecoder *j,register HuffmanTable *h,ushort &run)
{
	...
	else 
	{
		...
		{		// bad huffman code
			run = 64;
			v = -1;
			pprint("Invalid Huffman code");
			goto done;
		}
		if (IsWarning(code < h->mincode[l]) || IsWarning(code > h->maxcode[l]) )
		{
			j->error = kBadHuffmanErr;
			return -1;
		}
				...
				else 
				{
					for (k = cspec->specStart; k <= cspec->specEnd; k++) 
					{
						...
						if (v) {
							...
							k += run;
							if ( IsWarning(k > cspec->specEnd) ) 
							{
								pprint("Coeff past end of band");
								j->error = kBadHuffmanErr;
								return -1;
							}
							...
							if ( run == 15 ) {
								...
								if ( IsWarning(k > cspec->specEnd) ) 
								{
									pprint("Coeff run past end of band");
									j->error = kBadHuffmanErr;
									return -1;
								}
							}
				...
				if ( j->bandSkip == 0 ) 
				{
					for (k = cspec->specStart; k <= cspec->specEnd; k++) 
					{
						...
						else 
						{
							...
							if ( IsWarning(l > 16) ) 
							{
								pprint("Bad AC Successive Approximation Huffman code");
								j->error = kBadHuffmanErr;
								return -1;
							}
							if (IsWarning(code < h->mincode[l]) || IsWarning(code > h->maxcode[l]) )
							...
							if ( s != 0  ) 	
							{
								...
								{
									j->error = kBadHuffmanErr;
									pprint("Bad AC Successive Approximation Correction Huffman code");
									return -1;
								}
								bitcount--;
								if ( (bitbuffer >> bitcount) & bmask(1) )
						...
						if ( IsWarning(run > (cspec->specEnd-k)+1) ) 
						{
							j->error = kBadHuffmanErr;
							pprint("Bad AC Successive Approximation run length code %d > %d-%d+1",run,cspec->specEnd,k);
							return -1;
						}
...
static void BeginInterval(JPEGDecoder* j)
{
	...
	JPEGMessage(("Interval reset %d",j->Interval));
...
//	Start a new interval if there was a restart marker or start a new scan
	...
	if (j->RstMarker) 
	{
		...
		{
			SetupScan(j);
			JPEGMessage(( j->isProgressive ?  "Start new progressive scan to %X": "Start new scan to %X",marker));
		}
#ifdef	CHK_MIN_DATA
	...
	{
		JPEGMessage(("skip entire scan current %d last %d",j->currentSlice,j->lastSlice));
		skipScan = true;
	}
	...
	if ( !j->isProgressive ) 
	{
		...
		if ( (restartMarker & 0xf8) == 0xd0 && (j->MCUCount + j->Interval) < j->firstMCU ) 
		{
			...
			JPEGMessage(("skip entire interval marker %X next %X count %d, interval %d first %d",restartMarker,j->RstMarker,j->MCUCount,j->Interval,j->firstMCU));
			j->data = j->dataEnd;
			j->bits = 0;
...
//			j->skipToFirstSlice = false;
	...
	if ( skipScan ) 
	{
		JPEGMessage(("Skipping to end of scan"));
		ResetScan(j);
...
//	or until we run out of data
	...
	JPEGMessage(("Start scan from %d %d scanline mdu %d, interval %d",xMCU,yMCU,j->MCUCount,j->Interval));
	JPEGMessage(("Scan %d components firstslice %d currentslice %d lastSlice %d x=%d,y=%d",j->CompInScan,j->firstSlice,j->currentSlice,j->lastSlice,xMCU,yMCU));
#if 0
	for ( i = 0; i < j->CompInFrame; i++ ) 
	{
		...
		if (cspec->inScan) {
			switch (i) {
				case 0:	pprint("Y In Scan");	break;
				case 1:	pprint("U In Scan");	break;
				case 2:	pprint("V In Scan");	break;
			}
		}
	}
#endif
	...
	if ( scanIsNonInterleaved ) 
	{
		...
		JPEGMessage(("non-interleaved scan at mcu %d %d ( mcus/slice= %d)",j->MCUCount,MCUsTillSlice,MCUsPerSlice));
	}
	...
	for ( i=0; i < j->CompInFrame; i++ ) 
	{
		...
		if (useBlockBuffer) {
			if ( IsWarning(cspec->blockBufferData == NULL) ) {
	...
	{			// Decode the next MCU
		...
		if (!marker)  
		{
			...
			{	
				JPEGMessage(("ran dry at %d (%d,%d)",j->MCUCount,xMCU,yMCU));
				j->Interval++;
				break;
			}
		}
		...
		{					// Died or was cancelled
			pprint("Scan error at mcu %d %d,%d with %d bytes left",j->MCUCount,xMCU,yMCU,j->dataEnd-j->data);	
			goto done;		
		}
		...
		if ( scanIsNonInterleaved ) 
		{
			...
			if ( ++MCUsTillSlice == MCUsPerSlice || j->MCUCount == (j->ScanMCUs-1) ) 
		...
		if ( completedSlice ) 
		{
			//JPEGMessage(("Did slice %d draw %d",j->currentSlice,j->didDraw));

			if ( j->didDraw ) 
			{
				JPEGMessage(("draw slice %d (y = %d)",j->currentSlice,j->currentSlice * j->MCUVPixels));
			...
			if ( scanCompleted ) 
			{
				JPEGMessage(("MMMX ending slice at %d (%d)",j->currentSlice,j->MCUCount));
		...
		{
			JPEGMessage(("End of JPEG data reached"));
			EndJPEGImage(j);
			break;
		}
	}
...
{	
	...
	JPEGMessage(("skipping scan data %d bytes",dataLength));
	...
	while ( data < dataEnd ) 
	{
		if ( *data++ == 0xff ) 
		{
			...
			if ( data == dataEnd ) 
			{	
				JPEGMessage(("split marker during skip"));
				return dataLength-1;		
			}
			...
			{		// if it's real
				if ( marker == kEOI ) 
				{
					JPEGMessage(("end of image reached skipping scan"));
					j->phase =  kSkippingToEnd;	
				...
				{	
					...
					if ( (marker & 0xf8) != 0xd0 ) 
					{
						JPEGMessage(("found marker skipping %X %d bytes used",marker,(data-2) - dataStart));
						switch ( marker ) 
						{
						case kDQT:
						case kDHT:
						...
						case kCOM:
							break;
						default:
							JPEGMessage(("Invalid marker found in scan"));
							return -1;
						}
...
//	Write data until our buffer is full, they ran out of data or we hit a marker
	...
	JPEGMessage(("process scan %d bytes",count));
	while (count-- > 0) {
		if ( (*dst++ = *data++) == 0xFF ) 
		{
			...
			if (marker != 0) 
			{
				JPEGMessage(("marker in scan %X",marker));
				if ( (marker & 0xF8) == kRST ) 
				{
				...
				else 
				{
					...
					{
						j->phase = kSkippingToEnd;
						JPEGMessage(("end of image in scan"));
						dst--;
						break;
					} 
					else 
	...
	{
		JPEGMessage(("waiting for marker %X",j->SkipToRestartMarker));
		if ( marker == j->SkipToRestartMarker )
			j->SkipToRestartMarker = 0;
		return data-dataStart;
	}
	
	//JPEGMessage(("Processed %d bytes from %d - marker %X, total avail = %d",dst - (j->WriteBuffer + j->inBuffer),data-dataStart,marker,dst-j->WriteBuffer));
	...
	else 
	{
		...
		{
			pprint("JPEG DecodeScan: Error %d", (int)error);
			ResetJPEGDecoder(j);
			return error;				// Error
		}
		
		//JPEGMessage(("Decoded %d bytes of %d",j->data-j->WriteBuffer,j->dataEnd-j->WriteBuffer));
		...
		if ( j->phase == kSkippingScan )
		{
			...
			else 
			{
				...
				{
					JPEGMessage(("JPEG skipping rest of scan %d bytes used %d left",used,dataLength-used));
					skipped = SkipScan(j,data,dataLength - used);
					JPEGMessage(("skipped %d bytes",skipped));
					if ( skipped < 0 )
						return skipped;
				}
				return used + skipped;
	...
	//	Copy Unused data to buffer head
		...
		else
		{
			...
			JPEGMessage(("%d kept in buffer",j->inBuffer));
...
//				CopyMemory(j->data,j->WriteBuffer,j->inBuffer);
		...
		if (data < dataStart)
			JPEGMessage(("Underflow In ScanWrite: %d",data - dataStart));
	}
...
static long NextMarker(const uchar *data,long dataLength,uchar *rMarker,ushort *rSize)
{
	...
	*rSize = 0;
	if ( dataLength <= 2 ) 
	{
		//JPEGMessage(("no room for marker"));
		return 0;
	}
	while ( used < dataLength ) 
	{
		...
		} else 
		{
			...
			else 
			{
				...
				{
					//JPEGMessage(("no room for marker header %X",marker));
...
long JPEGWrite(JPEGDecoder* j, const uchar* data, long dataLength, BRect *)
{
	...
	{
		JPEGMessage(("End of image at first"));
		ResetJPEGDecoder(j);
		j->phase = kWaitingForNewImage;
	}
...
newPhase:
	if ( j->phase == kSkippingToEnd ) 
	{
		JPEGMessage(("End of image reached"));
		goto completed;
	}
	...
	{
		JPEGMessage(("only %d bytes left, returning with %d",dataLength,bytesConsumed));
		if ( IsWarning(bytesConsumed < 0) )
	...
	{
	case kSkippingScan:
//		JPEGMessage(("kSkippingScan %d",dataLength));
		l = SkipScan(j,data,dataLength);
		if ( l < 0 ) 
		{
			JPEGMessage(("SkipScan error %ld",l));
			ResetJPEGDecoder(j);
			result = l;
			goto done;
		}
...
//		JPEGMessage(("process scan %x %d",data,dataLength));
		...
		if ( l < 0 ) 
		{
			JPEGMessage(("ScanWrite error %ld",l));
			ResetJPEGDecoder(j);
			result = l;
			goto done;
		}
...
completed:
//		JPEGMessage(("kSkippingToEnd %d",dataLength));
		...
		if ( j->error == 0 ) 
		{
			...
			{					// requires another display pass on full stream
				JPEGMessage(("end of image - need another pass"));
				j->phase = kWaitingForNewImage;
	...
	case kWaitingForNewImage:
		...
		JPEGMessage(("kWaitingForNewImage %d",dataLength));
		l = NextMarker(data,dataLength,&marker,&s);
		markerOffset = l;
		bytesConsumed += l;
		if ( IsWarning(l > dataLength) ) {
			result = kGenericError;
			goto done;
		}
		if ( marker == 0 ) {
	...
	case kWaitingForScan:
		...
		JPEGMessage(("kWaitingForScan %d",dataLength));
		...
		JPEGMessage(("process marker %X size %d",marker,s));
	
		if ( IsWarning(marker < kSOF0 || s <= 0) ) {
			result = kGenericError;
			goto done;
		}
		...
		if ( s > (dataLength-2) ) 
		{
			...
			JPEGMessage(("not enough data to process marker need %d",s));
			j->unusedSkip = s;
...
//			case kEOI:		EndJPEGImage(j); return kJPEGCompletedPass;
			...
			case kSOF2:  
					...
					if ( error == kNoError ) 
					{
						JPEGMessage(("Progressive JPEG Image"));
					}
			...
			case kCOM:	  
					if ( dataLength < (s-2) ) 
					{			// marker doesnt fit	
						JPEGMessage(("JPEG Ignoring big marker %X",marker));
					//	j->unusedSkip = s-2;
						result = l;
						goto done;
					}
			...
			default:	
					...
					{
						JPEGMessage(("Unexpected JPEG marker %X",marker));
						error = kBadMarkerErr;
					}
					break;
		}
		if (error != kNoError) 
		{
			JPEGMessage(("Marker decode error %d",error));
			ResetJPEGDecoder(j);
			result = error;
			goto done;
		}
