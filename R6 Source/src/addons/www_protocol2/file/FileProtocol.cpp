...
class FileProtocol : public Protocol
{
	public:
		...
		virtual status_t Open(const BUrl &url, const BUrl&, BMessage *errorParams,
		        uint32 flags);
	...
	private:
		void ExpandPathVariables(BStringBuffer &out, BStringBuffer &in);
...
status_t FileProtocol::Open(const BUrl &url, const BUrl&, BMessage* outErrorParams,
	uint32 flags)
{
	// get the URL string
	BStringBuffer urlString;
	url.AppendTo(urlString);

	// expand the URL string
	BStringBuffer expanded;
	ExpandPathVariables(expanded, urlString);

	BUrl newURL(expanded.String());
	...
	if (fFD < 0) {
		outErrorParams->Data().Overlay(BValue::String(S_ERROR_TEMPLATE), BValue::String("Errors/file.html"));
		outErrorParams->Data().Overlay(BValue::String("filename"), BValue::String(path));
		outErrorParams->Data().Overlay(BValue::String(S_ERROR_MESSAGE), BValue::String(strerror(fFD)));
		return fFD;
	}
...
void FileProtocol::ExpandPathVariables(BStringBuffer &out, BStringBuffer &inBuf)
{
...
public:
	virtual void GetIdentifiers(BMessage* into)
	{
		...
		into->Data().Overlay(S_PROTOCOL_SCHEMES, BValue::String("file"));
	}
