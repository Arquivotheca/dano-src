...
BIntegerList::BIntegerList(int32 initialAllocSize)
	:_list(initialAllocSize)
{
...
{
	return (int32) _list.RemoveItem((int32) index);
}
...
static void
count_total_items_recursive(BElementDecl::ListNode * node, int32 & runningSum)
{
	if (!node)
		exit(1);
	if (node->type == BElementDecl::ListNode::STRING)
...
static int32
count_total_items(BElementDecl * decl)
{
	int32 runningSum = 0;
	BElementDecl::ListNode * node = decl->GetPattern();
...
*/
static int32
count_list_items(BElementDecl::ListNode * node)
{
	int32 runningSum = 0;
...
static bool
get_next_items_recursive(BElementDecl::ListNode * node, int32 & currentIndex, int32 index, TransitionInfo & transition)
{
	if (!node)
...
static void
get_next_items(BElementDecl * decl, int32 index, TransitionInfo & transition)
{
	int32 currentIndex = 0;
	BElementDecl::ListNode * node = decl->GetPattern();
...
void
_init_validate_table_(BElementDecl * decl, const void ** tablePtr)
{
