...
MWarningsView::UpdateCheckBoxes()
{
	...
	fNewSettings.pImplicitConversion = fImplicitConversion->Value() == 1;
	fNewSettings.pNotInlined = fNotInlined->Value() == 1;
	...
	BMessage&	inOutMessage)
{
	...
	inOutMessage.AddData(kWarningsPrefs, kMWCCPlugType, &fNewSettings, sizeof(fNewSettings));
	...
	BMessage&	inMessage)
{
	...
	if (B_NO_ERROR == inMessage.FindData(kWarningsPrefs, kMWCCPlugType, (const void**) &prefs, &length))
	{
...
MWarningsView::UpdateValues()
{
	...
	fImplicitConversion->SetValue(fNewSettings.pImplicitConversion);
	fNotInlined->SetValue(fNewSettings.pNotInlined);
}
...
MWarningsView::AttachedToWindow()
{
	...
	r.Set(30, top, 300, top + 15.0);
	msg = new BMessage(msgCheckBoxChanged);
	checkBox = new BCheckBox(r, "extra", "Implicit Arithmetic Conversions", msg); 
	box->AddChild(checkBox);
	checkBox->SetTarget(this);
	SetGrey(checkBox, kLtGray);
	fImplicitConversion = checkBox;
	top += 20.0;

	// Not inlined
	r.Set(30, top, 300, top + 15.0);
	msg = new BMessage(msgCheckBoxChanged);
	checkBox = new BCheckBox(r, "extra", "'inline' Functions not Inlined", msg); 
	box->AddChild(checkBox);
	checkBox->SetTarget(this);
	SetGrey(checkBox, kLtGray);
	fNotInlined = checkBox;
	top += 20.0;
