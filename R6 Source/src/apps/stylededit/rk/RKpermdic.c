...
{				/* ARGSUSED */
	...
	if (dm->dm_class == ND_SWD) {
		...
		if (tgram) {
			...
			if (dm->dm_gram) {
				dm->dm_gram->gramdic = tgram;
				dm->dm_gram->P_BB = RkGetGramNum(tgram, "BB");
				dm->dm_gram->P_NN = RkGetGramNum(tgram, "NN");
				dm->dm_gram->P_T00 = RkGetGramNum(tgram, "T00");
				dm->dm_gram->P_T30 = RkGetGramNum(tgram, "T30");
				dm->dm_gram->P_T35 = RkGetGramNum(tgram, "T35");
...
static int
readThisCache(struct DM * dm, struct ND * xdm, int pgno, unsigned val, Wchar * key, int cur, int ylen, struct nread * nread, int mc, int nc, int *cf)
{
	int                 remlen;
...
static int
SearchInPage(struct DM * dm, struct ND * xdm, int pgno, unsigned char *buf, unsigned val, Wchar * key, int cur, int ylen, struct nread * nread, int mc, int nc, int *cf)
{
	Wchar               kv, wc;
...
int
_Rkpsearch(RkContext * cx, struct DM * dm, Wchar * key, int n, struct nread * nread, int mc, int *cf)
/* ARGSUSED */
{
	struct ND          *xdm;
...
int
_Rkpsync(RkContext * cx, struct DM * dm, struct DM * qm)
{
	struct DF          *df;
	struct DD          *dd;
	char               *file;
