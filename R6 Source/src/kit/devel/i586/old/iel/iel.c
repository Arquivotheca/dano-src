...
/* The FUNC - definition is used for an automatic generation of the
   functions headers */
...
FUNC char IEL_mul(U128 *xr, U128 *y, U128 *z)
{
	MUL128 	y1, z1;
	U128   	temp[16], x;
	char   	ovfl=0;
	...
	#ifndef BIG_ENDIAN
		for (ui=8; ((!y1.w[ui-1]) && (ui>0)); ui--);
		for (uj=8; ((!z1.w[uj-1]) && (uj>0)); uj--);
		if ((ui+uj)<8)
		{
			tmin = ui+uj;
			tmax = 0;
...
FUNC char IEL_rem(U128 *x, U128 *y, U128 *z)
{
	U128 	x1, y1, z1, t1, z2;
	long 	carry, i,j;
...
FUNC char IEL_div(U128 *x, U128 *y, U128 *z)
{
	U128 	x1, y1, z1, t1, z2;
	long 	carry, i, j;
...
FUNC IEL_Err IEL_U128tostr(const U128 *x, char *strptr, long base, const long length)
{
	...
	if ((length) < (long)strlen (tempstr)+1)
	{
		return (IEL_OVFL);
	} else
	{
...
FUNC IEL_Err IEL_U64tostr(const U64 *x, char *strptr, long base, const long length)
{
	U128 y;    
...
FUNC IEL_Err IEL_S128tostr(const S128 *x, char *strptr, long base,const long length)
{
	U128 y;
	IEL_ASSIGNU(y, (*x));
	return(IEL_U128tostr( &y, strptr, base, length));
...
FUNC IEL_Err IEL_S64tostr(const S64 *x, char *strptr, long base,const long length)
{
	U128 y;
...
FUNC int IEL_strtoU128( char *str1, char **endptr, long base, U128 *x)
{
/*
  	U128 	cur_digit, base_mul;
  	long	k, toi, size;
  	char	c, *c1;
  	char    tempstr[1025]; */
	U128	sum;
	unsigned long	i, j=0;
	int     ovfl = IEL_FALSE, negative=IEL_FALSE;
...
FUNC int IEL_strtoU64(char *str1, char **endptr, long base, U64 *x)
{
	U128	y1;
	int     ovfl;

	ovfl = IEL_strtoU128(str1, endptr, base, &y1);
...
FUNC int IEL_strtoS128(char *str1, char **endptr, long base, S128 *x)
{
	U128	y1;
	int ovfl = 0;
...
FUNC int IEL_strtoS64(char *str1, char **endptr, long base, S64 *x)
{
	U128	y1;
	int ovfl;
...
FUNC int IEL_au(void *x, void *y, int sx, int sy)
{
        U128 tmp, zero;
...
FUNC IEL_Err IEL_as(void *x, void *y, int sx, int sy)
{
	S32		s32y;
	S64		s64y;
	S128	s128y;
