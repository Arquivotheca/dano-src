...
#define disable_color(_c_)	shift_color(_c_, DISABLED_C)
...
// created for each choice.  ChoiceListView just pulls the choice strings directly
// from the BComboBox and draws them.
...
// TextInput is a somewhat modified version of the _BTextInput_ class defined in
// TextControl.cpp.
...
// In BeOS R4.5, SetEventMask(B_POINTER_EVENTS, ...) does not work for getting all
// mouse events as they happen.  Specifically, when the user clicks on the window
// dressing (the borders or the title tab) no mouse event will be delivered until
// after the user releases the mouse button.  This has the unfortunate side effect
// of allowing the user to move the window that contains the BComboBox around with
// no notification being sent to the BComboBox.  We need to intercept the B_WINDOW_MOVED
// messages so that we can hide the popup window when the window moves.
...
BComboBox::ChoiceListView::ChoiceListView(BRect frame, BComboBox *parent)
	: BView(frame, "_choice_list_view_", B_FOLLOW_ALL_SIDES, B_WILL_DRAW | B_NAVIGABLE),
	  fClickLoc(-100, -100)
{
	fParent = parent;
...
void BComboBox::ChoiceListView::Draw(BRect update)
{
	...
	for (index = (int32)floor(update.top / h); index < choices; index++) {
		...
		DrawString(	fParent->fChoiceList->ChoiceAt(index),
					BPoint(rect.left + 2, rect.bottom - fFontHeight.descent - 1));
	}
...
void BComboBox::ChoiceListView::MouseDown(BPoint where)
{
	...
	if (!rect.Contains(where)) {
		...
		// HACK: the window is locked and unlocked so that it will get activated
		// before we potentially send the mouse down event in the code below.  Is
		// there a way to wait until the window is activated before sending the mouse
		// down? Should we call fParent->Window()->MakeActive(true) here?
	...
	// XXX: this probably isn't necessary since we are doing a SetEventMask whenever
	// the popup window becomes visible which routes all mouse events to this view
...
void BComboBox::ChoiceListView::MouseMoved(	BPoint where,
											uint32 transit,
...
void BComboBox::ChoiceListView::SetFont(const BFont *font,
										uint32 properties)
{
...
BComboBox::TextInput::TextInput(BRect rect, BRect text_r, ulong rMask, ulong flags)
...
{
	// XXX: is "TextInput" the correct name for this class?  Perhaps BComboBox::TextInput?
...
//+			PRINT(("a:ptr = %s\n", ptr));
	...
	if (parent) {
		...
		BMessage *msg;
		parent->Window()->PostMessage(msg = new BMessage(kTextInputModifyMessage), parent);
		delete msg;
	}
...
{
	if (fPopupWindow && fPopupWindow->Lock()) {
		fPopupWindow->MoveBy(new_position.x - fFrameCache.left, new_position.y - fFrameCache.top); 
		fPopupWindow->Unlock();
	}
...
void BComboBox::Draw(BRect updateRect)
{
	...
	BRect rect(fText->Frame() | fButtonRect);
	rgb_color oldCol = HighColor();
	
	/*// draw border around text input area
	rect.InsetBy(-1, -1);
//	SetHighColor(0, 0, 0); // black
	StrokeRect(rect, B_SOLID_HIGH);
	
	// draw focus indicator or erase it
	rect.InsetBy(-2, -2);
	SetHighColor(fText->IsFocus() ? keyboard_navigation_color() : LowColor());
	StrokeRect(rect, B_SOLID_HIGH);
	
	
	// draw label
	if (Label() && fDivider > 0) {
		SetHighColor(oldCol); // black
		// XXX: should properly position string based on alignment
		font_height fh;
		GetFontHeight(&fh);
		DrawString(Label(), BPoint(0, rect.bottom - 3 - ceil(fh.descent)));
	}
	*/
...
void BComboBox::MessageReceived(BMessage *msg)
{
	...
	case kPopupButtonInvokeMessage:
		if( fChoiceList && fChoiceList->CountChoices() )
		if (!fPopupWindow) {
...
void BComboBox::AttachedToWindow()
{
	...
	if (!enabled)
		base = shift_color(base, DISABLED_FILL_C);
	else
...
void BComboBox::SetFlags(uint32 flags)
{
	if (!fSkipSetFlags) {
		...
		ASSERT((Flags() & B_NAVIGABLE) == 0); // the ComboBox should never be navigable
...
// fPopupWindow must exist and already be locked & hidden when this function is called
...
// fPopupWindow must exist and already be locked & shown when this function is called
...
{
	BString *str = fList->ItemAt(index);
	if (str) {
		return str->String();
	} else {
		return NULL;
...
status_t BDefaultChoiceList::GetMatch(	const char *prefix,
										int32 startIndex,
										int32 *matchIndex,
...
{
	BString *str = fList->RemoveItemAt(index);
	if (str) {
		delete str;
		if (fOwner) {
			fOwner->ChoiceListUpdated();
