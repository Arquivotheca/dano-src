...
#endif
	/** this version of configure is new in version 2 of the joystick module API */
	int (*configure_alt)(void * back_cookie, uchar (*read_func)(void *), void (*write_func)(void *, uchar),
			joystick_module_info * info, size_t size, void ** out_cookie);
	/** this version of read is new in version 2 of the joystick module API */
	int (*read_alt)(void * in_cookie, void * back_cookie, uchar (*read_func)(void *),
			void (*write_func)(void *, uchar), extended_joystick * data, size_t size);
	/** this version of crumble is new in version 2 of the joystick module API */
	int (*crumble_alt)(void * in_cookie, void * back_cookie, uchar (*read_func)(void *),
			void (*write_func)(void *, uchar));
	int _is_zero_;
	/** new stuff goes here (_is_zero_ used to be force, must be zero) */
	int _reserved_;
} joystick_module;
...
typedef struct _generic_gameport_module {
	...
	//	WARNING: The hooks passed to create_device_alt are called with interrupts
	//	disabled. This means there's a very restricted set of things you can do
	//	in those hook functions.
	//	create_device_alt is new in version 2 of the module API
	status_t (*create_device_alt)(void * drvr_cookie, uchar (*read_port_func)(void *),
			void (*write_port_func)(void *, uchar), void ** out_storage);
	int	_reserved_;
} generic_gameport_module;

#define B_GAMEPORT_MODULE_NAME_V1 "generic/gameport/v1"
#define B_GAMEPORT_MODULE_NAME "generic/gameport/v2"
