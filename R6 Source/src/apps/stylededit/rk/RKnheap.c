 ...
 */
static void
_RkHset(unsigned char *s, unsigned char *e)
{
	struct RkParam     *sx = &SNX;
	int                 sp, ep;
	int                 sb, eb;
	unsigned char       sm, em;

	sp = (s - sx->heap) / HEAPUNIT;
	ep = (e - sx->heap - 1) / HEAPUNIT;
	sb = sp / 8;
	eb = ep / 8;
	sm = smask[sp & 7];
	em = emask[ep & 7];
 ...
 */
static void
_RkHclr(unsigned char *s, unsigned char *e)
{
	struct RkParam     *sx = &SNX;
	int                 sp, ep;
	int                 sb, eb;
...
static
int
_RkHtst(unsigned char *s, unsigned char *e)
{
	struct RkParam     *sx = &SNX;
 ...
 */
static void
_Rkenque(struct block * h, struct block * e)
{
	e->bk_next = h;
	e->bk_prev = h->bk_prev;
	h->bk_prev->bk_next = e;
	...
	/* block ga shiyouchuu naraba yameru */
		...
		for (b = Hpool[lg].bk_next; b != &Hpool[lg]; b = b->bk_next) {
			if (((unsigned char *)b + pw) == p) {	/* b:front half */
				...
				p = (unsigned char *)b;
				goto next;
			} else if ((unsigned char *)b == (p + pw)) {	/* b: rear half */
				_Rkdeque(b);
				goto next;
			};
		};
