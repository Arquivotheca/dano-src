							   ...
							   int32 index, int32 count)
{
	...
	switch (pos->header.subcode) {
		case SINGLE_DATA_SUBCODE: {
			if (pos->type == B_MESSAGE_TYPE) {
				if (!(mode&AM_NO_RECURSION)) {
					HandleAtoms(tag, (const uint8*)(pos->data_struct()),
								pos->entry_size, mode);
				}
			} else if (pos->entry_size == 4 && index == 0) {
		...
		case FIXED_ARRAY_SUBCODE: {
			...
			for (int32 i=index; i<(int32)data->count && count > 0; i++, count--) {
				if (pos->type == B_MESSAGE_TYPE) {
					if (!(mode&AM_NO_RECURSION)) {
						HandleAtoms(tag, data->data(pos->entry_size, i),
									pos->entry_size, mode);
					}
				} else if (pos->entry_size == 4) {
		...
		case VARIABLE_ARRAY_SUBCODE: {
			...
			for (int32 i=index; i<(int32)data->count && count > 0; i++, count--) {
				...
				if (pos->type == B_MESSAGE_TYPE) {
					if (!(mode&AM_NO_RECURSION)) {
						HandleAtoms(tag, bytes, size, mode);
					}
				} else if (size == 4) {
	...
	if (size < B_OK) {
		...
		if (size < 0) {
			...
			if (size != B_TIMED_OUT) {
				...
				pi.name[0] = 0;
				get_port_info(port, &pi);
				DOUT << PREFIX_TEXT << ": Error in team " << this_team()
