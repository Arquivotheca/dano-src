...
#define WNULLL		(unsigned short)0x0000
...
#define RK_SS2 (uchar)0x8e
#define RK_SS3 (uchar)0x8f
...
#define l_to_bst4(src, dst) {\
    dst[0] = (uchar)(((src) >> 24) & 0xff);\
    dst[1] = (uchar)(((src) >> 16) & 0xff);\
    dst[2] = (uchar)(((src) >> 8) & 0xff);\
    dst[3] = (uchar)((src) & 0xff);\
}
...
#define l_to_bst3(src, dst) {\
    dst[0] = (uchar)(((src) >> 16) & 0xff);\
    dst[1] = (uchar)(((src) >> 8) & 0xff);\
    dst[2] = (uchar)((src) & 0xff);\
}
...
#define s_to_bst2(src, dst) {\
    (dst)[0] = (uchar)((src >> 8) & 0xff);\
    (dst)[1] = (uchar)(src & 0xff);\
}
...
/* HEADER:
 *	uchar	magic[2];
 *	uchar	pakeuc;		key ga asshuku sareteiru
 *	uchar	name[];		jisho mei
...
#define WriteVal(csn, tick, buf){\
   (buf)[0] = (uchar)(((csn)>>12) & 0xff);\
   (buf)[1] = (uchar)(((csn)>>4) & 0xff);\
   (buf)[2] = (uchar)((((csn)<< 4) & 0xf0)|(((tick)>>16) & 0x0f));\
   (buf)[3] = (uchar)(((tick)>>8) & 0xff);\
   (buf)[4] = (uchar)((tick) & 0xff);\
}
 ...
 * tango:
 *	uchar	yomi[];			yomigana 
 *	uchar	nkouho;			kouho no kazu 
 *		uchar	flags;		kouho no nagasa + flags
 *		uchar	row;		kouho no row bangou
 *		uchar	col;		kouho no col bangou
 *		uchar	freq		kouho no shiyou hindo
 *		uchar	kouho[];
 *	
 */
...
struct ncache	{
    ...
    uchar		*nc_address;	/* word record no file address */
};
...
struct nword {
    ...
    uchar	nw_ylen;	/* #chars in yomi */
    uchar	nw_klen;	/* #chars in kanji */
    uchar	nw_class;	/* word class */
    uchar	nw_flags;	/* word flags (see below) */
    uchar	nw_lit;		/* literal conversion */
    ...
    uchar	*nw_kanji;	/* kanji kouho ichi/douteki na kouho */
...
/* attributes */
    ...
    uchar	dm_packbyte;	/* shouryaku sareta joui byte */
/* reference  count from MD */
    int			dm_rcount;
/* setuzoku jouhou (SWD nomi) */
    struct RkGram	*dm_gram;
    RkUnion		dm_extdata;
    uchar	*dm_qbits;	/* MS1214 gakushuu jouhou */
/* */
...
typedef struct TN {
  uchar	tn_flags;	/* type of node (see blow) */
  Wchar		tn_key;
  union {
    struct TD	*tree;
    struct TW	*word;
