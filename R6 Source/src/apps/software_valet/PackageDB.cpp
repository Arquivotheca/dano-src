...
const char kPackageDirName[] = "packages";
...
long PackageDB::DBDir(BEntry *dirEntry, dev_t dev)
{
	...
	// find dir on device; create it if needed
	if (find_directory(	B_COMMON_SETTINGS_DIRECTORY, dev, true,
						settingsPath, B_PATH_NAME_LENGTH) == B_OK)
	{
		BPath path(settingsPath, "packages");
		dirEntry->SetTo(path.Path(), true);
		PRINT(("Got %s\n",path.Path()));
		return B_NO_ERROR;
	} else {
		PRINT(("Did not find B_COMMON_SETTINGS_DIRECTORY\n"));
		return B_ERROR;
	}
}
...
long PackageDB::CreateDB(dev_t volID, BEntry *result)
{
	char settingsPath[B_PATH_NAME_LENGTH];
	// find dir on device; create it if needed
	if (find_directory(	B_COMMON_SETTINGS_DIRECTORY, volID, true,
						settingsPath, B_PATH_NAME_LENGTH) == B_OK)
	{
		BPath path(settingsPath, kPackageDirName);
		result->SetTo(path.Path(), true);
		if (!result->Exists()) {
			BDirectory parent, package_dir;
			result->GetParent(&parent);
			parent.CreateDirectory(kPackageDirName, &package_dir);
			result->SetTo(path.Path(), true); // is this necessary?
		}
		PRINT(("Got %s\n",path.Path()));
		return B_NO_ERROR;
	} else {
					...
					char *vid)
{
	...
	if (strcasecmp(suf,"Limited") == 0) {
		softtype = PackageItem::SOFT_TYPE_TRIAL;	
		memcpy(newname,name,c-name);
		newname[c-name] = 0;
		return true;
	}
	else if (strcasecmp(suf,"ESD") == 0) {
		softtype = PackageItem::SOFT_TYPE_COMMERCIAL;
		memcpy(newname,name,c-name);
		newname[c-name] = 0;
		return true;
	}
