...
TimeSlider::TimeSlider(BRect frame, const char *name,
	const char *label, BControl *target, BMessage *message)
: BSlider(frame, name, label, message, 0, sizeof(time_seconds) / sizeof(time_seconds[0]) - 1, B_TRIANGLE_THUMB, B_FOLLOW_LEFT | B_FOLLOW_TOP, B_FRAME_EVENTS|B_WILL_DRAW | B_NAVIGABLE),
  drawonupdate1(0), drawonupdate2(0), labeltarget(target)
{
//	labeltarget->SetLabel(label);
}
...
//	strcat(str, " after");
	...
	*str = 0;
	...
	num = seconds / 3600;
	if(num)
		sprintf(str + strlen(str), " %ld hour%s", num, num > 1 ? "s" : "");

	num = (seconds / 60) % 60;
	if(num)
		sprintf(str + strlen(str), " %ld minute%s", num, num > 1 ? "s" : "");

	num = seconds % 60;
	if(num)
		sprintf(str + strlen(str), " %ld second%s", num, num > 1 ? "s" : "");
...
UpperSlider::UpperSlider(BRect frame, const char *name,
	const char *label, BControl *target, BMessage *message)
 : TimeSlider(frame, name, label, target, message)
{
}
