 ...
 /* 次の off_from_top の計算がうさんくさいぞ */
	...
	for (src = localbuffer; src < (localbuffer + hdrsize);) {
		...
		if (len == 0) {
			...
			if (off < hdrsize) {
				memcpy(hd->data[i].ptr, localbuffer + off, len);
			} else {
				if (lseek(fd, off_from_top + off, 0) < 0) {
					RkSetErrno(RK_ERRNO_EACCES);
					goto read_err;
				}
...
_RkCreateHeader(struct HD * hd, unsigned *size)
{
	...
	for (i = 0; i < HD_MAXTAG; i++) {
		...
		if (hd->flag[i] == -1) {
			...
			memcpy(datadst, hd->data[i].ptr, len);
			datadst += len;
		} else {
			len = 0;
			off = 0;
...
_RkGetTick(int mode)
{
	static unsigned     ttime = 10000;

	return (mode ? ttime++ : ttime);
}
...
int
_RkGetLink(struct ND * dic, int pgno, unsigned off, unsigned *lvo, unsigned *csn)
{
