...
int
RkwCreateDic(int cx_num, unsigned char *dicname, int mode)
{
	struct RkParam     *sx = RkGetSystem();
	RkContext   *cx = RkGetContext(cx_num);
	struct DM          *sm, *um, *tm;
	int                 type;
	...
	if (mode & PL_DIC) {
		...
		if (!um) {
			...
			if (um->dm_rcount > 0)
				return TXTBSY;
			if (!um->dm_file)
				return BADCONT;	/* INVAL SHOULD BE
						 * REPLACED... MAKO 1225 */

			if (_RkRealizeDF(um->dm_file))	/* on 1993.11 */
				return ACCES;
			if (copyFile(sm, um))
				return ACCES;
			else
	...
	/* um = _RkSearchDDQ(userDDP, (char *)dicname, DF_TEMPDIC); */
		...
		if (!um) {
			...
			if (_RkRealizeDF(um->dm_file))
				return ACCES;
			return 1;	/* backward compatiblity ... 1224
					 * Xmas */
		};
	}
}
 ...
 */
int
RkwListDic(int cx_num, unsigned char *dirname, unsigned char *buf, int size)
{
	int                 dicscnt;
	int                 new_cx_num;
 ...
 */
int
RkwRemoveDic(int cx_num, unsigned char *dicname, int mode)
{
	RkContext   *cx = RkGetContext(cx_num);
 ...
 */
int
RkwRenameDic(int cx_num, unsigned char *old, unsigned char *newc, int mode)
{
	RkContext   *cx = RkGetContext(cx_num);
					...
					if (dm1) {
						...
						if (type != DF_RUCDIC) {
							...
							if (dm2) {	/* to があって、強制モードなら消す */
								...
								char               *templatestr, *filename;

								RkwSync(co, from);	/* sometimes, this
											 * failes to an error */
								templatestr =
								...
								res = ACCES;
								filename = _RkCreateUniquePath(userDDP[0], templatestr);
								if (filename) {
									char                spec[RK_LINE_BMAX];
...
pushTdn(RkContext * cx, struct TD * tdp)
{
	struct td_n_tupple *newc;
	struct _rec        *gwt;

	if (!cx || !(gwt = (struct _rec *) cx->cx_gwt) ||
	    !(newc = (struct td_n_tupple *) malloc(sizeof(struct td_n_tupple)))) {
		return (struct td_n_tupple *) 0;
	}
	newc->td = (char *)tdp;
	newc->n = 0;
	newc->next = (struct td_n_tupple *) gwt->tdn;
	gwt->tdn = (struct td_n_tupple *) newc;
	return newc;
}


void
freeTdn(RkContext * cx)
{
...
static int
GetLine(RkContext * cx, struct RkKxGram * gram, struct TD * tdp, Wchar * line, int size)
{
	struct TD          *vtd;
	struct TN          *vtn;
	struct _rec        *gwt = (struct _rec *) cx->cx_gwt;
