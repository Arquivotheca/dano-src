...
status_t 
IMAP::GetMessageInfo(const char *uid, MimeMessage *letter)
{
	status_t status = GetEnvelope(uid, letter);
	if (status == B_OK)
		status = GetStructure(uid, letter);
	return status;
}
	...
	while (fInHeader) {
		...
		if (type == kEndOfStreamToken) {
			...
			if (strcasecmp(GetTokenString(), "ok") == 0) {
				...
				if (ParseQuota() < 0)
					return B_ERROR;
			} else if (strcasecmp(GetTokenString(), "list") == 0) {
				if (ParseList() < 0)
					return B_ERROR;
			} else {
...
status_t IMAP::ParseList()
{
	...
	for (;;) {
		...
		if (type == kEndOfStreamToken || type == kEndOfLineToken) {
			break;
		}
		else if (strcmp(GetTokenString(), "(") == 0) {
			GetNextToken(); 
			...
			GetNextToken();
			fFolderList.AddItem(new BString(GetTokenString()));			
		}
	}
