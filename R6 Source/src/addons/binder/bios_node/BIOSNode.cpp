...
class BIOSNode : public BinderContainer
{
	Gehnaphore m_lock;

	get_status_t ReadProperty(const char *name, property &prop, const property_list &args)
	{
		if (!strcmp(name,"Reboot")) {
			if (getenv("MACHINE_ID")) {
				sync();
				_kshutdown_(true);
				prop = "failed";
			} else {
				printf("If I were on a BeIA device, I would be rebooting right now!\n");
				_sPrintf("If I were on a BeIA device, I would be rebooting right now!\n");
				prop = "on desktop";
			}
		} else
	...
	void ReadVersion()
	{
		...
		node_ref ref;
		BDirectory dir("/etc/update");
		dir.GetNodeRef(&ref);
		_kstart_watching_vnode_(ref.device, ref.node, B_WATCH_DIRECTORY, Port(), Token());
	
		BFile file("/etc/update/version_id",O_RDONLY);
		file.GetNodeRef(&ref);
		_kstart_watching_vnode_(ref.device, ref.node, B_WATCH_ALL, Port(), Token());
		
		char buffer[8192];
		int32 r = file.Read(buffer,8192);
		if (r > 0) {
			if (buffer[r-1] == '\n') r--;
			buffer[r] = 0;
			AddProperty("version_id",buffer);
		}
	}

	status_t HandleMessage(BMessage *msg)
	{
		if (msg->what == B_NODE_MONITOR) ReadVersion();
		else BinderContainer::HandleMessage(msg);
		return B_OK;
	}
	...
	BIOSNode()
	{
		...
		SetPermissions(permsRead);
		
		printf("*********************************** BIOSNode ****************************\n");

		const char *machine_id_var = getenv("MACHINE_ID");
		if (machine_id_var) AddProperty("device_id", machine_id_var);
		...
		if (!machine_id_var) {
			machine_id_var = (char*)find_string((const smbios_generic_struct*)si, si->serial_number);
			AddProperty("device_id",machine_id_var);
		}
		...
		AddProperty("bios_vendor",(char*)find_string((const smbios_generic_struct*)bi, bi->vendor));
		AddProperty("bios_version",(char*)find_string((const smbios_generic_struct*)bi, bi->bios_version));
		AddProperty("bios_date",(char*)find_string((const smbios_generic_struct*)bi, bi->bios_release_date));
