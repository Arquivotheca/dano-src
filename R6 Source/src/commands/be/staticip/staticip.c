// iad/src/commands/be/staticip.c
//
...
int main(long argc, char **argv) {
	...
	if (argc!=5) {
		printf("usage: staticip <ipaddr> <netmask> <router> <dns>\n"
		       "\tRewrites /boot/home/config/settings/beia-network to use a static \n"
		       "\tIP address.\n"
	...
	cf = fopen("/boot/home/config/settings/beia-network-static", "w");

	if (oldcf == NULL) {
		fprintf(stderr, "staticip: error: could not open beia-network\n");
		exit(1);
	}

	if (cf == NULL) {
		fprintf(stderr, "staticip: error: could not open beia-network-static for writing\n");
		exit(1);
	}
	...
	fprintf(cf, "interface0_ipaddress=\"%s\"\n", argv[1]);
	fprintf(cf, "interface0_netmask=\"%s\"\n", argv[2]);
	fprintf(cf, "interface0_router=\"%s\"\n", argv[3]);
	fprintf(cf, "interface0_dhcp=\"0\"\n");
	fprintf(cf, "DNS_PRIMARY=\"%s\"\n", argv[4]);
	fclose(cf);
	
	rename("/boot/home/config/settings/beia-network",
		"/boot/home/config/settings/beia-network-old");
	rename("/boot/home/config/settings/beia-network-static",
