...
//	ENDIAN_2 used for flattened BParameters
#define ENDIAN_2 0x02040607
//	ENDIAN_3 used for flattened BParameterGroups
#define ENDIAN_3_OLD 0x03040507
#define ENDIAN_3 0x03040509
...
const char * const NO_B_PARAMETER B_WEB_LOGICAL_BUS = "LogBus";
...
status_t 
BParameterWeb::StartWatching(const BMessenger &where, int32 notificationType)
{
	if(mNode != media_node::null)
	{
		...
		{
			fprintf(stderr,
				"BParameterWeb::StartWatching(): no BMediaRoster!\n");
			return B_BAD_VALUE;
		}
		return r->StartWatching(where, mNode, notificationType);
	}
	else
	{
		return BLocalControllable::AddObserver(this, where, notificationType);
	}
}

status_t 
BParameterWeb::StopWatching(const BMessenger &where, int32 notificationType)
{
	if(mNode != media_node::null)
	{
		BMediaRoster* r = BMediaRoster::CurrentRoster();
		...
		{
			fprintf(stderr,
				"BParameterWeb::StopWatching(): no BMediaRoster!\n");
			return B_BAD_VALUE;
		}
		return r->StopWatching(where, mNode, notificationType);
	}
	else
	{
		return BLocalControllable::RemoveObserver(this, where, notificationType);
	}
}
	...
	ssize_t size) const
{
	...
	put_4(buffer, mLocalID);
	put_4(buffer, mLocalTeam);
	put_4(buffer, mLocalChangeCount);
	...
	ssize_t size)
{
	...
	get_N(buf, size, &mNode, (ssize_t)sizeof(mNode));
	if (swap) swap_data(B_INT32_TYPE, &mNode, sizeof(mNode), B_SWAP_ALWAYS);
	get_4(buf, size, temp);
	mLocalID = (swap) ? B_SWAP_INT32(temp) : temp;
	get_4(buf, size, temp);
	mLocalTeam = (swap) ? B_SWAP_INT32(temp) : temp;
	get_4(buf, size, temp);
	mLocalChangeCount = (swap) ? B_SWAP_INT32(temp) : temp;
...
status_t 
BParameterWeb::ApplyParameterData(const void *data, size_t size)
{
	...
	for (int ix=0; ix<q->num_values; ix++)
	{
		...
		ParameterAt(ix)->SetValue(ptr, hdr.size, hdr.when);
		ptr += hdr.size;
	}
	return B_OK;
}
...
status_t 
BParameterWeb::MakeParameterData(void *buffer, size_t *ioSize)
{
	status_t err = B_OK;
	...
	ptr += sizeof(hdr1);
	togo -= sizeof(hdr1);
	for (int ix=0; ix<CountParameters(); ix++)
	{
		struct {
			bigtime_t when;
			int32 control;
		...
		status_t e = ParameterAt(ix)->GetValue(ptr+sizeof(hdr), &hdr.size, &hdr.when);
		if (e == B_OK)
		{
...
BTextParameter *
BParameterGroup::MakeTextParameter(int32 id, media_type m_type, const char *name, const char *kind, size_t max_bytes)
{
	CHECK_WEB(mWeb);
	BTextParameter* ret = new BTextParameter(id, m_type, mWeb, name, kind, max_bytes);
	ret->mGroup = this;
	mControls->AddItem((BParameter*)ret);
	return ret;
}
	...
	int32 type)
{
	...
	case BParameter::B_TEXT_PARAMETER:
		ret = MakeTextParameter(0, B_MEDIA_NO_TYPE, NULL, NULL, 0);
		break;
	default:
		break;
	}
	...
	ssize_t size)
{
	...
	else if (B_SWAP_INT32(temp) == ENDIAN_3) {
		flags = true;
		swap = true;
	}
	else if (temp == ENDIAN_3_OLD) {
		flags = false;
		swap = false;
	}
	else if (B_SWAP_INT32(temp) == ENDIAN_3_OLD) {
		flags = false;
		swap = true;
	}
	else {
	...
	if (flags) {
		get_4(buf, size, temp);
		mFlags = temp;
		if (swap) mFlags = B_SWAP_INT32(mFlags);
	}
	uchar nlen;
	get_1(buf, size, nlen);
	free(mName);
...
void 
BParameter::SetProperties(const BMessage *props)
{
	if(mProperties)
	{
		delete mProperties;
	}
	mProperties = new BMessage(*props);
}

BMessage * 
BParameter::Properties()
{
	return mProperties;
}
...
{
	CHECK_WEB(mWeb);
	dassert((((ValueType() == B_INT32_TYPE) || (ValueType() == B_FLOAT_TYPE)) && (*ioSize >= (CountChannels() * 4UL))) ||
		((ValueType() == B_BOOL_TYPE) && (*ioSize >= (CountChannels() * 1UL))));
	
	media_node node = mWeb->Node();
	if(node == media_node::null)
	{
		int32 localID = mWeb->LocalID();
		if(!localID)
			return B_BAD_TYPE;
		return BLocalControllable::GetValue(mWeb, mID, when, data, ioSize);
	}
	...
	cmd->count_ids = 1;
	cmd->ids[0] = mID;
	status_t err = write_port(node.port, CT_GET_VALUES, cmd, 
	...
	bigtime_t when)
{
	...
	if(node == media_node::null)
	{
		...
		if(!localID)
			return B_BAD_TYPE;
		return BLocalControllable::SetValue(mWeb, mID, when, data, size);
	}

	smart_ptr<set_values_q> cmd;
	cmd->node = node;
	cmd->num_values = 1;
	if(size > sizeof(cmd->raw_data))
		size = sizeof(cmd->raw_data);
	struct {
		bigtime_t when;
		int32 id;
		size_t size;
...
{
	CHECK_WEB(mWeb);
	ASSERT(!(Type() == B_TEXT_PARAMETER && channel_count > 1));
...
BParameter::FlattenedSize() const
{
	...
	total += 4;	/* mFlags */
	total += 4; /* mProperties size */
	if (mProperties) total += mProperties->FlattenedSize();
	return total;
}
...
BParameter::Flatten(void *buffer, ssize_t size) const
{
	...
	put_4(buffer, mFlags);
	temp = (mProperties) ? mProperties->FlattenedSize() : 0;
	put_4(buffer, temp);
	if (mProperties)
	{
		mProperties->Flatten((char*)buffer, temp);
	}
	return B_OK;
}
	...
	ssize_t size)
{
	...
	mFlags = 0;
	if (size >= 4) {
		get_4(buffer, size, mFlags);
		if (mSwapDetected) mFlags = B_SWAP_INT32(mFlags);
	}
	if (mProperties) {
		delete mProperties;
		mProperties = 0;
	...
	if (temp > 0)
	{
		mProperties = new BMessage;
		mProperties->Unflatten((char*)buffer);
		buffer = ((const char*)buffer) + temp;
	}
	return B_OK;
}
...
BDiscreteParameter::Flatten(void *buffer, ssize_t size) const
{
	...
	size_t upsize = BParameter::FlattenedSize();
	ssize_t mysize = (ssize_t)FlattenedSize();
...
{
	ssize_t total = BParameter::FlattenedSize();
	total += 4; // _max_bytes
	return total;
}
...
BTextParameter::Flatten(void *buffer, ssize_t size) const
{
	...
	put_4(buffer, _max_bytes);
	return B_OK;
}

status_t 
BTextParameter::Unflatten(type_code c, const void *buffer, ssize_t size)
{
	...
	get_4(buffer, size, _max_bytes);
	if(SwapOnUnflatten())
		_max_bytes = B_SWAP_INT32(_max_bytes);
...
BTextParameter::BTextParameter(
	...
	const char *kind,
	size_t max_bytes) :
	BParameter(id, m_type, B_TEXT_PARAMETER, web, name, kind, NULL),
	_max_bytes(max_bytes)
{
	CHECK_WEB(mWeb);
	DATA_NAME(BTextParameter)
}
...
status_t 
BTextParameter::_Reserved_TextParameter_0(void *)
{
	CHECK_WEB(mWeb);
	return B_ERROR;
}
...
status_t 
BTextParameter::_Reserved_TextParameter_1(void *)
{
	CHECK_WEB(mWeb);
	return B_ERROR;
}
...
status_t 
BTextParameter::_Reserved_TextParameter_2(void *)
{
	CHECK_WEB(mWeb);
	return B_ERROR;
}
...
status_t 
BTextParameter::_Reserved_TextParameter_3(void *)
{
	CHECK_WEB(mWeb);
	return B_ERROR;
}
...
status_t 
BTextParameter::_Reserved_TextParameter_4(void *)
{
	CHECK_WEB(mWeb);
	return B_ERROR;
}
...
status_t 
BTextParameter::_Reserved_TextParameter_5(void *)
{
	CHECK_WEB(mWeb);
	return B_ERROR;
}
...
status_t 
BTextParameter::_Reserved_TextParameter_6(void *)
{
	CHECK_WEB(mWeb);
	return B_ERROR;
}
...
status_t 
BTextParameter::_Reserved_TextParameter_7(void *)
{
	CHECK_WEB(mWeb);
	return B_ERROR;
}
