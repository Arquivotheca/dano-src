...
/*
	...
	Windows Revision Level:
		$Workfile:   SPCVRT.C  $
		$Logfile:   O:\pc_src\dll\stdprof\spcvrt.c_v  $
		$Revision:   2.1  $
		$Date:   04 Apr 1994 09:36:08  $
		$Author:   lsh  $
...
/*--------------------------------------------------------------------
 * DESCRIPTION
 *	Put N 4 byte numbers to a buffer in external format.
 *
 * AUTHOR
 * 	lsh
 *
...
/*--------------------------------------------------------------------
 * DESCRIPTION
 *	Put a short fixed point number to a buffer in external format.
 *
 * AUTHOR
 * 	lsh
 *
...
{
	while (Count--)
		SpPutUInt16 (Ptr, (KpUInt16_t) *Values++);
}


/*--------------------------------------------------------------------
 * DESCRIPTION
 *	Put a short fixed XYZ to a buffer in external format.
 *
 * AUTHOR
 * 	lsh
 *
...
{
	SpPutF1d15 (Ptr, &Value->X, 1);
	SpPutF1d15 (Ptr, &Value->Y, 1);
	SpPutF1d15 (Ptr, &Value->Z, 1);
}
...
/*--------------------------------------------------------------------
 * DESCRIPTION
 *	Get N 4 byte numbers in internal format.
 *
 * AUTHOR
 * 	lsh
 *
...
void SpGetUInt32s (
				char		KPHUGE * FAR *Ptr,
				KpUInt32_t	KPHUGE *Value,
				KpUInt32_t	Count)
{
	while (Count--)
		*Value++ = SpGetUInt32 (Ptr);
}
...
/*--------------------------------------------------------------------
 * DESCRIPTION
 *	Convert a short fixed to internal format.
 *
 * AUTHOR
 * 	lsh
 *
...
{
	while (Count--)
		*Values++ = SpGetUInt16 (Ptr);
}


/*--------------------------------------------------------------------
 * DESCRIPTION
 *	Convert a short fixed XYZ to internal format.
 *
 * AUTHOR
 * 	lsh
 *
...
{
	SpGetF1d15 (Ptr, &XYZ->X, 1);
	SpGetF1d15 (Ptr, &XYZ->Y, 1);
	SpGetF1d15 (Ptr, &XYZ->Z, 1);
}
