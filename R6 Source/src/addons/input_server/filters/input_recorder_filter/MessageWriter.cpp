			...
			if(!_Error) {
				...
				if(memcmp(buffer, DATA_TYPE, strlen(DATA_TYPE)+1)){
					...
					if(data > DATA_VERSION){
						...
						fprintf(stderr, "Data is of later version: %d\n", data);
					} else if(data == 1) {
						if(!memcmp(cp, _VendorClass, strlen(_VendorClass)+1)){
							cp += strlen(_VendorClass)+1;
						}else{
							fprintf(stderr, "File was written with unsupported class: %s\n", cp);
						}
					} else if(data == 0) {
						data = *((int32*)cp);
						swap_data(B_INT32_TYPE, &data, sizeof(int32), B_SWAP_LENDIAN_TO_HOST);
						...
						if(data != 1) { // Once refered to as: T_WRITE_ATTRIBUTE_FILES
							...
							fprintf(stderr, "File is stored in unsupported format: %d, we only support 2\n", data);
						} else if(dirtymime){
							_File->RemoveAttr("BEOS:TYPE");
							_File->WriteAttr("BEOS:TYPE", B_MIME_TYPE, 0, DATA_TYPE, sizeof(DATA_TYPE));
						}
...
status_t AttributeMessageWriter :: Write(BMessage *msg)
{
	...
	sprintf(namebuf, "%s/%08x",_AttrBase,_Item);
	...
	if ((_Error = _File->SetTo(name, fmode )) == B_OK) {
		if(mode & B_WRITE_ONLY){
			...
			if(!strcmp(buffer, "application/octet-stream")){
				...
				_Error = B_BAD_TYPE;
				SERIAL_PRINT(("InputRecorder:Attribute says file is not our type: (Read: '%s' Expected: '%s')\n", buffer, DATA_TYPE));
			}
			if(!_Error) {
				_File->Read(buffer, T_MAX_HEADER_SIZE-1);
				buffer[T_MAX_HEADER_SIZE-1] = '\0';
				if(memcmp(buffer, DATA_TYPE, strlen(DATA_TYPE)+1)){
					_Error = B_BAD_TYPE;
					SERIAL_PRINT(("InputRecorder:File is not our type.\n"));
				} else {
					...
					if(data > DATA_VERSION){
						_Error = T_BAD_VERSION;
						SERIAL_PRINT(( "InputRecorder: Data is of later version: %d\n", data));
					} else if(data == 1) {
						if(!memcmp(cp, _VendorClass, strlen(_VendorClass)+1)){
							cp += strlen(_VendorClass)+1;
						}else{
							SERIAL_PRINT(("InputRecorder: File was written with unsupported class: %s\n", cp));
						}
					} else if(data == 0) {
						data = *((int32*)cp);
						swap_data(B_INT32_TYPE, &data, sizeof(int32), B_SWAP_LENDIAN_TO_HOST);
						...
						if(data != 2) { // once refered to as T_WRITE_NORMAL_FILES
							_Error = B_UNSUPPORTED;
							SERIAL_PRINT(("File is stored in unsupported format: %d, we only support 2\n", data));
						} else if(dirtymime){
							_File->RemoveAttr("BEOS:TYPE");
							_File->WriteAttr("BEOS:TYPE", B_MIME_TYPE, 0, DATA_TYPE, sizeof(DATA_TYPE));
						}
...
void FlatMessageWriter :: ResetDataPointer(void)
{
	_File->Seek(_EndOfHeader, SEEK_SET);
	if(_Mode == B_WRITE_ONLY) {
		_File->SetSize(0);
	}
}
...
status_t FlatMessageWriter :: Write(BMessage *msg)
{
	...
	if(_Error) SERIAL_PRINT(("InputRecorder: %s:%d - Failed to Write Input: (0x%x) %s\n",__FILE__,__LINE__, _Error, strerror(_Error)));
...
status_t FlatMessageWriter :: Read(BMessage *msg)
{
	...
	if(_Error) SERIAL_PRINT(("InputRecorder: %s:%d - Failed to Read Input Normally: (0x%x) %s\n",__FILE__,__LINE__, _Error, strerror(_Error)));
...
bool FlatMessageWriter :: HaveMore(void)
{
	...
	result = (_File->Position() < _LengthOfFile);
...
const char *AreaMessageWriter :: _VendorClass = "Be/AreaMessageWriter";
