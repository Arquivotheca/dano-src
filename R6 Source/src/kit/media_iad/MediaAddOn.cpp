...
dormant_flavor_info::dormant_flavor_info(const dormant_flavor_info & that)
{
	assign_atoms(that);
...
status_t dormant_flavor_info::Unflatten(type_code c, const void *buf, ssize_t size)
{
	...
	if (s < sizeof(media_format) * (in_format_count + out_format_count)) {
	...
	void * _reserved)
{
	...
	if (err == B_OK)
		fprintf(stderr, "Old Node (%d); calling SniffType(%s) works (but what kind?)\n",
				ImageID(), type.Type());
	return err;
}
