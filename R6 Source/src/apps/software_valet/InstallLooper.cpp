...
//	#endif
		...
		if (err >= B_NO_ERROR) {
			...
			int result = doError("Installation is finished.", NULL, "OK");
//			closeWhenComplete = (result != 0);
			// Close the InstallWindow all the time now, since we don't ask whether
			// or not to close
					...
					(it->data.FindInt32("registered") == PackageItem::REGISTERED_NO) )
			{
				...
				// don't auto-close when the register window is up
				reg.AddBool("quit_after_reg", true);
				InstallWindow::SetCloseApp(false);
									...
									bool topDir)
{
	...
	**/
	...
	fCatalog->StartFolderGet();
	int32 subitem;
	while ((subitem = fCatalog->GetFolderItemType()) != 0) {
		switch(subitem) {
			case FOLDER_FLAG:
