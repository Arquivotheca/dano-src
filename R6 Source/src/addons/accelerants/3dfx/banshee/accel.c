...
status_t acquireengine (uint32 caps, uint32 max_wait, sync_token * st, engine_token ** et)
{
	(void) max_wait;

// dprintf (("3dfx_accel: acquireengine(et:0x%08x, st:0x%08x) - ENTER\n", et, st));
...
void waitengineidle (void)
{
	...
	do
	{
		stat = _V3_ReadReg( ci, V3_2D_STATUS );
 ...
 */
void rectfill_gen (engine_token * et,
				  register uint32 color, register fill_rect_params * list, register uint32 count)
{
	uint32 srcColorFormat;

	if (et != &enginetoken)
	{
//    dprintf(("3dfx_accel: blit - EXIT - no enginetoken\n"));
		return;
	}
	switch (ci->Depth)
	{
	...
	case 32:
		...
		break;
	default:
		dprintf (("3dfx_accel: blit - Unsupported Color Format !\n"));
		return;
	}
	...
	{
		FifoMakeRoom (1, 8);
		SET_PKT2_HEADER (SSTCP_CLIP0MIN | SSTCP_CLIP0MAX | SSTCP_DSTBASEADDR | SSTCP_DSTFORMAT |
							  SSTCP_COLORFORE | SSTCP_DSTSIZE | SSTCP_DSTXY | SSTCP_COMMAND);
...
void rectangle_invert (engine_token * et, register fill_rect_params * list, register uint32 count)
{
	uint32 srcColorFormat;

	if (et != &enginetoken)
	{
//    dprintf(("3dfx_accel: rectangle_invert - EXIT - no enginetoken\n"));
		return;
	}
	switch (ci->Depth)
	{
	...
	case 32:
		...
		break;
	default:
		dprintf (("3dfx_accel: rectangle_invert - Unsupported Color Format !\n"));
		return;
	}
	while (count--)
	{
		...
		FifoWriteLong (1, SST_2D_CLIP0MIN, 0);
		FifoWriteLong (1, SST_2D_CLIP0MAX, (list->right + 1) | ((list->bottom + 1) << 16));
		FifoWriteLong (1, SST_2D_DSTBASEADDR,
							((uint32) (ci->FBBase) - (uint32) (ci->BaseAddr1)));
		FifoWriteLong (1, SST_2D_DSTFORMAT,
							(ci->BytesPerRow << SSTG_DST_STRIDE_SHIFT) | (srcColorFormat));
		FifoWriteLong (1, SST_2D_DSTSIZE,
							(list->right - list->left + 1) | ((list->bottom - list->top + 1) << 16));
		FifoWriteLong (1, SST_2D_DSTXY, (list->left) | ((list->top) << 16));
		FifoWriteLong (1, SST_2D_COMMAND,
							(SSTG_RECTFILL | SSTG_GO | (SSTG_ROP_NDST << SSTG_ROP0_SHIFT)));
		list++;
	}
	ci->PrimitivesIssued++;
	WriteSerialNumber (ci->PrimitivesIssued);
...
//    dprintf(("3dfx_accel: blit - EXIT - no enginetoken\n"));
	...
	while (count--)
	{
		...
		FifoMakeRoom (1, 11);
		SET_PKT2_HEADER (SSTCP_DSTBASEADDR | SSTCP_SRCBASEADDR | SSTCP_CLIP0MIN | SSTCP_DSTFORMAT |
							  SSTCP_SRCFORMAT | SSTCP_SRCSIZE | SSTCP_SRCXY | SSTCP_DSTXY | SSTCP_CLIP0MAX
							  | SSTCP_DSTSIZE | SSTCP_COMMAND);
		FifoWriteLong (1, SST_2D_CLIP0MIN, 0);
		/* The Clipping Region is specified in screen co-ordinates */
		FifoWriteLong (1, SST_2D_CLIP0MAX,
							(list->dest_left + list->width +
							 1) | ((list->dest_top + list->height + 1) << 16));
		FifoWriteLong (1, SST_2D_DSTBASEADDR,
							((uint32) (ci->FBBase) - (uint32) (ci->BaseAddr1)));
		FifoWriteLong (1, SST_2D_DSTFORMAT,
							(ci->BytesPerRow << SSTG_DST_STRIDE_SHIFT) | (srcColorFormat));
		FifoWriteLong (1, SST_2D_SRCBASEADDR,
							((uint32) (ci->FBBase) - (uint32) (ci->BaseAddr1)));
		FifoWriteLong (1, SST_2D_SRCFORMAT,
							(ci->BytesPerRow << SSTG_DST_STRIDE_SHIFT) | (srcColorFormat));
		FifoWriteLong (1, SST_2D_SRCSIZE, (list->width + 1) | ((list->height + 1) << 16));
		FifoWriteLong (1, SST_2D_SRCXY,
							(list->src_left) + blitOffsetX | ((list->src_top + blitOffsetY) << 16));
		FifoWriteLong (1, SST_2D_DSTSIZE, (list->width + 1) | ((list->height + 1) << 16));
		FifoWriteLong (1, SST_2D_DSTXY,
							(list->dest_left) + blitOffsetX | ((list->dest_top + blitOffsetY) << 16));
		// Write the command register
		FifoWriteLong (1, SST_2D_COMMAND,
							(SSTG_BLT | SSTG_GO | SSTG_ROP_SRCCOPY | (blitDirection << 14)));
...
void span_fill (engine_token * et, uint32 color, uint16 * list, uint32 count)
{
	uint32 srcColorFormat;
	...
	case 32:
		...
		break;
	default:
		dprintf (("3dfx_accel: span_fill - Unsupported Color Format !\n"));
		return;
	}
	...
	{
		FifoMakeRoom (1, 8);
		SET_PKT2_HEADER (SSTCP_DSTBASEADDR | SSTCP_DSTFORMAT | SSTCP_CLIP0MIN | SSTCP_COLORFORE |
							  SSTCP_DSTXY | SSTCP_DSTSIZE | SSTCP_CLIP0MAX | SSTCP_COMMAND);
		FifoWriteLong (1, SST_2D_CLIP0MIN, 0);
		/* The Clipping Region is specified in screen co-ordinates */
...
void WriteSerialNumber (uint32 serial_num)
{
	...
	packet_buffer[0] = (0x1 << SSTCP_PKT4_MASK_SHIFT) |
		((((uint32) offsetof (regs_3d, nopCMD)) << 1)) | SSTCP_PKT4;
	packet_buffer[1] = 0;
	FifoAllocateSlots (1, 2);
	...
	packet_buffer[0] = (SSTCP_PKT5_LFB | (1 /*32 bit word */  << SSTCP_PKT5_NWORDS_SHIFT) |
							  SSTCP_PKT5);
	packet_buffer[1] =
		((uint32) ((uint32) (ci->AllocSentinal->address) - (uint32) (ci->BaseAddr1))) &
		SSTCP_PKT5_BASEADDR;
...
status_t Init3DEngine (char *device_path, void *context)
{
	...
	if (!initCount)
	{
		accelerator_image = load_add_on ("_CPU_accelerants/libVoodoo3");
		...
		{
			if (get_image_symbol
				 (accelerator_image, "_AcceleratorInit", B_SYMBOL_TYPE_TEXT, (void **) &init) < B_OK)
			{
...
void get_device_info (char *device_path, const char **name, uint8 * depth, uint8 * stencil,
							 uint8 * accum)
{
	*name = "3dfx Voodoo 3";
	*depth = 1;
...
void get_3d_modes (char *device_path, void (*callback) (video_mode * modes), uint32 min_color,
						 uint32 min_depth, uint32 min_stencil, uint32 min_accum)
{
	...
	if (min_accum > BGL_NONE)
		return;
	if (!
		 (((min_color & BGL_BIT_DEPTH_MASK) < BGL_8_BIT) ||
