...
status_t AIFFExtractor::Sniff(int32 *out_streamNum, int32 *out_chunkSize) {
	...
	pio->Seek(0, SEEK_SET);	
	if (pio->Read(&form, sizeof(form)) != sizeof(form))
		return B_IO_ERROR;
...
status_t AIFFExtractor::ReadHeader() {
	int8					codec;
	int16					rate;
	int32					skip, frame_size, pos, length, channels, offset, blockSize, lastpos;
	status_t				err;
	...
	while (pio->Read(&chunk_head, sizeof(chunk_head)) == sizeof(chunk_head)) {	
PRINTF("Header 0x%08x [%c%c%c%c]\n", B_BENDIAN_TO_HOST_INT32(chunk_head.magic),
		...
		case _IFF_DATA :
			...
			if (length < fDataLength)
				fDataLength = length;
			break;
		case _IFF_VERSION :
			if (pio->Seek(12, SEEK_CUR) < 0)
				return B_IO_ERROR;
			if (pio->Read(&rate, 2) != 2)
				return B_IO_ERROR;
			if (pio->Seek(1, SEEK_CUR) < 0)
				return B_IO_ERROR;
			if (pio->Read(&codec, 1) != 1)
				return B_IO_ERROR;
			if (pio->Seek(4, SEEK_CUR) < 0)
				return B_IO_ERROR;
			fSampleFormat = 0x1;
			if (codec != 0)
				return B_BAD_TYPE;
...
status_t AIFFExtractor::ReadCommon() {
	...
	if (pio->Read(&channel_count, 2) != 2)
		return B_IO_ERROR;
	if (pio->Read(&frame_count, 4) != 4)
		return B_IO_ERROR;
	if (pio->Read(&sample_size, 2) != 2)
		return B_IO_ERROR;
	if (pio->Read(&srate_exponent, 2) != 2)
		return B_IO_ERROR;
	if (pio->Read(&srate_mantissa_0, 4) != 4)
		return B_IO_ERROR;
	if (pio->Read(&srate_mantissa_1, 4) != 4)
		return B_IO_ERROR;
...
status_t AIFFExtractor::ReadVersion()
{
	...
	int32 timestamp;
	io->Read(&timestamp, sizeof(timestamp));
	
	return B_OK;
}

/* Provide information about output format */
status_t AIFFExtractor::TrackInfo(int32 in_stream, media_format *out_format, void **out_info, int32 *out_size) {
...
status_t 
AIFFExtractor::AllocateCookie(int32 in_stream, void **cookieptr)
{
...
status_t 
AIFFExtractor::FreeCookie(int32 in_stream, void *cookie)
{
	free(cookie);
	return B_NO_ERROR;
}
