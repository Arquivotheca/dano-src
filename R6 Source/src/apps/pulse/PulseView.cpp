...
bool PulseView::Update(int32 accuracy) {
	system_info sys_info;
	get_system_info(&sys_info);
	...
	cpu_count = sys_info.cpu_count;
	if (cpu_count > B_MAX_CPU_COUNT) cpu_count = B_MAX_CPU_COUNT;
	for (int x = 0; x < cpu_count; x++) {
		...
		int32 cur = (int32)(cpu_time*accuracy);
		int32 last = (int32)(cpu_times[x]*accuracy);
