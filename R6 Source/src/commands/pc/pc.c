...
#define STR1     "%20Lu  0x%.16Lx signed: %20Ld" 
#define STR2     "%20Lu  0x%.16Lx"
#define STR3     " char: "
...
usage(char *pname)
{
	fprintf(stderr, "%s -- A Programmer's (Integer) Calculator.\n", pname);
	fprintf(stderr, "\tBy default it reads from stdin and evaluates the\n");
	fprintf(stderr, "\texpressions typed.  You can also give an expression\n");
	fprintf(stderr, "\ton the command line (make sure you do proper quoting)\n");
	fprintf(stderr, "\tand have that evaluated.  All standard C expressions\n");
	fprintf(stderr, "\tare understood.  Variables are also supported.\n");
	exit(0);
}
