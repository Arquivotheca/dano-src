...
{
	ASSERT(signature < kNumBitmaps);
	if (stash[signature] == 0) {
...
{
	snooze(500000);
		// hack for initial delay, should be a parameter really
	for (;;) {
		if (requestToQuit)
			break;
...
filter_result 
SkipButtonKeypressFilter::Filter(BMessage *message, BHandler**)
{
	if (target->IsEnabled()
...
void 
TransportButton::MouseMoved(BPoint point, uint32, const BMessage *)
{
