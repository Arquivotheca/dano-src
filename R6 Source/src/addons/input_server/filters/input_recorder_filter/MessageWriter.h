 ...
 *
 *   The file format version 2, uses absolute points in the input record, and
 *     relative points (if used) should be handled at play-back. Also, non-input
 *     events started appearing in the stream. (T_TIME_WARP, T_ALERT)
*/
...
public:	
	...
	status_t InitCheck();
	virtual void ResetDataPointer(void) =0;
...
public:	
	...
	virtual ~FlatMessageWriter ();
	virtual void ResetDataPointer(void);
...
public:	
	...
	virtual ~AttributeMessageWriter ();
...
class AreaMessageWriter : public MessageWriter
{
public:	
	AreaMessageWriter (area_id area, int32 mode);
	virtual ~AreaMessageWriter ();
	virtual void ResetDataPointer(void);
	virtual bool HaveMore(void);
	virtual status_t Write(BMessage *msg);
	virtual status_t Read(BMessage *msg);
