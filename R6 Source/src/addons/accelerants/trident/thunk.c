...
_set_display_mode (display_mode *mode_to_set)
{
	...
	if ((retval = ioctl (devfd, MAXIOCTL_TRIDENT + B_SET_DISPLAY_MODE,
			     &sdm, sizeof (sdm))) == B_OK)
		retval = AccelInit (ci);
	else
		retval = errno;
...
_get_accelerant_mode_count (void)
{
	...
	ioctl (devfd, MAXIOCTL_TRIDENT + B_ACCELERANT_MODE_COUNT,
	       &count, sizeof (count));
	return (count);
}
...
_get_mode_list (display_mode *dm)
{
	...
	/*  The size is wrong...  */
	if ((retval = ioctl (devfd, MAXIOCTL_TRIDENT + B_GET_MODE_LIST,
			     dm, sizeof (*dm))) < 0)
		retval = errno;
...
{
	trident_propose_display_mode	pdm;
	status_t			retval;

	pdm.pdm_Target	= target;
	pdm.pdm_Lo	= low;
	pdm.pdm_Hi	= high;
	if ((retval = ioctl (devfd, MAXIOCTL_TRIDENT + B_PROPOSE_DISPLAY_MODE,
			     &pdm, sizeof (pdm))) < 0)
		retval = errno;
...
_get_frame_buffer_config (frame_buffer_config *a_frame_buffer)
{
	...
	if ((retval = ioctl (devfd, MAXIOCTL_TRIDENT + B_GET_FRAME_BUFFER_CONFIG,
...
{
	trident_pixel_clock_limits	pcl;
	status_t			retval;

	pcl.pcl_DisplayMode	= dm;
	if ((retval = ioctl (devfd, MAXIOCTL_TRIDENT + B_GET_PIXEL_CLOCK_LIMITS,
			     &pcl, sizeof (pcl))) == B_OK)
	{
		*low = pcl.pcl_Lo;
		*high = pcl.pcl_Hi;
...
{
	trident_move_display md;

	md.md_HDisplayStart	= h_display_start;
	md.md_VDisplayStart	= v_display_start;
	return (ioctl (devfd, MAXIOCTL_TRIDENT + B_MOVE_DISPLAY,
		       &md, sizeof (md)));
}
#endif
...
{
	trident_set_indexed_colors sic;

	sic.sic_Count		= count;
	sic.sic_First		= first;
	sic.sic_ColorData	= color_data;
	sic.sic_Flags		= flags;
	ioctl (devfd, MAXIOCTL_TRIDENT + B_SET_INDEXED_COLORS,
	       &sic, sizeof (sic));
}
...
_get_dpms_mode (void)
{
	...
	ioctl (devfd, MAXIOCTL_TRIDENT + B_DPMS_MODE,
	       &mode, sizeof (mode));
	return (mode);
}
...
_set_dpms_mode (uint32 dpms_flags)
{
	...
	if ((retval = ioctl (devfd, MAXIOCTL_TRIDENT + B_SET_DPMS_MODE,
...
)
{
	...
	if ((retval = ioctl (devfd, MAXIOCTL_TRIDENT + B_SET_CURSOR_SHAPE,
		       &scs, sizeof (scs))) < 0)
		retval = errno;
...
_show_cursor (bool on)
{
	 ...
	 */
	ioctl (devfd, MAXIOCTL_TRIDENT + B_SHOW_CURSOR,
	       &on, sizeof (on));
}
