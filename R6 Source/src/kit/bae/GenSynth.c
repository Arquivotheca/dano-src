...
{// 15000000
		   0, 10000000, 5000000, 4000000, 3000000, 2500000, 2000000, 1500000, 1000000, 700000,
...
void PV_DoCallBack(GM_Voice *this_one)
{
	...
	{
		(*this_one->NoteEndCallback)(this_one->NoteContext);
	}
}
...
void PV_CalcScaleBack(void)
{
	...
	pMixer = MusicGlobals;
	scaleSize = (pMixer->MaxNotes*UPSCALAR + pMixer->MaxEffects*UPSCALAR) << 8;
	...
	pMixer->scaleBackAmount = (scaleSize << 8) / (pMixer->MaxNotes*UPSCALAR+pMixer->MaxEffects*UPSCALAR);
										...
										GM_Voice *pVoice)
{
	...
	bufferSize = pVoice->NotePtrEnd - pVoice->NotePtr;
...
unsigned long PV_GetPositionFromVoice(GM_Voice *pVoice)
{
	...
	size = pVoice->NotePtrEnd - pVoice->NotePtr;	// get sample size of active voice
...
UFLOAT PV_GetWavePitchFloat(XFIXED notePitch)
{
	...
	{
		case Q_48K:
			//return (22050.0 * pitch) / 48000.0;
			return pitch * 0.459375;
		case Q_24K:
			//return (22050.0 * pitch) / 24000.0;
			return pitch * 0.91875;
		case Q_44K:
			//return (22050.0 * pitch) / 44100.0;
			return pitch * 0.5;
		case Q_22K:
		case Q_22K_TERP_44K:
			//return (22050.0 * pitch) / 22050.0;
			return pitch; // * 1.0;
		case Q_11K:
		case Q_11K_TERP_22K:
			//return (22050.0 * pitch) / 11025.0;
			return pitch * 2.0;
		case Q_8K:
			//return (22050.0 * pitch) / 8000.0;
			return pitch * 2.75625;
	}
...
// If performing at 44 khz then the pitch rates must be 1/8
		...
		case Q_22K:
		case Q_22K_TERP_44K:
// if performing at 22 khz then pitches must be 1/4 for the 20.12 fixed values
	...
	switch (a->ADSRFlags[index])
	{
		case ADSR_SUSTAIN:
			...
			if (a->ADSRLevel[index] < 0)
			{
				if (a->sustainingDecayLevel)
				{
					...
					else
					{
						...
						a->sustainingDecayLevel = ( a->sustainingDecayLevel * 
													(expDecayLookup[logLookupTable[-ADSRLevel]/50000L] >> 1L)  ) >> 15L;
					}
				}
			}
			else
			{
				if (currentTime)
				{
					currentTime += BAE_GetSliceTimeInMicroseconds() - 610;		// microseconds;
		...
		default:
			...
			if (currentTime >= a->ADSRTime[index])
			{
				...
				else
				{
					...
					currentTime -= BAE_GetSliceTimeInMicroseconds() - 610;		// prevent long note times from overflowing if they stay on for more than 32.767 seconds
				}
			}
			else
			{
...
// Process LFO's
	...
	if (pVoice->LFORecordCount)
	{
		for (i = 0; i < pVoice->LFORecordCount; i++)
		{
			...
			{
				rec->LFOcurrentTime += BAE_GetSliceTimeInMicroseconds() - 610;
				if (rec->period)
				{
...
static void PV_FillTone(void *pBuffer, unsigned long bufferByteSize)
{
	char	*pAudio;
	long	count, toneSize;
	...
	toneSize = sizeof(tone) / sizeof(unsigned short);
...
// Set up the various procs for filters:
	switch (pMixer->interpolationMode)
	{
		...
		case E_LINEAR_INTERPOLATION_U3232:
			...
			break;
	#endif
	#if (USE_TERP2 == TRUE) || (USE_DROP_SAMPLE == TRUE) || (USE_TERP1 == TRUE)
		default:
...
void PV_ProcessSampleFrame(void *threadContext, void *destinationSamples)
{
	...
	if (pMixer->systemPaused == FALSE)
	{
		...
		if (pMixer->pModPlaying)
		{
			...
			{
				PV_WriteModOutput(pMixer->outputQuality, pMixer->generateStereoOutput);
			}
		}
#endif
...
// or notes naturally fading out (preferable)
	...
	for (count = 0; count < pMixer->MaxNotes; count++)
	{
		...
		{
			the_entry = &pMixer->NoteEntry[count];
			goto EnterNote;
		}

#if 1
		if (pMixer->NoteEntry[count].voiceMode != VOICE_SUSTAINING)
		{
			if (bestLevel > 0x2000)
			{
				bestLevel = 0x2000;
		...
		{
			if (pMixer->NoteEntry[count].NoteProgram == the_instrument)
			{
				if (pMixer->NoteEntry[count].NoteChannel == the_channel)
				{
					if (pMixer->NoteEntry[count].volumeADSRRecord.sustainingDecayLevel < bestLevel)
					{
						bestLevel = pMixer->NoteEntry[count].volumeADSRRecord.sustainingDecayLevel;
						the_entry = &pMixer->NoteEntry[count];
					}
				}
			}
...
// Now kill notes that are much lower in volume than the current note (less than 25% of the volume)
	for (count = 0; count < pMixer->MaxNotes; count++)
	{
		if (pMixer->NoteEntry[count].NoteVolume < bestLevel)
		{
			bestLevel = pMixer->NoteEntry[count].NoteVolume;
			the_entry = &pMixer->NoteEntry[count];
		}
	}
	...
	for (count = 0; count < pMixer->MaxNotes; count++)
	{
		if ( bestLevel > ((pMixer->NoteEntry[count].NoteVolume * 
			pMixer->NoteEntry[count].NoteVolumeEnvelopeBeforeLFO) >> VOLUME_PRECISION_SCALAR))
		{
			bestLevel = (pMixer->NoteEntry[count].NoteVolume * 
				pMixer->NoteEntry[count].NoteVolumeEnvelopeBeforeLFO) >> VOLUME_PRECISION_SCALAR;
			the_entry = &pMixer->NoteEntry[count];
...
// Now kill notes that are in sustain pedal mode, are in same channel or instrument
	for (count = 0; count < pMixer->MaxNotes; count++)
	{
		if (pMixer->NoteEntry[count].sustainMode != SUS_NORMAL)
		{
			if (pMixer->NoteEntry[count].NoteProgram == the_instrument)
			{
				if (pMixer->NoteEntry[count].NoteChannel == the_channel)
...
// drums rolls don't sound right.
	for (count = 0; count < pMixer->MaxNotes; count++)
		if (pMixer->NoteEntry[count].voiceMode != VOICE_UNUSED)
			if (pMixer->NoteEntry[count].NoteProgram == the_instrument)
				if (pMixer->NoteEntry[count].NoteMIDIPitch == notePitch)
					if (pMixer->NoteEntry[count].NoteChannel == the_channel)
					{
						if (the_channel != PERCUSSION_CHANNEL)
						{
							pMixer->NoteEntry[count].volumeADSRRecord.mode = ADSR_TERMINATE;
							pMixer->NoteEntry[count].volumeADSRRecord.currentPosition = 0;
							pMixer->NoteEntry[count].volumeADSRRecord.ADSRLevel[0] = 0;
							pMixer->NoteEntry[count].volumeADSRRecord.ADSRTime[0] = 1;
							pMixer->NoteEntry[count].volumeADSRRecord.ADSRFlags[0] = ADSR_TERMINATE;
							pMixer->NoteEntry[count].NoteVolumeEnvelopeBeforeLFO = 0;		// so these notes can be reused
						}
					}
#endif
	return;
...
//
// This function is primarly called from the BAE midi sequencer. There's no need to call it directly.
void PV_StopMIDINote(GM_Song *pSong, INT16 the_instrument, INT16 the_channel, INT16 the_track, INT16 notePitch)
{
	register LOOPCOUNT		count;
	register GM_Mixer		*pMixer;
