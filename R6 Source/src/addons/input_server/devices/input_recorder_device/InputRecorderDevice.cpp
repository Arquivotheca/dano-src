...
status_t InputRecorderDevice::Start(const char *device, void *cookie)
{
	...
	if(_Port < 0) {
		SERIAL_PRINT(("InputRecorderDevice: create_port error: (0x%x) %s\n",_Port,strerror(_Port)));
		return(_Port);
	}
...
void InputRecorderDevice::_WatchPort()
{
	...
	while (true) {
		...
		if(err != length) {
			if(err >= 0) {
				SERIAL_PRINT(("InputRecorderDevice: failed to read full packet (read %u of %u)\n",err,length));
			} else {
				SERIAL_PRINT(("InputRecorderDevice: read_port error: (0x%x) %s\n",err,strerror(err)));
			}
		} else if ((err = event->Unflatten(buffer)) < 0) {
			SERIAL_PRINT(("InputRecorderDevice: (0x%x) %s\n",err,strerror(err)));
		} else {
