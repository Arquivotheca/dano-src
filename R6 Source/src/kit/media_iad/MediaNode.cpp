...
FPRINTF((stderr, "BMediaNode::Acquire() (%ld/%s) count %d\n", _mNodeID, _mName, _mRefCount));
	return this;
}
...
{
FPRINTF((stderr, "%s::Release() (%ld/%s) count %d\n", Name(), _mNodeID, _mName, _mRefCount));
#if DEBUG
if ((_mRefCount < 0) || (_mRefCount > 100000)) debugger("Bad Release() happened\n");
	...
	if (r == 1) {
		if (DeleteHook(this) != B_OK) {
			DIAGNOSTIC((stderr, "%d::DeleteHook() returned error!\n", id));
		}
	}
	return (r > 1) ? this : 0;
}
...
//		return err;
	...
	//	call base interface
	if (BMediaNode::HandleMessage(code, msg, B_MEDIA_MESSAGE_SIZE) == B_OK)
		goto ok;

	//	call possible subclass interfaces
	if((_m_producerThis != NULL) &&
			(_m_producerThis->BBufferProducer::HandleMessage(code, msg, B_MEDIA_MESSAGE_SIZE) == B_OK))
		goto ok;
		
	if((_m_consumerThis != NULL) &&
			(_m_consumerThis->BBufferConsumer::HandleMessage(code, msg, B_MEDIA_MESSAGE_SIZE) == B_OK))
		goto ok;
		
	if((_m_fileInterfaceThis != NULL) &&
			(_m_fileInterfaceThis->BFileInterface::HandleMessage(code, msg, B_MEDIA_MESSAGE_SIZE) == B_OK))
		goto ok;
		
	if ((_m_controllableThis != NULL) &&
			(_m_controllableThis->BControllable::HandleMessage(code, msg, B_MEDIA_MESSAGE_SIZE) == B_OK))
		goto ok;
		
	if ((_m_timeSourceThis != NULL) &&
			(_m_timeSourceThis->BTimeSource::HandleMessage(code, msg, B_MEDIA_MESSAGE_SIZE) == B_OK))
		goto ok;

	//	call most-derived interface
	if (HandleMessage(code, msg, B_MEDIA_MESSAGE_SIZE) == B_OK)
		goto ok;

	HandleBadMessage(code, msg, B_MEDIA_MESSAGE_SIZE);
...
/*
	if (!_mTimeSource) {
		...
		if (((_BMediaRosterP *)BMediaRoster::Roster())->GetTimeSource(&ts) == B_OK) {
			...
			printf("IMPLICITLY setting time source to %d, %08x\n",ts.node,_mTimeSource);
	...
	size_t size)
{
	...
	case M_SET_TIMESOURCE: {
		...
		BTimeSource * source = new _BTimeSourceP(_mTimeSourceID);
	...
	case M_REQUEST_COMPLETED: {
		...
		mri->format.MetaData();
		assert((size == sizeof(media_request_info)+4) || (mri->what != media_request_info::B_REQUEST_FORMAT_CHANGE));
