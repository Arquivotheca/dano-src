/* :ts=8 bk=0
 *
 * banshee.c:	Main Voodoo Banshee (and Voodoo 3) accelerant.  Works extremely closely
...
int32			devfd = -1;
int32			clonefd = -1;
__mem_AreaDef	*MemMgr_2D;

__mem_Area *memArea =0;
area_id memArea_id;
...
//	dprintf(("3dfx_accel: get_accelerant_hook - ENTER, feature = 0x%x\n", feature));
...
// Clone Hooks are only required for Game Kit Support
	...
	case B_PROPOSE_DISPLAY_MODE:
		return (void *) propose_video_mode;
	...
	dprintf(("3dfx_accel: get_accelerant_hook(%d, 0x%08x) failed!\n", feature, data));
//	dprintf(("3dfx_accel: get_accelerant_hook - EXIT\n"));
	return 0;
}
...
dprintf(("3dfx_accel: init - ENTER\n"));
	...
	{
//		dprintf (("3dfx_accel: init - Failed to get globals.\n"));
		return (retval);
	}
	...
	if ((ci_areaid = clone_area ("3dfx driver data: share",
				     ...
				     gg.gg_GlobalArea)) < 0)
	{
//		dprintf (("3dfx_accel: init - Failed to clone global area.\n"));
	}

	MemMgr_2D = __mem_InitClient( devfd );
dprintf (("3dfx_accel: MemMgr_2D %p\n", MemMgr_2D));
dprintf (("3dfx_accel: init - InitClient.\n"));

	// Initialise the card
dprintf (("3dfx_accel: init - calling thdfx_init()\n"));
	thdfx_init();
...
dprintf(("3dfx_accel: init - Allocating 3D Fifo\n"));
...
dprintf(("3dfx_accel: Fifo Alloc 0 start = %ld \n", ci->AllocFifo[0]->start ));
dprintf(("3dfx_accel: Fifo Alloc 0 end = %ld \n", ci->AllocFifo[0]->end ));
	

	// Allocate the command fifo (for 2D)
dprintf(("3dfx_accel: init - Allocating 2D Fifo\n"));
...
dprintf(("3dfx_accel: Fifo Alloc 1 start = %ld \n", ci->AllocFifo[1]->start ));
dprintf(("3dfx_accel: Fifo Alloc 1 end = %ld \n", ci->AllocFifo[1]->end ));

	ci->PrimitivesIssued = 0;
	ci->AllocSentinal = __mem_Allocate( MemMgr_2D, 64 );		// Space for AllocSentinal
	ci->AllocSentinal->locked = 255;
dprintf(("AllocSentinal = %ld \n", ci->AllocSentinal ));
dprintf(("AllocSentinal start = %ld \n", ci->AllocSentinal->start ));
dprintf(("AllocSentinal end = %ld \n", ci->AllocSentinal->end ));


	ci->AllocCursor = __mem_Allocate( MemMgr_2D, SIZEOF_CURSORPAGE );		// Space for cursor
	ci->AllocCursor->locked = 255;
	ci->CursorBase = (void *) ci->AllocCursor->address;
dprintf(("AllocCursor = %ld \n", ci->AllocCursor ));
dprintf(("AllocCursor start = %ld \n", ci->AllocCursor->start ));
dprintf(("AllocCursor end = %ld \n", ci->AllocCursor->end ));
	...
	if (InitFifo(1) != B_OK)
	{
		dprintf(("3dfx_accel: init - InitFifo(1) failed\n"));
	}

	if (InitFifo(0) != B_OK)
	{
		dprintf(("3dfx_accel: init  - InitFifo(0) failed\n"));
	}

dprintf(("3dfx_accel: init - returning 0x%x\n", ci_areaid < 0 ?  ci_areaid :  B_OK));
...
//	dprintf (("3dfx_accel: init_clone() src=0x%08x\n", data));
	strcpy (devname, "/dev/");
...
dprintf(("3dfx_accel: init_clone - ENTER\n"));
...
//	MemMgr_2D = __mem_InitClient( clonefd );
dprintf (("3dfx_accel: init_clone - Done.\n"));
...
static void uninit (void)
{
	dprintf (("3dfx_accel: uninit - ENTER\n"));
	...
	dprintf (("3dfx_accel: uninit - EXIT\n"));
}
