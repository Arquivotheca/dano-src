...
class MimeMessage {
	public:
		...
		status_t 				LoadFromBuffer(BufferedFileAdapter &stream);
		status_t 				LoadPartsFromBuffer(BufferedFileAdapter &stream, MessagePart *part, int32 version);
		inline void				LoadField(BufferedFileAdapter &stream, BString &field);
		inline void 			ExtractPart(BufferedFileAdapter &stream, MessagePart *part, int32 version);
...
index from the message where a MIME section starts.  It won't be present
