...
const int kMaxMCUs = 10;									// the standard allows up to 10, but we only support up to 22:11:11 sampling so...
...
//  App tag proc
//  Define the app proc that gets called when a tag in the range 0xE0 through 0xEF is detected
//  Returns any valid JPEGError
typedef JPEGError (*InterpretAppProcPtr)(void* j, uchar tag, const uchar* data, short size);
...
typedef struct JPEGDecoder {
	...
	InterpretAppProcPtr	AppTagProc;
	void*			DrawRefCon;
...
JPEGDecoder* NewJPEGDecoder(const BRect* r, short thumbnail, DrawRowProcPtr drawProc, InterpretAppProcPtr appProc, const BRect* invalid);
...
JPEGError	InterpretAPP(JPEGDecoder* j, uchar tag, const uchar* data, short size);
