/* ++++++++++
	...
	REVS:	$Revision: 1.2 $
	NAME:	herold
	DATE:	Fri Jan 19 16:12:56 PST 1996
	Copyright (c) 1996 by Be Incorporated.  All Rights Reserved.
+++++ */
extern long tabgauche2;
extern long tabdroite2;
extern long OffScreen_largeur2;
extern long OffScreen_baseAddr2;
...
BG_EraseTampon (void)
{
				sub			ZROWBYTES,ZROWBYTES,ZROWSIZE
				subi		ZDEST,ZDEST,4
@lab0			subfic		ZTEMP,ZDEST,28
				andi.		ZTEMP,ZTEMP,31
				sub			ZSIZE2,ZROWSIZE,ZTEMP
				beq			@lab1
@lab2			stwu		ZCOLOR,4(ZDEST)
				subic.		ZTEMP,ZTEMP,4
				bne			@lab2
@lab1			//li			ZCPT,16
				srawi		ZTEMP,ZSIZE2,5
@lab3			//dcbz		ZCPT,ZDEST
				subic.		ZTEMP,ZTEMP,1
				stwu		ZCOLOR,4(ZDEST)
				stwu		ZCOLOR,4(ZDEST)
				stwu		ZCOLOR,4(ZDEST)
				...
				stwu		ZCOLOR,4(ZDEST)
				bne			@lab3
				andi.		ZTEMP,ZSIZE2,31
				beq			@lab4
@lab5			stwu		ZCOLOR,4(ZDEST)
				subic.		ZTEMP,ZTEMP,4
				bgt			@lab5
@lab4			add			ZDEST,ZDEST,ZROWBYTES
				subic.		ZNBLINE,ZNBLINE,1
				bgt			@lab0
				blr
}
...
#define QPASH2			r23
#define QLUM			r22
#define Qtabgauche2		r21
#define Qtabdroite2		r20
#define QLARGEUR		r19
...
//  preprocessing of test end of point list
				...
				lhz		RX2,0(RTAB)
				lhz		RL1,-2(RTAB)
				lwz		RTABLEAU,tabdroite2(RTOC)
				b		@tabdroite2
				
@tabgauche2		lhz		RX2,-6(RTAB)	// permut the points
				lhz		RX1,0(RTAB)
				lhz		RY2,-4(RTAB)
				lhz		RY1,2(RTAB)
				lhz		RL1,4(RTAB)
				lwz		RTABLEAU,tabgauche2(RTOC)
@tabdroite2
...
//  goureaud the horizontal segments one by one.				
				lwz		Qtabgauche2,tabgauche2(RTOC)
				lwz		Qtabdroite2,tabdroite2(RTOC)
				lwz		QLARGEUR,OffScreen_largeur2(RTOC)
				lwz		QDRAWLINE,OffScreen_baseAddr2(RTOC)
...
@line0			
	//  preprocessing of end test
				cmpw	cr5,RMIN,RMAX
				lhax	QX2,Qtabgauche2,RMIN
				lhax	QX1,Qtabdroite2,RMIN
