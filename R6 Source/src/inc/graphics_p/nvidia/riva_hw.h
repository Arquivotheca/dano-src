...
typedef struct _riva_hw_inst
{
    ...
    U032 IO;
    U032 VBlankBit;
//    U032 FifoFreeCount;  /*  ewhac  */
    U032 *FIFOFreeCntPtr;  /*  ewhac  */
    U032 FifoEmptyCount;
    U032 BusyBit;  /*  ewhac  */
    /*
     * Non-FIFO registers.
...
{                                                                  \
   while ((hwinst).FifoFreeCount < (cnt)) {                          \
        mem_barrier();  /*  What does this mean?  */  \
...
#define RIVA_FIFO_FREE(hwinst,hwptr,cnt)				\
{									\
   while (*(hwinst).FIFOFreeCntPtr < (cnt)) {				\
	*(hwinst).FIFOFreeCntPtr = (hwinst).hwptr->FifoFree >> 2;	\
   }									\
   *(hwinst).FIFOFreeCntPtr -= (cnt);					\
}
#endif
