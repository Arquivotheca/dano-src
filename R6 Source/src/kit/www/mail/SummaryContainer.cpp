...
status_t SummaryContainer::Sync(MailServer *server)
{
	MDB(MailDebug md);
	ASSERT(server != NULL);
	...
	for (int32 k = 0; k < uidCount; k++) {
		...
		if (server->GetMessageInfo(uid.String(), message) != B_OK)
			continue;
...
status_t SummaryContainer::SyncLocalFlagsUpstream(MailServer *server)
{
	ASSERT(server != NULL);
...
status_t SummaryContainer::SyncNewEntriesUpstream(MailServer *server)
{
	ASSERT(server != NULL);
	(void)server;
...
status_t SummaryContainer::Expunge(MailServer *server)
{
	ASSERT(server != NULL);
	(void)server;
