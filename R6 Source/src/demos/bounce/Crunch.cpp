...
/*
 Can't find a constant for 'pi', so I'll use:
	acos(0) = pi/2
*/

inline float rad_to_degree(float rad)
{
	return 360.0 * (rad / (4 * acos(0.0)));
}

inline float degree_to_rad(float degree)
{
	return (degree / 360.0) * (4 * acos(0.0));
}
...
//	PRINT(("angle = %.1f, ", angle)); PRINT_OBJECT(the_rect);
	...
	// create a rect that is the 1st quadrant
	BRect qrect(0, dv, dh, 0);
	...
	float end, BRect bounds)
{
	...
	*/
	...
	float	corner = corner_case(start, end);
	float	angle;
