...
#define RESAMPLE DO_NOTHING //FPRINTF
#define LOCATION DO_NOTHING //FPRINTF
#define NODE DO_NOTHING //FPRINTF
#define DATA DO_NOTHING //FPRINTF
#define CONNECT FPRINTF
#define LATENCY DO_NOTHING //FPRINTF
...
FPRINTF(stderr, "pre legacy_check: %g; %lx; %ld; %ld; %ld\n", out.frame_rate, out.format,
		out.channel_count, out.byte_order, out.buffer_size);
	legacy_check(out);
FPRINTF(stderr, "post legacy_check: %g; %lx; %ld; %ld; %ld\n", out.frame_rate, out.format,
		out.channel_count, out.byte_order, out.buffer_size);
}
...
BSoundPlayer::get_volume_slider()
{
	...
	DIAGNOSTIC(stderr, "BSoundPlayer: did not find gain slider for ID %ld in mixer\n",
...
float BSoundPlayer::VolumeDB(bool forcePoll)
{
	...
	//	calculate average
	float volume = 0;
	for (uint ix=0; ix<ioSize/sizeof(float); ix++) {
		volume += array[ix];
	}
...
{
	bigtime_t latency = 0;
	BMediaRoster::Roster()->GetLatencyFor(_m_node->Node(), &latency);
	return latency;
}
	...
	const media_raw_audio_format & format)
{
	...
	try {
		...
		size_t nframes = size/((format.format&0xf)*format.channel_count);
