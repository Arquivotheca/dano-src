...
bool LoadInDeskbar() {
	...
	if (deskbar.HasItem("DeskbarPulseView")) return false;
	// Otherwise, add it
	if (deskbar.AddItem(&ai.ref) == B_OK) return true;
	else return false;
}
...
BView *instantiate_deskbar_item() {
	...
	int width = prefs.deskbar_icon_width;
	int min_width = GetMinimumViewWidth();
	if (width < min_width) {
		prefs.deskbar_icon_width = min_width;
		width = min_width;
	}

	BRect rect(0, 0, width - 1, 15);
	return new DeskbarPulseView(rect);
}
...
PulseApp::PulseApp(int argc, char **argv) : BApplication(APP_SIGNATURE) {
	...
	bool quit_if_deskbar_present = false;
	if (deskbar) {
	...
	prefs->Save();
	BuildPulse(quit_if_deskbar_present);
}

void PulseApp::BuildPulse(bool quit_if_deskbar_present) {
	...
	// Remove this case for Deskbar add on API
		...
		if (LoadInDeskbar()) {
			...
			// If we ran Pulse --deskbar but the replicant was
			// already present then quit. If we double clicked
			// in Tracker, then launch Pulse in the normal mode
			if (quit_if_deskbar_present) {
				PostMessage(new BMessage(B_QUIT_REQUESTED));
				return;
			} else {
