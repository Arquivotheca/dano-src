...
static
struct nword	*
setLit(RkContext *cx, struct nword *word, int maxword, int rc, Wchar *src, int srclen, int format)
{
  struct nword	*w = word;
  int 		dstlen;
  unsigned long	mode;
...
static
struct nword	*
readWord(RkContext *cx, int yy, int ys, int ye, int classnum, struct nword *nword, int maxword, int doflush, int douniq)
{
  Wchar		*key = cx->store->yomi + yy;
  struct nword	*wrds;
  struct MD	*head = cx->md[classnum], *md;
  static struct nread	nread[128];
  ...
  for (md = head->md_next; md != head; md = md->md_next) {
    ...
    for (c = 0; c < nc; c++) {
      ...
      unsigned		offset;
      int		nk, cnt = 1;
      static unsigned		permutation[RK_CAND_NMAX];
      static unsigned char	*candidates[RK_CAND_NMAX];
      unsigned		csnb;
      int		bitSize;
...
static
struct nword	*
makeWord(RkContext *cx, int yy, int ys, int ye, int classnum, struct nword *word, int maxword, int doflush, int douniq)
{
 ...
 */
static int
parseWord(RkContext *cx, int yy, int ys, int ye, int classnum, struct nword *xqh[], int maxclen, int doflush, int douniq)
{
  ...
  for (clen = 0; (clen <= maxclen && clen < ye); clen++) {
    ...
    static struct nword	right[64*16];
    int			sameLen;
    int			t;
    struct nword	*p, *q, *r;
    int			ys1, ye1;

    /* 読みの長さ clen の単語のうち、後ろに classnum で指定された単語が
       つながる可能性があるものをリストアップし、tail に記録する */
    for (p = xqh[clen], sameLen = 0; p; p = p->nw_next) {
      if (classmask[p->nw_class] & (1<<classnum)) {
	/* p の後ろに classnum の単語がつながる可能性がある */
    ...
    ye1 = ye - clen;
    r = readWord(cx, yy + clen, ys1, ye1, classnum,
		 right, RkNumber(right) - 1, doflush, douniq);
    if (Is_Word_Make(cx)) 
      r = makeWord(cx, yy + clen, ys1, ye1, classnum,
...
/*ARGSUSED*/
static int
doParse(RkContext *cx, int yy, int ys, int ye, struct nword *xqh[], int maxclen, int doflush, int douniq)
{
...
static
int
compword(struct compRec *x, struct compRec *y)
{
...
{
  int			i;
  struct nword		*e, *p, *head = (struct nword *)0, *tail;
...
static
struct nword	*
parseBun(RkContext *cx, int yy, int ys, int ye, int doflush, int douniq, int *maxclen)	/* bunsetu saidai moji suu */
{
  struct nstore	*st = cx->store;
...
static void
storeBun(RkContext *cx, int yy, int ys, int ye, struct nbun *bun)
{
  struct nword	*full;
  struct nword	*w;
  int		maxclen;
...
static
int	
calcSplit(RkContext *cx, int yy, struct nword *top, struct nqueue xq[], int maxclen, int flush)
{
  unsigned		L, L1 = 0, L2;
  unsigned		U2;
  struct nword	*w;
...
static
int
splitBun(RkContext *cx, int yy, int ys, int ye)
{
  struct nstore			*st = cx->store;
  struct nqueue	*xq = st->xq;
  struct nword		*w;
 ...
 */
static void
parseQue(RkContext *cx, int maxq, int yy, int ys, int ye, int doflush)
{
  struct nstore		 *st = cx->store;
  struct nqueue *xq = st->xq;
  struct nword	 **xqh = st->xqh;
...
static
int
IsStableQue(RkContext *cx, int c, int doflush)
{
...
static
int
Que2Bun(RkContext *cx, int yy, int ys, int ye, int doflush)
{
  struct nstore	*st = cx->store;
  struct nqueue	*xq = st->xq;
  unsigned	i;
 ...
 */
int
_RkRenbun2(RkContext *cx, int firstlen)  /* bunsetsu chou sitei(ow 0) */
{
  struct nstore		*st = cx->store;
 ...
 */
int
_RkSubstYomi(RkContext *cx, int ys, int ye, Wchar *yomi, int newLen)
{
  struct nstore		*st = cx->store;
  struct nbun	*bun;
  struct nqueue		*xq;
...
static void
doLearn(RkContext *cx, struct nword *thisW)
{
  struct nword	*leftW;
...
void
_RkLearnBun(RkContext *cx, int cur, int mode)
{
  struct nstore	*st = cx->store;
