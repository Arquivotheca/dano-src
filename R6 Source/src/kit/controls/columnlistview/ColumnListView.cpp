...
/*
static const rgb_color kTitleColor 				= {0x97, 0x9b, 0x8e, 0xFF};
static const rgb_color kTitleTextColor 			= {0xFF, 0xFF, 0xFF, 0xFF};
static const rgb_color kDefaultBackgroundColor 	= {0xDD, 0xDD, 0xDD, 0xFF};
static const rgb_color kRowDividerColor 		= {0x80, 0x84, 0x73, 0xFF};
static const rgb_color kDefaultSelectionColor 	= {0xFF, 0xFF, 0xFF, 0xFF};
static const rgb_color kDefaultEditColor 		= {0x80, 0x84, 0x73, 0xFF};
static const rgb_color kNonFocusSelectionColor	= {0xDD, 0xDD, 0xDD, 0xFF};
*/

static const float kTitleHeight = 19.0;
static const float kLatchWidth = 0.0;
#else
...
static const rgb_color kColor[B_COLOR_TOTAL] =
{
    ...
    {0xDD, 0xDD, 0xDD, 0xFF},       // B_COLOR_BACKGROUND
    {0x00, 0x00, 0x00, 0xFF},       // B_COLOR_TEXT
    {0x80, 0x84, 0x73, 0xFF},       // B_COLOR_ROW_DIVIDER
    {0xFF, 0xFF, 0xFF, 0xFF},       // B_COLOR_SELECTION
    {0x00, 0x00, 0x00, 0xFF},       // B_COLOR_SELECTION_TEXT
    {0xDD, 0xDD, 0xDD, 0xFF},       // B_COLOR_NON_FOCUS_SELECTION
    {0x80, 0x84, 0x73, 0xFF},       // B_COLOR_EDIT_BACKGROUND
    {0x00, 0x00, 0x00, 0xFF},       // B_COLOR_EDIT_TEXT
    {0x97, 0x9b, 0x8e, 0xFF},       // B_COLOR_HEADER_BACKGROUND
    {0xFF, 0xFF, 0xFF, 0xFF},       // B_COLOR_HEADER_TEXT
    {0x00, 0x00, 0x00, 0xFF},       // B_COLOR_SEPARATOR_LINE
    {0x00, 0x00, 0x00, 0xFF},       // B_COLOR_SEPARATOR_BORDER
...
#if NETRON_MODE
	fUpSortArrow->SetBits((const void*) kUpSortArrow8x8Invert, 64, 0, B_COLOR_8_BIT);
	fDownSortArrow->SetBits((const void*) kDownSortArrow8x8Invert, 64, 0, B_COLOR_8_BIT);
#else
...
void TitleView::DrawTitle(BView *view, BRect rect, BColumn *column, bool depressed)
{
	...
	view->FillRect(rect);
	view->SetHighColor(borderColor);
	view->StrokeLine(rect.LeftBottom(),rect.RightBottom()-BPoint(1,0));

	if(column && column->ShowHeading()) {
		// Make sure to skip the first heading
		for(int32 c=0; (c < fMasterView->CountColumns()) && (column != fMasterView->ColumnAt(c)); c++) {
			if(fMasterView->ColumnAt(c)->ShowHeading()) {
...
{
	if (row1 && row2) {
		for (int32 index = 0; index < fSortColumns->CountItems(); index++) {
