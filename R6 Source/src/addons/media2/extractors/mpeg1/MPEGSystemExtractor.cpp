...
extern "C" B::Private::Extractor* instantiate_extractor()
{
	const lock_status_t ls = locker.Lock();
	BAutolock autolock(ls);
...
MPEGSystemExtractor::GetFormatInfo()
{
	...
	ssize_t headerSize;
	if(seek->Seek(0LL,SEEK_SET)<0)
		return B_IO_ERROR;
	if ((headerSize = in->Read(&header[0], sizeof(header))) < 4) {
...
//		printf("position: %Ld\n",pio->Position());
		if(seek->Seek(id3length,SEEK_CUR)<0)
			return B_IO_ERROR;
		headerSize = in->Read(header, sizeof(header));
		if(headerSize < 4)
			return (headerSize >= 0) ? EINVAL : headerSize;
	}
...
#if 0
// +++ ugh.  we should cope with non-seekability properly
			...
			0 != strcmp("application/x-vnd.Web",ai.signature))
		{
			/* look for ID3 v1 tag */
			uchar buff[128];
			if(seek->Seek(fFileSize - 128,SEEK_SET)<0)
				return B_IO_ERROR;
			if (fAudioSize > 128 && in->Read( buff, 128) == 128) {
...
long MPEGSystemExtractor::BackgroundScanner()
{
	...
	while (fScanThread != -1 && (seek->Seek(pos,SEEK_SET)>=0) &&(len = in->Read(fSeekBuf, SEEK_CHUNK_SIZE)) >= HEADER_SIZE){
		ssize_t ret;
		if (fFileType == kVideoStreamOnly) {
			struct layer *l;
			l = (struct layer *)malloc(sizeof(*l));
...
status_t MPEGSystemExtractor::Sniff(int32 *out_streamNum, int32 *out_chunkSize)
{
	...
	read_size = in->Read(start, sizeof(start));
	if(read_size < 4) {
		if(read_size >= 0)
			return B_BAD_TYPE;
		return read_size;
...
//	printf("mpeg_audio_seek\n");
	...
	if(seek->Seek(pos,SEEK_SET)<0)
		return B_IO_ERROR;	
	ssize_t sizeRead = in->Read(fSeekBuf, read_size);
	if (sizeRead < 4) {
