...
TMultiThumbMediaSlider::TMultiThumbMediaSlider(TransportView *parent, BRect frame,
...
TMultiThumbMediaSlider::DrawBar(BView *osv, BRect barFrame)
{
	...
	if (IsEnabled()) {
		...
		if (fill) {
			...
			BRect enabledRect(fillRect);
			float midPos = barFrame.left + (fEnabledPortion * barFrame.Width());
			if (midPos > enabledRect.left && midPos < enabledRect.right)
				enabledRect.right = midPos;

			osv->SetHighColor(kFillGreen);
			osv->FillRect(enabledRect);
			if (midPos > fillRect.left && midPos < fillRect.right
				&& fEnabledPortion < 1.0) {
				BRect disabledRect(fillRect);
				disabledRect.left = midPos + 1;
				osv->SetDrawingMode(B_OP_COPY);
				osv->SetLowColor(144, 136, 186);
				osv->FillRect(disabledRect, B_MIXED_COLORS);
			}
		}
...
TMultiThumbMediaSlider::DrawHashMarks(BView *osv, BRect barFrame, BRect )
{
	...
	float disableStart = barFrame.left + barFrame.Width() * fEnabledPortion;
	while(true) {		
		if (pt1.x >= fillRect.left && pt1.x <= fillRect.right) {
			if (pt1.x < disableStart) {
				SetLowColor(kFillGreen);
				highHashColor.red = 144;
				highHashColor.green = 186;
				highHashColor.blue = 136;
				lowHashColor.red = 169;
				lowHashColor.green = 224;
				lowHashColor.blue = 158;
			} else {
				SetLowColor(kFillGreen);
				highHashColor.red = 124;
				highHashColor.green = 166;
				highHashColor.blue = 116;
	...
	if (drawFixed && fLabelPlacement != kLabelCenterWhileTracking) {
		...
		// 	in the fixed label - won't leak out the other side
 		if (!drawTracking
			...
			DrawLabelBox(osv, fixedRect, bg, kWhiteColor, kBorderGray);
	...
	rgb_color)
{
	...
	minutes = minutes % 60;
	seconds = seconds % 60;
	int hundredths = (playTime / 10) % 100;
	...
	loc.x += offset;
	offset = DrawDigit(osv, bits, hundredths >= 0 ? hundredths / 10 : 0, loc);
	loc.x += offset + 1;
	offset = DrawDigit(osv, bits, hundredths >= 0 ? hundredths % 10 : 0, loc);
...
void TMultiThumbMediaSlider::SetEnabledPortion(float ratio)
{
	if (fEnabledPortion == ratio)
		return;
	...
	if (Window()->Lock()) {
		...
		float x1 = PointForValue(MIN(ratio, fEnabledPortion) * MaxValue()).x - 5;
		float x2 = PointForValue(MAX(ratio, fEnabledPortion) * MaxValue()).x + 5;
		Invalidate(BRect(x1, bounds.top, x2, bounds.bottom));
		fEnabledPortion = ratio;
		Window()->Unlock();
	}
}
