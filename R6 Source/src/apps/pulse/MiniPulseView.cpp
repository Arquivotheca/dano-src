...
void MiniPulseView::Draw(BRect rect) {
	if (cpu_count > B_MAX_CPU_COUNT || cpu_count <= 0) return;
	...
	float top = 1, left = 1;
	float bottom = top + h;
	float bar_width = (bounds.Width()) / cpu_count - 2;
...
void MiniPulseView::Pulse() {
	...
	if (!IsHidden()) {
		...
		// Max accuracy is either the view size*32 intermediate levels,
		// or 100 for the tool tip.
		int32 accuracy = int32(b.Height()*32);
