...
#define DMGL_NO_TPARAMS	(1 << 2)	/* Exclude all template parameters */
...
#define PRINT_TARGS	      (!(work -> options & DMGL_NO_TPARAMS))
...
/* char *cplus_demangle (const char *mangled, int options)
   ...
   The OPTIONS arg may contain one or more of the following bits:
	...
	DMGL_TPARAMS	Template parameters are included.
   ...
   cplus_demangle ("foo__t1A1Zi", DMGL_TPARAMS)		=> "A<int>::foo"
   cplus_demangle ("foo__t1A1Zi", 0)			=> "A<>::foo"
     ...
     int remember;
{
  ...
  for (i = 0; i < r; i++)
      ...
      else
	{
	  ...
	  string_init (&temp);
	  success = demangle_template_value_parm (work, mangled, &temp);
...
int
demangle(char *mangled_name, char *unmangled_name, size_t len)
{
	char *ptr;

	ptr = cplus_demangle(mangled_name, DMGL_PARAMS|DMGL_ANSI);
	if (ptr == NULL)
		return 0;

	strncpy(unmangled_name, ptr, len);
	unmangled_name[len-1] = 0;
	free(ptr);
