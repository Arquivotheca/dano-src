	...
	int PMV[2][2],video_data *ld)
{
...
#if ROTATE_DISPLAY
		//printf("Motion Compensation FORWARD\n");
		form_prediction(ld->forward_reference_frame,ld->current_frame,
			ld->Coded_Height,ld->Coded_Height-by-16,bx,-PMV[0][1],PMV[0][0],0);
#else
	...
	if (macroblock_type & MACROBLOCK_MOTION_BACKWARD)
	{
		...
		form_prediction(ld->backward_reference_frame,ld->current_frame,
			ld->Coded_Height,ld->Coded_Height-by-16,bx,-PMV[1][1],PMV[1][0],stw);
#else
		/* frame-based prediction */
		//printf("Motion Compensation BACKWARD\n");
		form_prediction(ld->backward_reference_frame,ld->current_frame,
