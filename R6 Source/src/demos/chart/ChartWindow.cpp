...
: BDirectWindow(frame, name, B_TITLED_WINDOW, 0)
{
	...
	/* init the bitmap buffer in a valid state */	
	bitmap_buffer.clip_list_count = 1;
	bitmap_buffer.clip_list = new clipping_rect[1];

	/* init the direct buffer in a valid state */	
	direct_buffer.clip_list_count = 1;
	direct_buffer.clip_list = new clipping_rect[1];
...
/* build the UI content of the window */
	...
	top_view->SetViewUIColor(B_UI_PANEL_BACKGROUND_COLOR);
	AddChild(top_view);
	...
	left_view->SetViewUIColor(B_UI_PANEL_BACKGROUND_COLOR);
	AddChild(left_view);
...
ChartWindow::~ChartWindow()
{
	...
	delete [] direct_buffer.clip_list;
	delete [] bitmap_buffer.clip_list;
}
...
void ChartWindow::OpenColorPalette(BPoint here)
{
	...
	if (window == NULL) {
		...
		color_ctrl->SetViewUIColor(B_UI_PANEL_BACKGROUND_COLOR);
...
void ChartWindow::OpenStarDensity(BPoint here)
{
	...
	if (window == NULL) {
		...
		slider->SetViewUIColor(B_UI_PANEL_BACKGROUND_COLOR);
...
void ChartWindow::SwitchContext(direct_buffer_info *info)
{
	...
	case B_DIRECT_MODIFY :
		...
		/* Allocate a new clipping region */
		if (info->clip_list_count) {
			delete [] direct_buffer.clip_list;
			direct_buffer.clip_list = new clipping_rect[info->clip_list_count];
		}
