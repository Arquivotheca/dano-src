/*____________________________________________________________________________
	...
	$Id: cupL1.c,v 1.2 1999/07/13 18:42:13 robert Exp $
____________________________________________________________________________*/
...
static float *cs_factorL1 = cs_factor[0];
static float look_c_valueL1[16];
...
static void unpack_baL1()
{
   ...
   for (j = 0; j < nbatL1; j++)
   {
      ...
      if (j >= nsb_limit)
	 bit_skip += bat_bit_masterL1[samp_dispatch[j]];
...
/*-------------------------------------------------------------------------*/
#define UNPACKL1_N(n) s[k]     =  cs_factorL1[k]*(load(n)-((1 << (n-1)) -1));  \
    goto dispatch;
