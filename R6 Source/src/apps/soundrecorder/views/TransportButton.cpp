...
PeriodicMessageSender::Launch(BMessenger target, const BMessage *message,
	bigtime_t period)
{
	PeriodicMessageSender *result;
	result = new PeriodicMessageSender(target, message, period);
...
TransportButton::BitsForMask(uint32 mask) const
{
	switch (mask) {
		...
		case kPressedMask:
			return pressedBits;
		case kPressedMask | kDisabledMask:
			return disabledBits;
		default:
			return normalBits;
	}	
...
RecordButton::RecordButton(BRect frame, const char *name,
		...
		uint32 key,						// optional shortcut key
		uint32 modifiers,				// optional shortcut key modifier
		uint32 resizeFlags)
	:	TransportButton(frame, name, normalBits, pressedBits,
			disabledBits, invokeMessage, 0,
			0, 0, 0, key, modifiers, resizeFlags),
		normalRecordingBits(normalRecordingBits),
		pressedRecordingBits(pressedRecordingBits),
		state(RecordButton::kStopped),
		lastRecordBlinkTime(0),
		lastModeMask(0)
{
}
...
const bigtime_t kRecordBlinkPeriod = 600000;

void 
RecordButton::SetRecording()
{
	// in record state blink the LED on and off
	bigtime_t now = system_time();
	if (state == kRecordLedOn || state == kRecordLedOff) {
		if (now - lastRecordBlinkTime < kRecordBlinkPeriod)
			return;
		if (state == kRecordLedOn)
			state = kRecordLedOff;
...
RecordButton::ModeMask() const
{
	...
	if (state != kStopped && state == kRecordLedOn)
...
RecordButton::BitsForMask(uint32 mask) const
{
	switch (mask) {
		case kRecordingMask:
			return normalRecordingBits;
		case kRecordingMask | kPressedMask:
			return pressedRecordingBits;
		default:
			return _inherited::BitsForMask(mask);
	}	
	TRESPASS();
...
{
	if (previousState == kStopped) {
	 	state = kRecordLedOn;
		lastRecordBlinkTime = system_time();
	} else
		state = kStopped;
