...
void 
ReplaceColor(BBitmap *bitmap, uint8 from, uint8 to)
{
...
ReplaceColor(BBitmap *bitmap, rgb_color from, rgb_color to)
{
	...
	uint8 fromIndex = screen.IndexForColor(from);
	uint8 toIndex = screen.IndexForColor(to); 
	ReplaceColor(bitmap, fromIndex, toIndex);
}
