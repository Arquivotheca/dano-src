...
status_t ConfigureOverlay (overlay_token ot, const overlay_buffer * ob, const overlay_window * ow, const overlay_view * ov)
{
	...
	uint32 reg_vidProcCfg = ci->reg.r_V3_VID_PROC_CFG;
	uint32 reg_vidDesktopOverlayStride = ci->reg.r_V3_VID_DESKTOP_OVERLAY_STRIDE;
	...
	if (!tot->used)
	{
		dprintf (("3dfx_accel: ConfigureOverlay - trying to configure un-allocated overlay\n"));
		return B_ERROR;
	}

	if( ov && ow && ((ov->width > ow->width) || (ov->height > ow->height)) )
	{
		dprintf (("3dfx_accel: ConfigureOverlay - trying to configure overlay with scale < 1\n"));
		return B_ERROR;
	}
...
//          SST_OVERLAY_DEINTERLACE_EN    |  // Disable backend deinterlace
	...
	_V3_WriteReg_NC( ci, V3_3D_LEFT_OVERLAY_BUF, base_address );
	_V3_WriteReg_NC( ci, V3_3D_SWAP_BUFFER_CMD, 1 );

	_V3_WriteReg_NC( ci, V3_VID_CHROMA_MIN, reg_vidOverlayChroma );
	_V3_WriteReg_NC( ci, V3_VID_CHROMA_MAX, reg_vidOverlayChroma );
	_V3_WriteReg_NC( ci, V3_VID_OVERLAY_DUDX_OFFSET_SRC_WIDTH, reg_vidOverlayDudxOffsetSrcWidth );
	_V3_WriteReg_NC( ci, V3_VID_OVERLAY_DUDX, reg_vidOverlayDudx );
	_V3_WriteReg_NC( ci, V3_VID_OVERLAY_DVDY, reg_vidOverlayDudy );
	_V3_WriteReg_NC( ci, V3_VID_OVERLAY_START_COORDS, reg_vidOverlayStartCoords );
	_V3_WriteReg_NC( ci, V3_VID_OVERLAY_END_SCREEN_COORD, reg_vidOverlayEndScreenCoord );
	_V3_WriteReg_NC( ci, V3_VID_PROC_CFG, reg_vidProcCfg );
