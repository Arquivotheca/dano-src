 ...
 * $Header: u:/rcs/cv/rcs/draw32.c 3.3 1994/10/20 17:39:29 bog Exp $
 ...
 * $Log: draw32.c $
 * Revision 3.3  1994/10/20 17:39:29  bog
 * Modifications to support Sunnyvale Reference version.
 * Revision 3.2  1994/04/30  10:51:16  unknown
 * (bog)  Clean up DECOMPRESS_SET_PALETTE for C version.
 *
 * Revision 3.1  1994/01/13  09:58:18  geoffs
 * Now works on MIPS
 *
 * Revision 3.0  1993/12/10  14:24:05  timr
...
void far DrawSmooth32(
  DCONTEXT *,			// -> decompress context
  unsigned char *,		// flat -> incoming compressed indices
  unsigned char *, 		// offset for DIB bits to fill
  short				// height of tile
);
void far DrawInter32(
  DCONTEXT *,			// -> decompress context
  unsigned char *,		// flat -> incoming compressed indices
  unsigned char *, 		// offset for DIB bits to fill
  short				// height of tile
...
)
{
	...
	unsigned long YDelta = yStep4 - pDC->Width * 4;
	...
	while (nVPatches--)
	{
		...
		for (cWidth = 0; cWidth < pDC->Width / 4; cWidth++)
		{
			if (NEXT_SWITCH (pIx))	// bit set means DETAIL
			{
				...
				*(RGB32 *)(pBits+ 4       ) = peDetail[pIx[0]].Pixel1;
				*(RGB32 *)(pBits   +yStep1) = peDetail[pIx[0]].Pixel2;
				*(RGB32 *)(pBits+ 4+yStep1) = peDetail[pIx[0]].Pixel3;
				*(RGB32 *)(pBits+ 8       ) = peDetail[pIx[1]].Pixel0;
				*(RGB32 *)(pBits+12       ) = peDetail[pIx[1]].Pixel1;
				*(RGB32 *)(pBits+ 8+yStep1) = peDetail[pIx[1]].Pixel2;
				*(RGB32 *)(pBits+12+yStep1) = peDetail[pIx[1]].Pixel3;
				*(RGB32 *)(pBits   +yStep2) = peDetail[pIx[2]].Pixel0;
				*(RGB32 *)(pBits+ 4+yStep2) = peDetail[pIx[2]].Pixel1;
				*(RGB32 *)(pBits   +yStep3) = peDetail[pIx[2]].Pixel2;
				*(RGB32 *)(pBits+ 4+yStep3) = peDetail[pIx[2]].Pixel3;
				*(RGB32 *)(pBits+ 8+yStep2) = peDetail[pIx[3]].Pixel0;
				*(RGB32 *)(pBits+12+yStep2) = peDetail[pIx[3]].Pixel1;
				*(RGB32 *)(pBits+ 8+yStep3) = peDetail[pIx[3]].Pixel2;
				*(RGB32 *)(pBits+12+yStep3) = peDetail[pIx[3]].Pixel3;
			...
			else			// bit clear means SMOOTH
			{
				...
				*(RGB32 *)(pBits+ 4       ) =
				*(RGB32 *)(pBits   +yStep1) =
				*(RGB32 *)(pBits+ 4+yStep1) = peSmooth[pIx[0]].Pixel0;
				*(RGB32 *)(pBits+ 8       ) =
				*(RGB32 *)(pBits+12       ) =
				*(RGB32 *)(pBits+ 8+yStep1) =
				*(RGB32 *)(pBits+12+yStep1) = peSmooth[pIx[0]].Pixel1;
				*(RGB32 *)(pBits   +yStep2) =
				*(RGB32 *)(pBits+ 4+yStep2) =
				*(RGB32 *)(pBits   +yStep3) =
				*(RGB32 *)(pBits+ 4+yStep3) = peSmooth[pIx[0]].Pixel2;
				*(RGB32 *)(pBits+ 8+yStep2) =
				*(RGB32 *)(pBits+12+yStep2) =
				*(RGB32 *)(pBits+ 8+yStep3) =
				*(RGB32 *)(pBits+12+yStep3) = peSmooth[pIx[0]].Pixel3;
