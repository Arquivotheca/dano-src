...
#define WARNING DO_NOTHING //FPRINTF
#define LATE FPRINTF
#define TRANSPORT DO_NOTHING //FPRINTF
#define NODE DO_NOTHING //FPRINTF
#define TIMING DO_NOTHING //FPRINTF
#define FORMAT FPRINTF
#define EVENT DO_NOTHING //FPRINTF
#define CALL	FPRINTF
	...
	BMediaEventLooper()
{
	...
	m_required_format = media_raw_audio_format::wildcard;
	if (format != NULL) {
		memcpy(&m_required_format, format, sizeof(media_raw_audio_format));
	}
	if (m_required_format.frame_rate <= media_raw_audio_format::wildcard.frame_rate) {
		m_required_format.frame_rate = PREFERRED_FRAME_RATE;
	}
	if (m_required_format.format <= media_raw_audio_format::wildcard.format) {
		m_required_format.format = PREFERRED_SAMPLE_FORMAT;
	}
	if (m_required_format.byte_order <= media_raw_audio_format::wildcard.byte_order) {
		m_required_format.byte_order = PREFERRED_BYTE_ORDER;
	}
	if (m_required_format.channel_count <= media_raw_audio_format::wildcard.channel_count) {
		m_required_format.channel_count = PREFERRED_CHANNEL_COUNT;
	}
	if (m_required_format.buffer_size <= media_raw_audio_format::wildcard.buffer_size) {
		m_required_format.buffer_size = PREFERRED_BUFFER_SIZE;
	}
	m_output.format.type = B_MEDIA_RAW_AUDIO;
	m_output.format.u.raw_audio = media_raw_audio_format::wildcard;
	m_current_format = m_output.format.u.raw_audio;
		...
		case BTimedEventQueue::B_HANDLE_BUFFER:
			{
				...
				if (buf)
				{
					...
					m_frames_played += buf->Header()->size_used/frame_size(m_current_format);
					if (!m_connected || SendBuffer(buf, m_output.destination) < B_OK)
					{
						buf->Recycle();
					}
				}
...
//	printf("creating new buffer for %Ld (delta=%Ld)\n",buffer_perf,m_delta);
	BBuffer *buffer = m_buffers->RequestBuffer(m_current_format.buffer_size);
	if (buffer) {
		if (m_player->_PlayBuffer) {
			...
			(*m_player->_PlayBuffer)(
				m_player->_m_cookie, buffer->Data(), m_current_format.buffer_size, 
				m_current_format);
		} else {
			m_player->PlayBuffer(
				buffer->Data(), m_current_format.buffer_size, 
				m_current_format);
		}
		buffer->Header()->size_used = m_current_format.buffer_size;
	};
	return buffer;
	...
	media_format * format)
{
	...
	format->u.raw_audio.frame_rate = m_required_format.frame_rate;
	format->u.raw_audio.channel_count = m_required_format.channel_count;
	format->u.raw_audio.format = m_required_format.format;
	format->u.raw_audio.byte_order = m_required_format.byte_order;
	if (m_current_format.buffer_size > media_raw_audio_format::wildcard.buffer_size)
		format->u.raw_audio.buffer_size = m_current_format.buffer_size;
	else
		format->u.raw_audio.buffer_size = 0; //m_required_format.buffer_size;
#if !NDEBUG
	char fmt[100];
	...
	media_format * format)
{
	...
	if (format->type <= 0) {
		...
		else if (format->u.raw_audio.channel_count != m_required_format.channel_count) {
			goto err;
		}
		if (format->u.raw_audio.format <= media_raw_audio_format::wildcard.format) {
			format->u.raw_audio.format = m_required_format.format;
		...
		if (m_current_format.buffer_size > media_raw_audio_format::wildcard.buffer_size) {
			// be picky about buffer size only if a format was previously negotiated
			if (format->u.raw_audio.buffer_size <= media_raw_audio_format::wildcard.buffer_size) {
				format->u.raw_audio.buffer_size = m_current_format.buffer_size;
			}
			else if (format->u.raw_audio.buffer_size != m_current_format.buffer_size) {
				goto err;
			}
		}
...
//			format->u.raw_audio.buffer_size = m_required_format.buffer_size;
//		}
	...
	bigtime_t * out_latency)
{
	...
	if (err >= B_OK) {
		CALL(stderr, "  BBufferProducer::GetLatency() -> %Ld\n", *out_latency);
	...
	char * out_name)
{
	...
	else if (!m_output.format.u.raw_audio.buffer_size) {
	...
	char * io_name)
{
	...
	m_output.format = format;
	m_current_format = format.u.raw_audio;
	m_required_format = m_current_format;
	m_connected = true;
...
void _SoundPlayNode::alloc_buffers()
{
	...
	if (!m_connected)
	{
		// allocate a single buffer as scratch space
		m_buffers = new BBufferGroup(m_current_format.buffer_size, 1);
		return;
	}

	bigtime_t latency = m_private_latency + m_downstream_latency;
	int count = (int)(latency/(1000000.0*m_current_format.buffer_size/(frame_size(
	...
	if (count*m_current_format.buffer_size > 128000) {
		count = 128000/m_current_format.buffer_size;
		// make sure there's at least one buffer, though
		if (count < 1) count = 1;
	}
FPRINTF(stderr, "SoundPlayNode::Connect() format %g;%lx;%ld;%ld;%ld\n", m_output.format.u.raw_audio.frame_rate, 
		m_output.format.u.raw_audio.format, m_output.format.u.raw_audio.channel_count, 
		m_output.format.u.raw_audio.byte_order, m_current_format.buffer_size);
#if NDEBUG
	if (BPrivate::media_debug) 
#endif
		DIAGNOSTIC(stderr, "SoundPlayNode creating %d buffers of size %ld for latency %Ld\n",
			count,m_current_format.buffer_size, latency);
	m_buffers = new BBufferGroup(m_current_format.buffer_size, count);
}
...
//  allow buffer-production events to keep flowing
//	EventQueue()->FlushEvents(B_INFINITE_TIMEOUT,BTimedEventQueue::B_BEFORE_TIME);
	...
	// allocate an interim buffergroup (used as scratch space so the hook can
	// continue producing audio)
	alloc_buffers();
}
...
void _SoundPlayNode::LateNoticeReceived(
	const media_source & /*what*/,
	bigtime_t how_much,
	bigtime_t DEBUGARG(performance_time))
{
	...
	case B_INCREASE_LATENCY:
		...
		break;
	default:	/* drop data */
		m_frames_played += m_current_format.buffer_size/frame_size(m_output.format.u.raw_audio);
		break;
	}
}
