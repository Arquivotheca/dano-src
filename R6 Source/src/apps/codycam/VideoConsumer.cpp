...
VideoConsumer::~VideoConsumer()
{
	...
	if (mWindow)
	{
		PROGRESS("Locking the window\n");
		if (mWindow->Lock())
		{
			PROGRESS("Closing the window\n");
			mWindow->Close();
			mWindow = 0;
		}
	}
	...
	media_input * out_input)
{
	...
	*out_input = mIn;
	...
	/* make sure we create an appropriate bitmap for the FTP translation */
	uint32 mXSize = with_format.u.raw_video.display.line_width;
	uint32 mYSize = with_format.u.raw_video.display.line_count;	
	color_space colordepth = with_format.u.raw_video.display.format;
	PROGRESS("VideoConsumer::Connected - colorspace: %d\n", with_format.u.raw_video.display.format);
	mFtpBitmap = new BBitmap(BRect(0, 0, mXSize-1, mYSize-1), colordepth, false, false);
				...
				const media_format &format)
{
	...
	mFtpBitmap = new BBitmap(BRect(0, 0, mXSize-1, mYSize-1), B_RGB32, false, false);
		...
		case BTimedEventQueue::B_HANDLE_BUFFER:
			...
			if (RunState() == B_STARTED && mConnectionActive)
			{
				...
				if (index == 3)
				{
					...
					PROGRESS("VidConsumer::HandleEvent - BUFFER BELONGS TO CONSUMER\n");
					mOurBuffers = false;
					index = 0;
				}
...
VideoConsumer::FtpSave(char *filename)
{
	...
	if (ftp.connect((string)mServerText, (string)mLoginText, (string)mPasswordText))		// connect to server
	{
		...
		{												// cd to the desired directory
			...
			if (ftp.putFile((string)filename, (string)"temp.jpg"))			// send the file to the server
			{
				UpdateFtpStatus("Renaming ...");
				if (ftp.moveFile((string)"temp.jpg",(string)filename))		// change to the desired name
				{
