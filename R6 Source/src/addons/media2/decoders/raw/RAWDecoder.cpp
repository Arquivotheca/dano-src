...
extern "C" B::Private::Decoder *instantiate_decoder() {
//	if (sgn8_encoding == 0) return 0;
	return new RAWDecoder();
}

RAWDecoder::RAWDecoder() {
berr << "RawDecoder()" << endl;
...
status_t
RAWDecoder::Sniff(const media_format *in_format, const void *, size_t)
{
	...
	while (needed_size > 0) {
		if (size > 0) {
			if (size > needed_size)
				...
				copy_size = size;
if ((uint32)in_buffer < (uint32)0x4000 || (uint32)in_buffer > (uint32)0xfffff000) {
	char buff[256];
	sprintf(buff, "in_buffer %p copy_size %ld, needed_size %ld size %ld offset %Ld\n",
...
if ((uint32)in_buffer < 0x4000 || (uint32)in_buffer > (uint32)0xfffff000) {
	char buff[256];
	sprintf(buff, "in_buffer %p offset %Ld\n", in_buffer, offset);
	debugger(buff);
}
