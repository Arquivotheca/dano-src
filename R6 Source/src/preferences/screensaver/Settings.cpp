...
#define WY 325
	...
	lastframe.Set((frame.Width() - WX) / 2, (frame.Height() - WY) / 2, (frame.Width() - WX) / 2 + WX, (frame.Height() - WY) / 2 + WY);

	if(m->HasRect(kWindowFrame))
	{
		m->FindRect(kWindowFrame, &lastframe);
		if(lastframe.left > frame.right || lastframe.top > frame.bottom)
		{
			lastframe.left = (frame.right - frame.left - WX) / 2;
			lastframe.top = (frame.bottom - frame.top - WY) / 2;
		}
		lastframe.right = lastframe.left + WX;
		lastframe.bottom = lastframe.top + WY;
		m->ReplaceRect(kWindowFrame, lastframe);
	}
	else
		m->AddRect(kWindowFrame, lastframe);
