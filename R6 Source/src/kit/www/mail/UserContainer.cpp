...
UserContainer &UserContainer::operator=(const UserContainer &container)
{
	if (&container != this) {
		...
		fServerType = container.fServerType;
	}
	return *this;
}
...
void 
UserContainer::SetServerType(const char *serverType)
{
	fServerType = serverType;
}
...
const char *
UserContainer::GetServerType() const
{
	return fServerType.String();
}
...
bool UserContainer::SetTo(const char *user)
{
	...
	fServerType = node["ServerType"];

	if (fServerType.ICompare("POP3") == 0) {
		fServer = node["Pop3Server"];
		fPort =int(node["Pop3Port"].Number());
	}
	else {
		// default to Imap
