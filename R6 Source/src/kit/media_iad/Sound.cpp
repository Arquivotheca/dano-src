...
#define GET_DATA //FPRINTF
#define ENDIAN //FPRINTF
#define THREADS //FPRINTF
	...
	bool free_when_done)
{
	...
	for (int ix=0; ix<sizeof(targets)/sizeof(float); ix++) {
...
read_from_disk:
				...
				if (!_m_data) {
					...
					char tname[32];
					sprintf(tname, "_loader:%.23s", sound_file->name);
					_m_loader_thread = spawn_thread(load_entry, tname, prio, this);
...
void BSound::loader_thread()
{
	...
	while (running) {
		...
		if ((load_start = (system_time() - load_start)) > warning_time) {
			fprintf(stderr, "BSound: it took %g ms to load another chunk!\n", (double)load_start/1000.0);
		}
		if (used <= 0) {
			_m_size = total_loaded;
			break;
