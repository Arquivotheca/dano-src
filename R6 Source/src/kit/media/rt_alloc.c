...
#if !NDEBUG
#define assert(x,y) do { if (!(x)) { fprintf(stderr, "%s:%d: thr %ld: assertion failed: %s\n", __FILE__, __LINE__, find_thread(NULL), #x); printf y ; fflush(stdout); debugger("assertion failed"); } } while (0)
#define DEBUGVAR(ident) ident
#else
#define assert(x,y) (void)0
#define DEBUGVAR(ident)
#endif
...
static int valid_pool_ptr(rtm_pool * p)
{
	...
	if (id < 0) {
		char str[100];
		sprintf(str, "Address %p is not in an area!\n", p);
		debugger(str);
		return 0;
	}
	return 1;
...
static void rtm_validate_pool(rtm_pool * pool)
{
	...
	int total_blocks = 0;
	int free_size = 0;
	DEBUGVAR(status_t err);
	DEBUGVAR(area_info info);
	
	(void)valid_pool_ptr(pool);
	/* validate the pool */
	assert(pool != NULL,("NULL"));
	assert(pool->lock_count < 0,("%ld\n", pool->lock_count));
	assert((err = get_area_info(pool->area, &info)) >= B_OK,("0x%lx\n", err));
	assert(info.address == (void *)pool,("%p %p\n", info.address, pool));
	assert(info.size == pool->total_size+sizeof(rtm_pool),("0x%lx 0x%lx", info.size, pool->total_size+sizeof(rtm_pool)));
	assert(pool->free_list.prev == NULL,("%p\n", pool->free_list.prev));
	assert(pool->free_blocks < pool->total_blocks || (pool->free_blocks == 1 && pool->total_blocks == 1), \
		("0x%lx 0x%lx\n", pool->free_blocks, pool->total_blocks));
	assert(pool->free_size <= (ssize_t)(pool->total_size-sizeof(rtm_block)*pool->free_blocks), \
		("0x%lx 0x%lx\n", pool->free_size, pool->total_size-sizeof(rtm_block)*pool->free_blocks));
	assert(pool->min_free_size <= pool->free_size,("0x%lx, 0x%lx\n", pool->min_free_size, pool->free_size));
	...
	while (fl->next != NULL) {
		...
		assert((char*)fl->next >= (char *)bl, ("%p %p\n", fl->next, bl));
		assert((char*)fl->next < (char *)en, ("%p %p\n", fl->next, en));
		assert(fl->next->prev == fl,("%d: %p %p %p\n", total_free, fl, fl->next, fl->next->prev));
		assert(fl->prev == fp,("%p %p %px\n", fl, fl->prev, fp));
		pr = &((rtm_block *)fl->next)[-1];
		assert(pr->log_size == FREE_BLOCK,("%p 0x%lx\n", pr, pr->log_size));
		free_size += pr->phys_size;
		fp = fl;
		fl = fl->next;
	}
	assert(total_free == pool->free_blocks,("%d %ld\n", total_free, pool->free_blocks));
	assert(free_size == pool->free_size,("0x%x 0x%lx\n", free_size, pool->free_size));
	...
	while (bl < en) {
		total_blocks++;
		assert(bl->pool == pool, ("%p %p %p\n", bl, bl->pool, pool));
		assert(bl >= (rtm_block*)&pool[1],("%p %p %p\n", bl, pool, &pool[1]));
		...
		assert(bl->phys_size >= (ssize_t)sizeof(rtm_free_list),("%p 0x%lx\n", bl, bl->phys_size));
		assert(!(bl->phys_size&(PADSIZE-1)), ("%p 0x%lx\n", bl, bl->phys_size));
		assert(bl->log_size <= bl->phys_size, ("%p 0x%lx 0x%lx\n", bl, bl->log_size, bl->phys_size));
		assert((bl->log_size == FREE_BLOCK) ||
			(bl->log_size >= (int32)((double)bl->phys_size*(double)REALLOC_SPLIT_PCT/100.0)) ||
			(bl->log_size < 0x10),
			("%p 0x%lx 0x%lx\n", bl, bl->log_size, bl->phys_size));
		assert(bl->log_size >= FREE_BLOCK,("%p 0x%lx\n", bl, bl->log_size));
		assert(bl->prev == pr, ("%p %p %p\n", bl, bl->prev, pr));
		pr = bl;
		bl = (rtm_block *)((char *)&bl[1]+bl->phys_size);
	}
	assert(bl == en,("%p %p\n", bl, en));
	assert(total_blocks == pool->total_blocks,("%d %ld\n", total_blocks, pool->total_blocks));
}
...
defaults:
	...
	if (is_debugging > 1) {
		fprintf(stderr, "rtm_create_area_etc(%ld, %s) returns %ld at %p\n", total_size, name, area, start);
	}
...
status_t rtm_delete_pool(rtm_pool * pool)
{
	...
	if (is_debugging > 1) {
		fprintf(stderr, "rtm_delete_pool(%p) called\n", pool);
		(void)valid_pool_ptr(pool);
	}
	while (pool) {
		status_t err;
		...
		assert(pool->area > 0,("0x%lx", pool->area));
...
void * rtm_alloc(rtm_pool * pool, size_t size)
{
	...
	printf("%6d: rtm_alloc %d at %08x\n",find_thread(NULL),size,mem);
	return mem;
#else
	const char *ptr;
	int max_pools;
...
//		PRINT(("max_pools allocated... fail\n"));
	...
	if (size > pool_size-sizeof(rtm_block)) {
		if (getenv("MEDIA_LOCKED_MEMORY_DEBUG") != 0) {
			fprintf(stderr, "Tried to allocate %lu bytes from pool size %d\n", size, pool_size);
		}
		if (atomic_add(&unlock->lock_count, 1) < -1) {
			release_sem(unlock->lock_sem);
		}
...
static void * rtm_internal(rtm_pool * pool, size_t size)
{
	...
	int32 padded_size = (((size > 0) ? size : 1) + PADSIZE-1) & ~(PADSIZE-1);
	assert(pool != NULL,("NULL\n"));
	...
	list = &pool->free_list;
	if (!list->next || ((size_t)(pool->free_size) < size)) {
		return NULL;	/*	guaranteed nothing big enough	*/
	}
	while (list->next && (size_t)((block = &((rtm_block *)list->next)[-1])->phys_size) < size) {
		list = list->next;
	}
	if (!list->next) {
		return NULL;	/*	nothing big enough	*/
	}
	assert(block->log_size == FREE_BLOCK,("%p 0x%lx\n", block, block->log_size));
	assert(block->pool == pool,("%p %p %p\n", block, block->pool, pool));
	assert(list->next->prev == list,("%p %p %p\n", list, list->next, list->next->prev));
	assert(block->phys_size >= padded_size,("%p 0x%lx 0x%lx\n", block, block->phys_size, padded_size));
	/*	split the block?	*/
	if (block->phys_size > padded_size+PADDING_OK) {
		...
		if (after < (rtm_block *)((char *)&pool[1] + pool->total_size)) {
			assert(after->log_size >= 0,("%p 0x%lx\n", after, after->log_size));
			after->prev = next;
		}
...
status_t rtm_free(void * data)
{
	...
	assert(pool != NULL,("NULL\n"));
	assert(valid_pool_ptr(pool), ("data %p, pool %p\n", data, pool));
	assert(block->phys_size >= block->log_size,("%p 0x%lx 0x%lx\n", block, block->phys_size, block->log_size));
	assert(block->phys_size <= pool->total_size,("%p 0x%lx %p 0x%lx\n", block, block->phys_size, pool, pool->total_size));
...
//		fprintf(stderr, "TRACE: merge next free block @ %x\n", &next[1]);
	...
	if (prev && prev->log_size == FREE_BLOCK) {
		...
		assert((rtm_block *)((char *)&prev[1]+prev->phys_size) == block,("%p 0x%lx %p\n", &prev[1], prev->phys_size, block));
...
static status_t _rtm_merge(rtm_pool * pool, rtm_block * block, rtm_block *next)
{
...
rtm_realloc(void ** data, size_t new_size)
{
	...
	if(block->log_size == (int32)new_size) {
		return B_OK;
	}
	...
	assert(block->phys_size >= block->log_size,("%p 0x%lx 0x%lx\n", block, block->phys_size, block->log_size));
	assert(block->phys_size <= pool->total_size,("%p 0x%lx %p 0x%lx\n", block, block->phys_size, pool, pool->total_size));
	assert(block >= (rtm_block*)&pool[1],("%p %p %p\n", block, pool, &pool[1]));
	assert((char*)&block[1]+block->phys_size <= (char*)&pool[1]+pool->total_size,("%p 0x%lx %p %p 0x%lx %p\n", block, block->phys_size, (char*)&block[1]+block->phys_size, pool, pool->total_size, (char*)&pool[1]+pool->total_size));
	...
	if(new_size > (size_t)block->phys_size) {
		int32 next_size;
		int32 prev_size;
		...
		if(padded_size <= block->phys_size + next_size) {
			...
			assert((ssize_t)new_size <= block->phys_size, ("%ld %p %ld\n", new_size, block, block->log_size));
			n_data = *data;
...
void rtm_dump_block(rtm_pool * pool, void * data)
{
	...
	if (data) {
		...
		assert(block->pool == pool,("%p %p %p\n", block, block->pool, pool));
		fprintf(stderr, "pool: %p through 0x%lx\n", pool, pool->total_size+sizeof(rtm_pool));
		fprintf(stderr, "block: %p phys 0x%lx log 0x%lx prev %p pool %p\n", block,
			block->phys_size, block->log_size, block->prev, block->pool);
		if (block->log_size < 0) {
			rtm_free_list * fl = (rtm_free_list *)&block[1];
			fprintf(stderr, "   free_next: %p free_prev: %p\n", fl->next, fl->prev);
		}
	}
	else {
		rtm_block * block = (rtm_block *)&pool[1];
		fprintf(stderr, "pool: %p first block: %p end: %p\n", pool,
			&((rtm_block*)&pool[1])[1], (char *)&pool[1] + pool->total_size);
		fprintf(stderr, "first free block: %p\n", pool->free_list.next);
		fprintf(stderr, "total size: %lu  free size: %lu  min free: %lu  total blocks: %lu  free blocks: %lu\n",
