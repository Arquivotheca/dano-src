#ifndef lint
static char rcsid[] = "$Header: /net/bally/be/rcs_rel/src/apps/3dmov/tiff/tif_read.cpp,v 1.1 1996/12/22 23:45:23 pierre Exp $";
#endif
...
TIFFReadEncodedStrip(TIFF* tif, tstrip_t strip, tdata_t buf, tsize_t size)
{
	...
	if (size == (tsize_t) -1)
		...
		size = stripsize;
	if (TIFFFillStrip(tif, strip) &&
    (*tif->tif_decodestrip)(tif, (tidata_t)buf, size, strip / td->td_stripsperimage)) {
...
TIFFReadEncodedTile(TIFF* tif, ttile_t tile, tdata_t buf, tsize_t size)
{
	...
	if (TIFFFillTile(tif, tile) && 
	    (*tif->tif_decodetile)(tif, (tidata_t)buf, size, tile/td->td_stripsperimage)) {
		(*tif->tif_postdecode)(tif, (tidata_t)buf, size);
		return (size);
	} else
		return (-1);
