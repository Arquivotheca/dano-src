...
#include <graphics_p/3dfx/voodoo4/voodoo4_defs.h>
...
//	dprintf("3dfx: setcursorshape - curs = 0x%x, curswide = %d, curshigh = %d, hot_x = %d, hot_y = %d\n", curs, curswide, curshigh, hot_x, hot_y);
	...
	_V5_WriteReg_NC( ci, V5_VID_CUR_C_0, (0xff << SST_CURSOR_RED_SHIFT) | (0xff << SST_CURSOR_GREEN_SHIFT) | (0xff << SST_CURSOR_BLUE_SHIFT));
	_V5_WriteReg_NC( ci, V5_VID_CUR_C_1, (0x00 << SST_CURSOR_RED_SHIFT) | (0x00 << SST_CURSOR_GREEN_SHIFT) | (0x00 << SST_CURSOR_BLUE_SHIFT));

	/*  Set cursor image address  */
	dstoff = (uint32) ci->CursorBase - (uint32) ci->BaseAddr1;
	_V5_WriteReg_NC( ci, V5_VID_CUR_PAT_ADDR, dstoff);
	...
	_V5_WriteReg_NC( ci, V5_VID_CUR_LOC, ((ci->MousePosX - ci->MouseHotX + 64) << SST_CURSOR_X_SHIFT) | ((ci->MousePosY - ci->MouseHotY + 64) << SST_CURSOR_Y_SHIFT));

//	dprintf("3dfx_accel: setcursorshape - EXIT\n");
	return (B_OK);
}

void _V5_show_cursor (bool on)
{
	register uint32		tmp;

	tmp = ci->reg.r_V5_VID_PROC_CFG;
	...
	_V5_WriteReg_NC( ci, V5_VID_PROC_CFG, tmp );
}

void _V5_move_cursor (int16 x, int16 y)
{
	uint32 tmp;
